...
while IFS= read -r key; do
  ...
  if printf '%s\n' "$SCLEAN" | grep -q "No test files found"; then
    echo "[FAIL] 샤드 [$key] 수집 0 / 공허 산출물 — vitest 가 테스트 파일을 찾지 못했다." >&2
  ...
  if [ -z "$SPASSED" ] || [ "$SPASSED" = "0" ]; then
    echo "[FAIL] 샤드 [$key] 통과 테스트 0 —$STESTS_LINE — 미측정을 초록으로 읽지 않는다." >&2
    FAILED=1
    continue
  fi
  # 열거와 실제 수집의 샤드 단위 정합 (필터 과수집/과소수집 차단).
  if [ "$SFILES" != "$EXPECT_FILES" ]; then
    echo "[FAIL] 샤드 [$key] 수집 파일 $SFILES != 열거 $EXPECT_FILES — 필터가 대상 집합을 왜곡했다" >&2
    FAILED=1
    continue
  fi
  if [ ! -f "$SDIR/coverage-final.json" ]; then
    echo "[FAIL] 샤드 [$key] coverage-final.json 미생성" >&2
    FAILED=1
    continue
  fi
...
if [ "$FAILED" != "0" ]; then
  echo "[FAIL] check:coverage-attribution — 위 샤드에서 단조성/실행 사실 판정이 실패했다" >&2
  exit 1
fi
