...
describe('파서 산출 ↔ sanitizer 허용 정합', () => {
	...
	//
	// 분해·전개 변형은 종전 게이트에서 rc=1 이었다 (배열이 둘이 되어 "정확히 하나" 가 깨짐).
	// 상수 소멸 변형은 종전 게이트에서 rc=0 이었다 (주석 잔존 배열을 후보로 잡고, 옛 태그는
	...
	// 주석을 걷는다. 상수 소멸 변형이 정확히 **주석 잔존**으로 들어온다 — 옛 정책을 `//` 로
	...
	// 분해·전개 변형이 배열을 쪼개도, 상수 소멸 변형이 배열을 없애고 인라인해도 따라간다.
	...
	it('허용 목록의 모든 태그가 sanitize 를 통과한다', () => {
		...
		// 있으면 그 태그를 쓰는 글은 무측정이다 — 상수 소멸 변형의 신규 태그가 여기서 걸린다.
		expect(missing(new Set(effectiveTags), exercised),
			'정책이 살려 보내지만 corpus 가 한 번도 통과시켜 보지 않는 태그 — 그 태그를 쓰는 글은 무측정이다').toEqual([]);

		// **역방향 교차 검증 — 효력면 도출이 실제 효력보다 좁으면 붉다.**
		//
		// 이 단언이 부모 문맥 프로브를 **동작으로 떠받친다.** 문맥을 주지 않는
		// 프로브로 되돌리면 thead·tbody·tr·th·td 가 효력 집합에서 빠지는데,
		// corpus 의 파이프 표 행은 그것들을 실제로 통과시킨다 — 두 집합이 갈리고
		// 여기서 잡힌다. 이 단언이 없으면 효력면 기계를 들어내도 위 단언은
		// 한 방향(효력 ⊆ corpus)만 재므로 **주석만 남기고 통과**한다 (실측).
		expect(missing(exercised, new Set(effectiveTags)),
			'corpus 가 통과시킨 태그가 효력 집합에 없다 — 효력면 도출이 실제 효력보다 좁다').toEqual([]);
	});
});
