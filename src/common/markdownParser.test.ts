...
describe('MD parsing — setext heading underline', () => {
	...
	// (I7) 밑줄이 올리는 것은 **바로 앞 한 줄**이다.
	//
	// CommonMark §4.3 은 밑줄이 앞 **문단 전체**를 제목으로 올린다고 적지만,
	// 이 파서에는 **여러 줄 문단이 없다** — `한 줄\n두 줄` 은 그 자체로
	// `<p>한 줄</p><p>두 줄</p>` 다. 그래서 여기서 밑줄이 올리는 단위는 한 줄이며
	// 그것이 현 산출이자 계약이다. 새 규칙이 아니라 이미 지배되던 자리를
	// 문면으로 옮긴 것이다.
	//
	// 마지막 단언이 **이 계약의 전제를 명시하는 자리**다. 전제(여러 줄 문단
	// 없음)가 없으면 앞의 세 단언은 CommonMark 위반으로 읽힌다. 전제를 같은
	// 블록에 두어 문단 모델이 도입되는 날 이 게이트가 **함께** 붉어지게 한다.
	//
	// `-` 축과 `=` 축을 한 블록에서 둘 다 잠근다 — 한쪽만 잠그면 다른 쪽이
	// 조용히 갈린다.
	it('밑줄은 바로 앞 한 줄만 제목으로 올린다', () => {
		expect(parser.markdownToHtml('한 줄\n두 줄\n---'))
			.toBe('<p>한 줄</p><h2>두 줄</h2>');
		expect(parser.markdownToHtml('한 줄\n두 줄\n==='))
			.toBe('<p>한 줄</p><h1>두 줄</h1>');
		expect(parser.markdownToHtml('한 줄\n두 줄\n세 줄\n---'))
			.toBe('<p>한 줄</p><p>두 줄</p><h2>세 줄</h2>');
		// 전제: 밑줄이 없으면 두 줄은 두 문단이다.
		expect(parser.markdownToHtml('한 줄\n두 줄'))
			.toBe('<p>한 줄</p><p>두 줄</p>');
	});
});
...
// (I9) 밑줄 구분자의 **조건 1** 여는 쪽. 여는 구분자 뒤가 공백이면 열지 않는다.
// 현 HEAD 산출을 계약으로 못 박는다 — 동작은 이미 옳고 없던 것은 게이트다.
// 닫는 쪽 (I10) 과 **이름을 나눈다**: 닫는 쪽 판정만 제거하는 주입에서 이 블록은
// rc=0 이고 (I10) 만 붉다. 한 이름으로 합치면 그 비대칭이 보이지 않는다.
describe('밑줄 구분자는 공백을 사이에 두면 강조를 열지 않는다', () => {

	it('여는 밑줄 뒤가 공백이면 양쪽이 떨어진 기울임을 열지 않는다', () => {
		expect(parser.markdownToHtml('_ 기울임 _').trim())
			.toBe('<p>_ 기울임 _</p>');
	});

	it('여는 겹밑줄 뒤가 공백이면 양쪽이 떨어진 굵게를 열지 않는다', () => {
		expect(parser.markdownToHtml('__ 굵게 __').trim())
			.toBe('<p>__ 굵게 __</p>');
	});

	it('여는 밑줄 뒤가 공백이면 닫는 쪽이 붙어 있어도 열지 않는다', () => {
		expect(parser.markdownToHtml('_ 기울임_').trim())
			.toBe('<p>_ 기울임_</p>');
	});

	it('여는 겹밑줄 뒤가 공백이면 닫는 쪽이 붙어 있어도 열지 않는다', () => {
		expect(parser.markdownToHtml('__ 굵게__').trim())
			.toBe('<p>__ 굵게__</p>');
	});
});

// (I10) 밑줄 구분자의 **조건 1** 닫는 쪽. 닫는 구분자 앞이 공백이면 닫지 않는다.
// 여는 자격만 있고 짝이 없으므로 구분자는 글자로 남는다.
describe('밑줄 구분자는 공백을 사이에 두면 강조를 닫지 않는다', () => {

	it('닫는 밑줄 앞이 공백이면 기울임을 닫지 않는다', () => {
		expect(parser.markdownToHtml('_기울임 _').trim())
			.toBe('<p>_기울임 _</p>');
	});

	it('닫는 겹밑줄 앞이 공백이면 굵게를 닫지 않는다', () => {
		expect(parser.markdownToHtml('__굵게 __').trim())
			.toBe('<p>__굵게 __</p>');
	});

	it('낱말에 붙어 열린 밑줄도 닫는 쪽 앞이 공백이면 닫지 않는다', () => {
		expect(parser.markdownToHtml('와_ 좋다 정말 대박_').trim())
			.toBe('<p>와_ 좋다 정말 대박_</p>');
	});
});

// (I11) 다섯 구분자(`*` · `**` · `_` · `__` · `~~`)의 공백 판정 대등성을 **산출**로 잰다.
// 종전 유일 채널은 등록 형태의 **개수**(arity)를 세므로 4번째 인자의 **값**이
// 뒤집혀도 형태는 그대로여서 무감하다 — 깨끗할 때도 주입해도 원복해도 rc=0 이다.
// 여기서는 다섯 구분자에 **같은 문장 틀**을 넣고 다섯 산출이 전부 글자 그대로
// 남는 것을 단언한다. 틀을 바꾸면 대등성이 아니라 다섯 개의 다른 명제가 된다.
describe('다섯 구분자의 공백 판정은 산출로 대등하다', () => {

	it('별표 하나는 공백을 사이에 두면 강조가 되지 않는다', () => {
		expect(parser.markdownToHtml('값은 a * b * c 이다').trim())
			.toBe('<p>값은 a * b * c 이다</p>');
	});

	it('겹별표는 공백을 사이에 두면 강조가 되지 않는다', () => {
		expect(parser.markdownToHtml('값은 a ** b ** c 이다').trim())
			.toBe('<p>값은 a ** b ** c 이다</p>');
	});

	it('밑줄 하나는 공백을 사이에 두면 강조가 되지 않는다', () => {
		expect(parser.markdownToHtml('값은 a _ b _ c 이다').trim())
			.toBe('<p>값은 a _ b _ c 이다</p>');
	});

	it('겹밑줄은 공백을 사이에 두면 강조가 되지 않는다', () => {
		expect(parser.markdownToHtml('값은 a __ b __ c 이다').trim())
			.toBe('<p>값은 a __ b __ c 이다</p>');
	});
