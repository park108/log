...
describe('useUpdateLog', () => {
	...
	it('resolves on status 200 and invalidates both [log, list] and [log, detail, timestamp]', async () => {
		vi.mocked(api.putLog).mockResolvedValueOnce({
	...
	it('surfaces network-level rejection as mutation error', async () => {
		vi.mocked(api.putLog).mockRejectedValueOnce(new Error('network'));
	...
	it('구독자가 없는 창에서 응답이 도착해도 훅 옵션 onSuccess 가 발화한다', async () => {
		...
		act(() => {
			result.current.mutate({ newItem: { timestamp: 21 } as unknown as LogItemPayload, isTemporary: false, timestamp: 21 });
		});
		unmount();
	...
	it('구독자가 없는 창에서 비-200 이 도착해도 훅 옵션 onError 가 발화한다', async () => {
		...
		act(() => {
			result.current.mutate({ newItem: { timestamp: 22 } as unknown as LogItemPayload, isTemporary: false, timestamp: 22 });
		});
		unmount();
