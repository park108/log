...
export interface UpdateLogVars {
	newItem: LogItemPayload;
	isTemporary: boolean;
	timestamp: number;
}
 ...
 * 아직 커밋되지 않은 창(window) 안에 수정 요청이 완료되면 성공 알림이 조용히 유실된다
 * — 글은 저장됐는데 폼은 `isProcessing` 으로 멈춘 상태가 되고, 사용자는 재시도해 중복 저장을 부른다.
 * 훅 옵션 콜백은 `Mutation.execute()` 가 구독 여부와 무관하게 호출하므로 그 창에서도 발화한다
 * (REQ-20260825-014 (M-1)(M-2)(M-3)).
 *
 * @param {{
 *   onSuccess?: (data: unknown, variables: { newItem: object, isTemporary: boolean, timestamp: number }) => void,
 *   onError?: (error: Error, variables: { newItem: object, isTemporary: boolean, timestamp: number }) => void,
 * }} [callbacks]
 ...
 * >}
 */
export const useUpdateLog = (callbacks: MutationCallbacks<UpdateLogVars> = {}) => {
	const queryClient = useQueryClient();
	return useMutation({
		mutationFn: async ({ newItem, isTemporary }: UpdateLogVars) => {
