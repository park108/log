...
- [ ] (Must, G-2) suffix 표기 0 — `bash -c 'test "$(grep -rnE --include="*.md" "specs/30\.spec/[^\"\` ]*-spec\.md" specs/30.spec | wc -l)" -eq 0'` → **rc=0**. **HEAD=`139cd78` (tick 223 재실행) 실측 11 line → 미충족.** 11 line 은 **전량 blue** 이며 green 스코프는 **0** 이다 (채널 출력 `green(… suffix=0) blue(… suffix=11 · ADVISORY)`). G-1 과 동일한 writer 부재 제약을 받는다.
