...
3. **(G-3) 인용 설정값의 일치** — spec 본문이 인용하는 설정 토큰은 설정 파일의 현재 값과 일치한다.
   - **HEAD=`ff699f9` (tick 222 재실행 — tick 221 `6f58541` 과 동일) 실측: 위반 2 line.** `vite.config.js` 의 `coverage.provider` 현재 값은 `istanbul` 인데, blue 2 파일이 종전 provider 토큰을 현재 값으로 인용한다 — `blue/foundation/regression-gate.md:91` · `blue/foundation/vitest-coverage-exclude-pattern-vacuous-zero-axis.md:10`. 실행되지 않는 측정 경로를 전제로 인과 모형과 baseline 수치를 서술한다.
...
- [ ] (G-1) 참조 실재 — HEAD=`ff699f9` (tick 222 재실행 — tick 221 `6f58541` 과 동일) 부재 **14 distinct, 전량 blue**. green 스코프는 **0**. 미충족 (blue 편집 writer 부재 — §미측정·비판정 항목).
- [ ] (G-2) suffix 표기 0 — HEAD=`ff699f9` (tick 222 재실행 — tick 221 `6f58541` 과 동일) **11 line, 전량 blue**. green 스코프는 **0**. 미충족 (동일 사유).
- [ ] (G-3) 인용 설정값 일치 — HEAD=`ff699f9` (tick 222 재실행 — tick 221 `6f58541` 과 동일) 종전 provider 토큰 인용 **2 line (전량 blue)** vs 실제 `istanbul`. 미충족. tick 220 판본의 4 hit 중 2 건은 본 spec 자신의 문장이었고 tick 221 이 제거했다.
...
> 전 항목 명령 1회로 rc 판정 가능 (RULE-07 §수용 기준 문장 규약). **HEAD=`ff699f9` (tick 222 재실행 — tick 221 `6f58541` 과 동일) 기준 2/5** — inspector tick 221 이 다섯 명령을 전수 재실행했다. 측정 명령은 `specs/30.spec` 트리와 `vite.config.js`·`package.json` 만 참조하며 **본 spec 자신의 파일 경로를 참조하지 않는다** (RULE-07 §promote 조건 2).
>
...
- [ ] (Must, G-1) 참조 실재 — `bash -c 'n=0; for p in $(grep -rhoE --include="*.md" "specs/30\.spec/(blue|green)/[A-Za-z0-9._/-]+\.md" specs/30.spec | sort -u); do [ -e "$p" ] || { echo "$p"; n=$((n+1)); }; done; exit $((n>0))'` → **rc=0**. **HEAD=`ff699f9` (tick 222 재실행 — tick 221 `6f58541` 과 동일) 실측 rc=1 / 부재 14 distinct → 미충족.** 부재 14 는 **전량 blue** 이며 green 스코프 부재는 **0** 이다 (등재 채널 출력 `green(missing=0 …) blue(missing=14 · ADVISORY)` 와 일치). blue 에는 create/edit writer 가 없으므로 이 항목의 잔여는 파이프라인 안에서 닫히지 않는다 — 시정 경로는 운영자 편집 또는 `RULE-01` 개정이다. 그 사실을 감추지 않기 위해 항목을 `[x]` 로 완화하거나 green 한정으로 축소하지 않는다.
- [x] (Must, G-5) 비공허 — 등재 채널이 **하한 단언을 보유하고 위반 시 하드 실패**한다: `bash -c 'grep -qE "SPEC_MIN_DISTINCT=[0-9]+" scripts/check-spec-coherence.sh && npm run check:spec-coherence 2>&1 | grep -qE "distinct=[0-9]+"'` → rc=0, 그리고 순회 distinct 수 ≥ **50**: `bash -c 'test "$(grep -rhoE --include="*.md" "specs/30\.spec/(blue|green)/[A-Za-z0-9._/-]+\.md" specs/30.spec | sort -u | wc -l)" -ge 50'` → rc=0. **HEAD=`ff699f9` (tick 222 재실행 — tick 221 `6f58541` 과 동일) 실측 채널 `SPEC_MIN_DISTINCT=50` (`:59`) + `:191` `G5 VIOLATION (vacuous)` 하드 실패 + 출력 `distinct=55`, 독립 재측정 **55** → 충족.** tick 219 가 `[ ]` 로 둔 이유("채널이 단언을 보유하지 않는다")가 해소됐다 — 충족 형태는 값의 크기가 아니라 **채널의 보유**이므로 판정 명령도 그 형태로 교정했다.
- [ ] (Must, G-2) suffix 표기 0 — `bash -c 'test "$(grep -rnE --include="*.md" "specs/30\.spec/[^\"\` ]*-spec\.md" specs/30.spec | wc -l)" -eq 0'` → **rc=0**. **HEAD=`ff699f9` (tick 222 재실행 — tick 221 `6f58541` 과 동일) 실측 11 line → 미충족.** 11 line 은 **전량 blue** 이며 green 스코프는 **0** 이다 (채널 출력 `green(… suffix=0) blue(… suffix=11 · ADVISORY)`). G-1 과 동일한 writer 부재 제약을 받는다.
- [ ] (Must, G-3) 인용 설정값 일치 — `bash -c 'v=$(sed -n "s/.*provider:[[:space:]]*.\([a-z0-9]*\).*/\1/p" vite.config.js | head -1); tok="v""8"; c=$(grep -rn --include="*.md" "provider: .$tok." specs/30.spec | wc -l); [ "$v" = "$tok" ] || [ "$c" -eq 0 ]'` → **rc=0**. **HEAD=`ff699f9` (tick 222 재실행 — tick 221 `6f58541` 과 동일) 실측 `v=istanbul` · `c=2` → rc=1 미충족.** 잔존 2 line 은 **전량 blue** 다. **tick 221 교정**: 종전 명령은 판정 토큰의 리터럴을 명령 문장에 담고 있어 본 파일이 스스로 2 hit 을 만들었다 (`c=4` 중 2). 토큰을 `"v""8"` 로 조립해 자기참조를 끊었고, 이로써 이 항목은 blue 시정만으로 0 에 도달할 수 있게 됐다 — 교정 전에는 어떤 시정으로도 도달 불가한 자기봉쇄 기준이었다. 확장자 제한도 함께 붙였다 (seen 파일 오염 차단, §수용 기준 교정 1).
- [x] (Must, G-4) 채널 스코프 포함 — 등재 채널을 실행했을 때 **`specs/30.spec` 트리를 실제로 판정**한다. 판정: `bash -c 'npm run check:spec-coherence >/tmp/sc.out 2>&1 || exit 1; grep -qE "spec-scope root=specs/30\.spec .*distinct=([5-9][0-9]|[1-9][0-9]{2,})" /tmp/sc.out'` → rc=0. **HEAD=`ff699f9` (tick 222 재실행 — tick 221 `6f58541` 과 동일) 실측 rc=0 → 충족** — 출력 `check-spec-coherence: spec-scope root=specs/30.spec files=65 distinct=55 green(missing=0 suffix=0) blue(missing=14 suffix=11 · ADVISORY)`. TSK-20260825-08 (`f2bd538`) 이 `SPEC_ROOT` 스코프를 신설했다. **스크립트 본문 grep 으로 판정하지 않는다** — `grep -c "specs/30.spec" scripts/check-spec-coherence.sh` 는 tick 219 시점에 이미 **4** 였고 전부 주석·진단 문구라 아무것도 고치지 않은 상태에서 통과하는 공허 기준이었다 (§동작 4). 판정은 **관측된 산출**(스캔 루트 리터럴 + distinct 하한)으로만 한다.
