...
> **최종 업데이트**: 2026-08-25 (by inspector — tick 221 Phase 1 reconcile @ HEAD=`6f58541`. TSK-20260825-08 착지 반영: 수용 기준 0/5 → **2/5** (G-4·G-5). G-1·G-2 는 잔존 위반이 **전량 blue** 이고 blue 에 편집 writer 가 없어 미충족 유지. G-1·G-5·G-2 의 판정 명령에 확장자 제한을, G-3 에 자기참조 회피를 각각 교정했다)
...
3. **(G-3) 인용 설정값의 일치** — spec 본문이 인용하는 설정 토큰은 설정 파일의 현재 값과 일치한다.
   - **HEAD=`6f58541` 실측: 위반 2 line.** `vite.config.js` 의 `coverage.provider` 현재 값은 `istanbul` 인데, blue 2 파일이 종전 provider 토큰을 현재 값으로 인용한다 — `blue/foundation/regression-gate.md:91` · `blue/foundation/vitest-coverage-exclude-pattern-vacuous-zero-axis.md:10`. 실행되지 않는 측정 경로를 전제로 인과 모형과 baseline 수치를 서술한다.
...
- [ ] (G-1) 참조 실재 — HEAD=`6f58541` 부재 **14 distinct, 전량 blue**. green 스코프는 **0**. 미충족 (blue 편집 writer 부재 — §미측정·비판정 항목).
- [ ] (G-2) suffix 표기 0 — HEAD=`6f58541` **11 line, 전량 blue**. green 스코프는 **0**. 미충족 (동일 사유).
- [ ] (G-3) 인용 설정값 일치 — HEAD=`6f58541` 종전 provider 토큰 인용 **2 line (전량 blue)** vs 실제 `istanbul`. 미충족. tick 220 판본의 4 hit 중 2 건은 본 spec 자신의 문장이었고 tick 221 이 제거했다.
...
> 전 항목 명령 1회로 rc 판정 가능 (RULE-07 §수용 기준 문장 규약). **HEAD=`6f58541` 기준 2/5** — inspector tick 221 이 다섯 명령을 전수 재실행했다. 측정 명령은 `specs/30.spec` 트리와 `vite.config.js`·`package.json` 만 참조하며 **본 spec 자신의 파일 경로를 참조하지 않는다** (RULE-07 §promote 조건 2).
>
