...
- [x] (I7 좁힘의 검출력 보존 — 적용처 실재) 좁힌 소유 항이 문면에 박제돼 있다: `bash -c 'f=specs/30.spec/green/common/markdown-pipe-table.md; test -f "$f" || exit 2; grep -qF "선언 앵커" "$f" && grep -qF "outside-owner" "$f" && grep -qF "왕복" "$f"'` → HEAD=`c82c629` 실측 **rc=0**. **정적 불변식이다.** 왕복 실측은 소유 spec 이 진다 — 격리 사본에 둘째 선언을 넣어 `outside-owner=1` **rc=1**, 원복 rc=0 (`markdown-pipe-table` §변경 이력). **본 계약은 일반 명제를 지고 적용은 소유 spec 이 진다** ((I1) 과 같은 형태).
- [ ] (I8 실행 창 분류 전수) green 판정 항목 중 **`-t` 로 창을 좁혔는데 실행 창 확인 선언이 없는 것이 0** 이다: `bash -c 'set -- $(find specs/30.spec/green -name "*.md"); test "$#" -ge 1 || exit 2; R=$(perl -ne '"'"'BEGIN{$B=chr(96)} if(/^## (테스트 현황|수용 기준)\s*$/){$k=1;next} if(/^## /){$k=0;next} if($k && /^- \[[ x]\]/){ my $line=$_; while(/$B([^$B]+)$B/g){ my $c=$1; next if $c=~/block-window-items/; next unless $c=~/npx vitest run/; next unless $c=~/ -t /; print(($line=~/실행 창 확인/) ? "C\n" : "U\n"); last } } if(eof){close ARGV}'"'"' "$@"); tot=$(printf "%s\n" "$R" | grep -c "."); test "$tot" -ge 1 || exit 2; u=$(printf "%s\n" "$R" | grep -c "^U"); echo "block-window-items=$tot unconfirmed=$u"; test "$u" -eq 0'` → HEAD=`c82c629` 실측 **rc=1**, 출력 `block-window-items=22 unconfirmed=22`. **정적 불변식이다.** **제외 규칙**: 본 항목 자신(명령에 `block-window-items` 를 담는 span — 자기참조 차단). **`unconfirmed` 는 결함 수가 아니라 미확인 수다** — 그중 몇이 실제로 창 안에 입력이 없는지는 확인이 끝나야 안다. (I4) 의 `static-undeclared` 와 같은 형태이며 정상화 경로도 같다 (§참고 §도입 경로 — inspector Phase 2).
