...
- [x] (반례 CSS 실재 baseline — 정적) 반례가 저장소에 실재한다: `bash -c 'grep -qE "^[[:space:]]*overflow-x:[[:space:]]*auto" src/styles/typography.css'` → HEAD=`af42803` 실측 **rc=0** (`typography.css:73`, `pre` 규칙 안). **이 반례가 사라지면 (I1) 의 특이도 대조가 근거를 잃는다** — 그때는 대조를 다시 고르는 것이지 계약이 닫히는 것이 아니다.
