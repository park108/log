...
- [x] typescript-eslint 도입 후 `npm run lint` 경고·오류 0 + `.d.ts` 파싱 pass (TSK-33 result).
- [x] coverage include 확장 후 `.d.ts` 가 통계에 포함되지 않음 확인 (`coverage/lcov.info` `.d.ts` 라인 0).
- [x] `.ts` staging pre-commit 훅 — typescript-eslint 파서 호출 실증 (TSK-33 result §pre-commit dry-run 로그).

## 수용 기준
- [x] (Must) `package.json` devDependencies 에 `typescript-eslint` (또는 `@typescript-eslint/parser` + `@typescript-eslint/eslint-plugin`) 추가 + `npm install` exit 0 + `package-lock.json` 갱신.
- [x] (Must) `grep -n "\*\*/\*\.d\.ts" eslint.config.js` → `ignores` 항목으로는 0 hits. 블록 구조상 그 외 참조(예: 파서 대상 패턴) 는 허용.
- [x] (Must) `npm run lint` exit 0 + `src/common/env.d.ts`, `src/types/env.d.ts` 양쪽 파싱 에러 없음 + 경고·오류 건수 변경 0.
- [x] (Must) `vite.config.js` coverage `include` = `['src/**/*.{js,jsx,ts,tsx}']`, `exclude` 도 `.{js,jsx,ts,tsx}` + `src/**/*.d.ts` 명시.
- [x] (Must) `npm test` exit 0 + 전 스위트 green + coverage 보고서 생성 + `coverage/lcov.info` 에서 `.d.ts` 라인 0.
- [x] (Must) `npm run typecheck` exit 0.
- [x] (Must) `npm run build` exit 0 + 번들 크기 회귀 ±5% 내 (146.82 kB / gzip 48.00 kB 동일 파일셋).
- [x] (Should) 임의 `.ts` 파일 staging → husky pre-commit → lint-staged → ESLint (typescript-eslint 파서) 성공. 검증 스크립트와 로그 발췌를 result.md 에 박제.
- [x] (Should) typescript-eslint `recommended` 규칙 세트 채택 여부를 planner 가 단일 값으로 고정 — spec 승격 시 본 문서 갱신 (planner 결정: "규칙 세트 미적용", `eslint.config.js:29` 주석 박제).
- [x] (NFR) `npm run lint` 실행 시간 회귀 +30% 내 (실측 ~1.23s, baseline 동일 대역).
- [x] (NFR) `npm audit` 고·치명 취약점 0 유지.
- [x] (NFR) Vite 빌드 시간 회귀 ±5% 내 (341ms, 설정만 변경).
...
| 2026-04-20 | TSK-20260420-33 / 14696b3 | typescript-eslint 파서 도입 + Vitest coverage `.ts/.tsx` 포함 — 전 Must/Should/NFR 체크박스 플립 (grep 게이트 현 HEAD 재실행 PASS: `ignores` 내 `**/*.d.ts` 0 hits, `typescript-eslint` devDep 1 hit, vite.config coverage include 확장). 368/368 green, lint/typecheck/build exit 0. | §수용 기준 (전체), §테스트 현황 |
