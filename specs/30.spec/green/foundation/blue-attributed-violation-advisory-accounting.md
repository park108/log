...
blue 트리를 판정 모집단 밖(`blue_hidden/`)으로 옮기되 `30.spec` 하위 md 총수는 86 으로 유지(비공허 하한 통과)한 상태에서 2차 모집단 게이트를 실행한 결과 (실험 수행: developer · 2026-08-28):

| 출력 항목 | 정상 | blue 은닉 |
|---|---|---|
| `files` | 86 | 86 |
| `distinct` | 56 | 56 |
| `blue(missing= / suffix=)` | 13 / 11 | 13 / 11 |
| `blue-scanned` | 81 | **0** |

**기존 위반 계수만으로는 모집단 제외가 무증상이다.** 회피가 "위반을 0 으로 만드는" 방식이 아니라 "모집단을 비우는" 방식이라 위반 계수는 그대로 남고 rc 도 `0` 이다. 움직이는 값은 `blue-scanned` 하나뿐이며, 이것이 §역할 §방어 대상 2 (ADVISORY 전환을 빌미로 blue 를 모집단에서 빼 검출력이 조용히 0 이 되는 회귀) 의 **유일한** 검출자다.

같은 실험에서 등급 분리도 확인됐다 — green 축 방향(Dir-1)은 `rc=1`, 스캔 루트 공집합(Dir-3)은 `rc=2` 로 갈린다 ((A-6)·(A-7) 이 서로 다른 등급을 낸다는 명제의 실측).

> 이 근거가 본문에 없으면 `blue-scanned` 는 "위반 계수와 중복되는 쓸데없는 출력" 으로 읽혀 다음 정리 때 삭제된다. 삭제되는 순간 (A-4) 는 "blue 를 모집단에서 뺐다" 로도 충족되고 회귀는 무증상으로 돌아간다.
