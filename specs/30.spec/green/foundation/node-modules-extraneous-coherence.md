...
- [x] (G4) CI / pre-commit / pre-push 훅 / `scripts.<name>` npm script 박제 — 회귀 방지 자동 게이트. hook-ack TSK-20260517-09 (`node-modules-extraneous-ci-gate`) `7d085a3` — `.github/workflows/ci.yml:28` `run: npm run check:deps` + `.husky/pre-push:1` `npm run check:deps || exit 1` 2 채널 박제. `scripts/check-deps-coherence.sh` POSIX shell + `package.json:26` `scripts.check:deps` 진입점. HEAD=`4b5cc1d` 재실측 PASS — `grep -nE "check:deps|check-deps" .github/workflows/ci.yml .husky/pre-push` → `.husky/pre-push:1` + `.github/workflows/ci.yml:28` 2 hit 박제.
