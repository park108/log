...
## 테스트 현황
- [ ] (G1) `npm ls --depth=0 2>&1 | grep -cE " extraneous$"` → 0 게이트 — 현재 837 hit (위반).
- [ ] (G2) `N == M` 등식 — 현재 `N=29 / M=866 / diff=837` (위반).
- [ ] (G3) 신규 dep 추가/제거 후 동일 PR 안에 G1·G2 동시 충족 — 회수 후 회귀 추적.
- [ ] (G4) CI / pre-commit / pre-push 훅 / `scripts.<name>` npm script 박제 — 회귀 방지 자동 게이트.

## 수용 기준
- [ ] (Must) `npm ls --depth=0 2>&1 | grep -cE " extraneous$"` → **0**. baseline 837 hits.
- [ ] (Must) `N == M` 등식 — `N = Object.keys({...deps,...devDeps}).length`, `M = npm ls --depth=0 2>&1 | grep -cE "^[├└]"`. baseline `N=29 / M=866 / diff=837`.
- [ ] (Must) 본 두 게이트는 신규 dep 추가·제거·메이저 bump 이벤트 후 1 PR 안에 동시 충족 (시점 비의존).
...
- [ ] (Must, 회귀 0) extraneous 0 수렴 후 `npm run build` exit=0 / `npm test` exit=0 / `npm run lint` exit=0 유지 — 회수 과정에서 의존성 의도 graph 절단 0.
- [ ] (Must, 범위 제한) transitive tree (depth>0), 메이저 버전 정합, lockfile 정합, binary 가용성, `npm audit` vulnerability 0 은 본 게이트 위반으로 카운트되지 않음 (각각 별 spec / 별 req).
...
| 일자 | TSK / 커밋 | 요약 | 영향 섹션 |
|------|-----------|------|----------|
| 2026-05-17 | inspector (Phase 2, REQ-20260517-073 흡수) / pending | 최초 박제 — `node_modules` ↔ `package.json` declared 일관성 (G1 extraneous 0 + G2 `N == M` 등식) 두 축 게이트. baseline 837 extraneous / `N=29 / M=866`. | all |
