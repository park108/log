...
- [ ] (Must, 회귀 가설 검출) `public/favicon.ico` 삭제 가설 회귀 시 FR-02 게이트 → exit 1 (참조 격차 검출).
- [ ] (Must, 회귀 가설 검출) `public/manifest.json.theme_color` 를 `"#ffffff"` 로 변경 가설 회귀 시 FR-04 두 값 추출 후 비교 → 불일치 검출.
- [ ] (Must, 회귀 가설 검출) `public/manifest.json.icons[2]` 항목 추가 (`{"src": "non-existent.png", ...}`) 가설 회귀 시 FR-05 게이트 → `test -f public/non-existent.png` 실패로 exit 1.
- [ ] (Must, 회귀 가설 검출) `index.html:12` `<link rel="apple-touch-icon">` 삭제 가설 회귀 시 FR-01 게이트 → grep count = 3 (감소 검출).
