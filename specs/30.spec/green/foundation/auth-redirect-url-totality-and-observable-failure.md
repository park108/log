...
> **현 HEAD (`dac5a60`) 갱신** — 위 표는 `139cd78` 스냅샷이다. 이후 (U-1)·(U-2) 두 축의 채널이 `check:env-api-base` (`scripts/check-env-api-base-presence.sh`) 로 **부착 완료**됐다: 도출 패턴이 `VITE_[A-Z_]+` 로 열려 선언 10 키 전수가 모집단에 들고, 산출이 판정 키를 이름으로 열거하며, 값 결손을 `missing-values` 로 집행한다. 배선 실경로는 `.husky/pre-commit:53` · `.github/workflows/ci.yml:78` (실측 2026-08-28). (U-3) 은 `TSK-20260825-29` (`7465638`) 로 닫혔다.
...
- [x] `VITE_*` 선언 키의 실재 검증 채널 — **tick 226 ack (TSK-20260826-37 / `5857431`)** `declared=10 uncovered=0`. tick 225 실측은 `declared=10 uncovered=10` (0건 커버) 이었다. TSK-20260825-31 의 seam 도입이 이 축을 움직이지 못한 것은 slug `foundation/gate-judgement-population-injectable-seam` §동작 (P-4) 예고대로였고, 실제로 닫은 것은 **판정 키 열거를 산출에 싣는** TSK-20260826-37 이었다.

## 수용 기준

> 전 항목 명령 1회로 rc 판정 가능 (RULE-07 §수용 기준 문장 규약). 측정 명령은 `src/**` · `scripts/**` · `.env*` · `package.json` · `.husky/*` · `.github/**` 만 참조한다 (spec 자신의 green/blue 경로 미참조 — RULE-07 §promote 조건 2). **HEAD=`dac5a60` (2026-08-28 재실행) 기준 전수 충족** — (U-3)(U-3 보조) 는 TSK-20260825-29 (`7465638`), (U-1) 은 TSK-20260826-37 (`5857431`) 로 닫혔고, 마지막 미판정이던 (U-2) 값 층은 판정 채널을 `.env.test` 직독에서 **게이트 경유**로 교체해 판정됐다. tick 224·226 의 미판정 사유(판정 주체의 `.env*` 권한 경계)는 그 교체로 해소되며 우회가 아니다 — 게이트가 읽고 rc 로 답한다.

- [x] (Must, U-1) 선언 전수 대상성 — `env.d.ts` 선언 `VITE_*` 키가 전부 검증 채널 **산출에 판정 대상으로 열거**된다. 판정:
  ```
...
- [x] (Must, U-2 값 층) 값 실재 — 선언된 `VITE_COGNITO_*` 키 전수가 **값 층 게이트의 판정 대상**이며 결손이 0 이다. 판정은 게이트를 경유한다:

  ```
  bash -c 'd=$(grep -cE "readonly[[:space:]]+VITE_COGNITO_[A-Z_]+" src/types/env.d.ts); [ "$d" -gt 0 ] || { echo "derive=0 vacuous" >&2; exit 2; }; out=$(npm run --silent check:env-api-base 2>&1) || { echo "gate rc!=0" >&2; exit 1; }; printf "%s" "$out" | grep -qE "missing-values=0" || { echo "missing-values!=0" >&2; exit 1; }; n=0; for k in $(grep -oE "readonly[[:space:]]+VITE_COGNITO_[A-Z_]+" src/types/env.d.ts | sed -E "s/.*(VITE_COGNITO_[A-Z_]+)/\1/" | sort -u); do case "$out" in *"$k"*) : ;; *) echo "unjudged: $k" >&2; n=$((n+1));; esac; done; echo "cognito-declared=$d unjudged=$n"; [ "$n" = 0 ]'
  ```

  → rc=0. **실측 2026-08-28 (HEAD `dac5a60`): `cognito-declared=4 unjudged=0` rc=0 → 충족.**

  **판정 채널을 `.env.test` 직독에서 게이트 경유로 교체했다.** 구 명령(`grep -cE "^VITE_COGNITO_[A-Z_]+=.+" .env.test`)은 판정 주체에게 `.env*` 읽기 권한을 요구했고, 그 때문에 tick 224·226 이 연속으로 **미판정**이었다. `RULE-07 §promote 조건 2` 는 promote 직전 **실제 실행**을 요구하므로, 판정 주체가 실행할 수 없는 명령은 이 항목을 영구 미판정으로 고정한다. 교체 후 판정 주체는 `.env*` 를 읽지 않는다 — 게이트가 읽고 rc·산출로 답한다.

  **"위반 시 실패하는 기존 게이트가 없다"는 구 사유는 현 HEAD 에서 성립하지 않는다.** `TSK-20260825-37` 이 `check-env-api-base-presence.sh` 의 도출 패턴을 `VITE_[A-Z_]*API_BASE` 에서 `VITE_[A-Z_]+` 로 열어 선언 키 전수가 모집단에 들어왔고(구 `:37` 의 자백 주석 `~~미선언 방향: VITE_COGNITO_* 4 키의 실재~~` 는 **해소됨**으로 정정돼 있다), 산출이 판정 키를 이름으로 열거하게 되면서 (U-1) 이 tick 226 에 `declared=10 uncovered=0` 으로 플립됐다. 값 층도 같은 게이트의 `missing-values` 가 집행한다 — 실측 `keys=10 envfiles=2 valued-files=1 missing-rows=0 missing-values=0`.

  **공허·이완 가드 3중** — (a) 도출 0 이면 `exit 2` (무판정), (b) 게이트 rc≠0 이면 `exit 1`, (c) 게이트가 다시 좁혀져 COGNITO 키가 산출에서 빠지면 `unjudged>0` 으로 `rc≠0`. (c) 가 없으면 게이트 모집단 축소가 이 항목을 조용히 통과시킨다. 음성 대조 실측: 산출에 없는 키 `VITE_COGNITO___PROBE_ABSENT` 주입 시 `unjudged=1`, `missing-values=2` 문자열에 대해 `grep -qE "missing-values=0"` 는 무매치.

  참고 — 구 명령도 본 tick 세션에서 실행돼 rc=0 (`d=4`, `.env.test` 일치 행 4) 이었다. 다만 그 경로는 판정 주체 의존적이므로 계약의 판정 채널로 두지 않는다.
...
| 2026-08-26 | TSK-20260826-37 / `5857431` (inspector tick 226) | **Phase 1 reconcile ack 1/1 (측정 가능분).** (U-1) 플립 — `check:env-api-base` 산출이 `env.d.ts` 선언 `VITE_*` **10 키를 전수 열거**하게 되어 `declared=10 uncovered=0` rc=0. tick 225 는 `uncovered=10` 이었다. (U-3)(U-3 보조)(U-4) 는 재실행 rc=0 유지. **(U-2) 는 본 tick 미측정** — 판정 명령이 `.env.test` 를 읽는데 tick 226 세션은 `.env*` 접근이 권한 거부다. 미측정을 충족으로 읽지 않기 위해 마커를 `[ ]` 로 **유지**한다 (`RULE-07 §promote 조건 2` 는 promote 직전 **실제 실행**을 요구하므로, 이 항목은 `.env*` 를 읽을 수 있는 주체가 실행해야 판정된다). 이로써 본 spec 은 수용 기준 4/5 이며 promote 후보가 아니다. | 테스트 현황 · 수용 기준 (U-1) |
| 2026-08-28 | inspector Phase 1 (HEAD `dac5a60`) | **(U-2) 값 층 플립 — 수용 기준 4/5 → 5/5.** 판정 채널을 `.env.test` 직독에서 `check:env-api-base` 경유로 교체: 판정 주체가 `.env*` 를 읽지 않으므로 tick 224·226 을 연속 미판정으로 고정하던 권한 경계가 해소된다 (`RULE-07 §promote 조건 2` 는 promote 직전 실제 실행을 요구하며, 실행 불가 명령은 항목을 영구 미판정으로 못박는다). 구 사유 "위반 시 실패하는 기존 게이트 부재" 는 `TSK-20260825-37` 의 도출 패턴 확장(`VITE_[A-Z_]*API_BASE` → `VITE_[A-Z_]+`)으로 성립하지 않게 됐다 — 실측 `cognito-declared=4 unjudged=0` rc=0, 게이트 산출 `keys=10 missing-rows=0 missing-values=0`. 이완 가드 3중(도출 0 → `exit 2` / 게이트 rc≠0 → `exit 1` / 모집단 축소 시 `unjudged>0`) + 음성 대조 2건 확인. 구 명령도 본 세션에서 rc=0 이었으나 판정 주체 의존적이라 계약 채널로 두지 않는다. §발화 채널 표에 현 HEAD 부착 상태 주석 추가 | 수용 기준 (U-2), 동작 §발화 채널, 변경 이력 |
