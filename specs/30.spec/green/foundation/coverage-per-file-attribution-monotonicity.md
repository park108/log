...
> **최종 업데이트**: 2026-08-25 (by inspector — Phase 1 tick 218. G-C 채널 등재·발화를 재측정해 플립. 등재 채널의 공허 통과 배제가 집계 수준에서 미완임을 실측해 G-D 신설)
...
5. (R-5) 채널 자신의 공허화 — 측정 대상 집합이 비면 단조성은 자명하게 성립하고 게이트는 초록을 낸다. G-D 가 이 축을 겨눈다. coverage `exclude` 축은 자매 게이트가 지키지만 `include` 축은 현 HEAD 어떤 게이트도 단언하지 않는다.
...
- [ ] G-D 공허 입력 불통과 — **미충족**. 등재 채널이 전수 공허 입력을 `allow-empty` 면제로 통과시킨다 (실측 rc=0, 기대 rc≠0).

## 수용 기준

> 전 항목 HEAD=`9174c92` 에서 명령 1회로 rc 판정 가능 (RULE-07 §수용 기준 문장 규약). **2/3 rc=0** — G-A·G-C 충족, **G-D 미충족**. 구 G-B 는 '가정 주입 요구' 부류라 §참고 §미측정·비판정 항목으로 강등했고 이관처를 표기했다 (아래). G-D 는 커밋된 픽스처만 쓰므로 그 부류가 아니다 — 트리를 파손하지 않고 현 HEAD 에서 rc 를 낸다.
...
- [ ] (Must) G-D: 등재 채널이 **전수 공허 입력**을 통과로 읽지 않는다. `bash scripts/check-coverage-attribution-monotonicity.sh --full scripts/fixtures/coverage-attribution/subset-empty.json --subset scripts/fixtures/coverage-attribution/subset-empty.json --allow-empty` → **rc≠0 기대 / HEAD=`9174c92` 실측 rc=0 → 미충족.** (M1) 이 샤드마다 `allow-empty` 를 무조건 넘기므로 (`scripts/check-coverage-attribution-monotonicity.sh:385`) 이 면제가 전수 공허까지 덮는다 — 측정 대상 집합이 통째로 비면 전 샤드가 `[NOTE]` 를 내며 게이트가 초록을 유지한다. 충족 형태: `full_covered == 0` 이면 `allow-empty` 와 무관하게 rc≠0.
...
- **(G-D 실측 근거 — 체크박스는 §수용 기준에 있다) 전수 공허 입력의 통과.** inspector 가 tick 218 에서 커밋된 픽스처만으로 판정 코어를 왕복했다. 대조 4 방향은 설계대로 동작한다 — 정상 `rc=0` / v8 회귀 4 슬롯 `rc=1` / 부분집합 공허 `rc=3` / 부분집합 공허 + `--allow-empty` `rc=0`. 추가로 확인한 검출 방향 — 2 파일 확산은 두 경로를 모두 출력하고(`rc=1`), 전수에 파일 자체가 부재하면 `(전수 산출물에 파일 자체가 부재)` 주석과 함께 `rc=1`, 입력 부재는 `rc=2`. **다만 full 과 subset 이 둘 다 공허한 입력에 `--allow-empty` 를 주면 `full_covered=0 subset_covered=0` 을 출력하고 `rc=0` 이다.** (M1) 은 `root` 샤드 예외를 위해 이 인자를 전 샤드에 무조건 넘기므로, 측정 대상 집합이 비는 손상은 전 샤드 `[NOTE]` 로 흡수된다. 방어 깊이가 일부 존재하기는 한다 — 전수 실행이 threshold 미달로 rc≠0 이면 게이트는 그 앞 단계에서 exit 2 로 끊고, coverage `exclude` 축은 자매 게이트 `src/__tests__/build-policy-and-vitest-config-coherence.test.ts` §"coverage exclude 패턴 vacuous-zero 부재" 가 지킨다. 남은 진입로는 `include` 축(`vite.config.js:83`)이며 현 HEAD 의 어떤 게이트도 이 키를 단언하지 않는다.
