...
- [x] (Must, REQ-057 FR-11) `grep -nE "^\s*'src/[a-zA-Z_][a-zA-Z0-9_]*\.(js|jsx|ts|tsx)',?$" vite.config.js` → 0 hits (src root 단일 확장자 literal 부재). — TSK-20260422-18 / `4af36ee` 수렴 ack (re-gate @HEAD=4af36ee 실측 0 hit, task result.md "baseline 2 hit → 0 hit, FR-11 gate 수렴" + 총 엔트리 수 5 유지 박제).
...
- [x] (Must, REQ-057 FR-11 → green FR-11) 본 spec §불변식 FR-11 에 "exclude src-root 엔트리는 `src/<name>.{js,jsx,ts,tsx}` extension-glob 형식" 평서문 박제 완료 (본 세션). — inspector / `4708eb8` (직전 세션) 에서 §불변식 FR-11 본문 박제 + 본 세션 구현 수렴 ack.
- [x] (Should, REQ-057 FR-02 → green FR-11) FR-11 수렴 후 `vite.config.js` exclude 배열 총 엔트리 수 5 유지 (2 root-file glob + `*mock.{…}` + `*.test.{…}` + `*.d.ts`). — TSK-20260422-18 / `4af36ee` 수렴 ack (task result.md `grep -cE "^\s*'src/" vite.config.js` → 5 박제, 재현 실측 5).
