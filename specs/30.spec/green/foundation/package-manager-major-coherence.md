...
> **최종 업데이트**: 2026-05-17 (by inspector — 최초 박제, REQ-090 흡수 / REQ-079 별 axis (패키지 매니저 메이저) 정식 후속 박제).
...
## 테스트 현황
- [ ] (I1) `package.json` 메이저 정합 채널 1+ 박제 — 현 baseline `engines.npm` 부재 + `packageManager` 부재 (§스코프 규칙 G1 MISS — 본 spec 회복 대상 zero-point). 회복 task 발행 + 수행 후 marker 플립.
- [ ] (I2) CI 채널 메이저 격차 0 박제 — 현 baseline `setup-node` step 의 npm 메이저 추종 채널 0 (`with.npm-version` 미사용 + `node-version-file` + `engines.npm` 자동 추종 비활성 + corepack 활성화 비박제 — 3 채널 0/3 hit). 회복 task 발행 + 수행 후 marker 플립.
- [ ] (I3) `lockfileVersion` 메이저 포맷 격차 0 박제 — 현 baseline `lockfileVersion: 2` (npm 7~8 메이저 포맷, 현 `setup-node@v6` + Node 24 LTS bundled npm 메이저는 활성 메이저 호환 포맷 격차 1+ — §스코프 규칙 G3 MISS). 회복 task 발행 + 수행 후 marker 플립.
- [ ] (I4) 4축 격차 0 동시 박제 — (I1)(I2)(I3) 회복 동반 marker 플립. 4축 동시 격차 0 검증 후.
- [ ] (I5) 위반 검출 단일성 — 단일 진단 명령 + grep 라벨 박제 후 marker 플립. `scripts/check-package-manager-coherence.sh` (또는 동등 1-line node 스크립트) 추가 + `npm run check:npm-coherence` (또는 동등 prefix) npm script 박제 후.
...
## 수용 기준
- [ ] (Must, FR-01) `package.json` 메이저 정합 채널 1+ 박제 — `node -e "const p=require('./package.json'); process.exit((p.engines && p.engines.npm) || p.packageManager ? 0 : 1)"` → exit 0. 현 baseline exit ≠ 0 (§스코프 규칙 G1 MISS — 본 spec 회복 대상). 별 task 발행 + 수행 후 marker 플립.
- [ ] (Must, FR-02) CI 채널 메이저 격차 0 박제 — `actions/setup-node` step 의 npm 메이저 채널 1+ 박제 후 CI step `npm -v` 출력 메이저가 FR-01 채널 메이저와 동치. 현 baseline 추종 채널 0 (§스코프 규칙 G2 MISS). 별 task 발행 + 수행 후 marker 플립.
- [ ] (Must, FR-03) `lockfileVersion` 메이저 포맷 격차 0 박제 — `grep -nE "lockfileVersion" package-lock.json` → 1 hit 값이 FR-01 채널 활성 메이저의 활성 메이저 호환 포맷과 격차 0. 현 baseline 격차 1+ (§스코프 규칙 G3 MISS — 본 spec 회복 대상). lockfile rewrite (수동 또는 자동화) 회수 후 marker 플립.
- [ ] (Must, FR-04) 위반 검출 단일성 — 단일 진단 명령 (1 명령 또는 1-line node 스크립트 또는 `scripts/check-*.sh`) stdout 출력에서 격차 카테고리 라벨 (`engines.npm 부재` / `packageManager 부재` / `lockfileVersion 격차 N` / `npm major 격차 N`) grep 가능 + exit code ≠ 0 (격차 발생 시). 회수 후 marker 플립.
