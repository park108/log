...
> **최종 업데이트**: 2026-08-25 (by inspector — tick 225 (B-1) 임계 전항 요구 상향; REQ-20260825-025 FR-05 선행 개정)
...
> 전 항목 명령 1회로 rc 판정 가능 (RULE-07 §수용 기준 문장 규약). 측정 명령은 `src/**` 만 참조한다 — **spec 파일 경로는 어느 것도 참조하지 않는다** (RULE-07 §promote 조건 2). **HEAD=`5b2ed3b` (tick 225 재실행) 기준 3/3 — (B-1) 을 전항 요구로 상향한 뒤에도 전수 충족.**

- [x] (Must, B-1) `Navigation` 음성 케이스의 구별 단언 — `ADMIN_MENU` 항목명을 **`Navigation.tsx` 에서 도출**해, 도출된 **전 항목**에 대한 **부재 단언**이 음성 케이스에 실재한다 (`asserted == adminmenu`). 판정:
  ```
  bash -c 'names=$(sed -nE "s/.*name:[[:space:]]*\"([A-Za-z0-9_-]+)\".*/\1/p" src/common/Navigation.tsx | sort -u); [ -n "$names" ] || { echo "derive=0 vacuous" >&2; exit 2; }; tot=$(echo $names | wc -w | tr -d " "); n=0; miss=""; for m in $names; do if grep -qE "query(ByText|AllByText)\([^)]*$m[^)]*\)[^;]*(toBeNull|not\.toBeInTheDocument|toHaveLength\(0\))" src/common/Navigation.test.tsx; then n=$((n+1)); else miss="$miss $m"; fi; done; echo "adminmenu=$tot asserted=$n"; [ -n "$miss" ] && echo "missing:$miss" >&2; test "$n" -eq "$tot"'
  ```
  → **rc=0**. **HEAD=`eb5019b` (tick 224 재실행) 실측 rc=0 / `adminmenu=3 asserted=3` → 충족** (TSK-20260825-30 / `eb5019b`). tick 223 실측은 rc=1 / `adminmenu=3 asserted=0` 이었다. 도출된 3 항목 **전부**가 부재 단언을 얻었다. **tick 225 개정**: 종전 기준은 `≥1` 이라 착지가 요구를 초과하는 상태였고, 그 여유가 곧 **부분 퇴행 사각**이었다 — 3 항목 중 2 개를 지워도 `asserted=1` 로 rc=0 이다 (seam 주입 실측 `adminmenu=4 asserted=1` rc=0). 기준을 전항 요구 `asserted == adminmenu` 로 상향했고 현 HEAD 는 상향 후에도 rc=0 이다 (tick 225 재실행 `adminmenu=3 asserted=3`). 미충족 항목명을 stderr 에 열거하도록 함께 보강했다 — 수치만으로는 어느 항목이 빠졌는지 관측되지 않는다. **원안(`grep -nE "queryBy|not\.toBeInTheDocument|toBeNull" … → ≥1 hit`)을 쓰지 않은 이유** — 현 HEAD 가 이미 1 hit 이다 (`:52` `expect(link).not.toBeNull()`). 그 단언은 두 갈래 모두에서 참이라 구별력이 0 이므로, 원안은 **아무것도 고치지 않은 상태에서 통과하는 공허 기준**이었다 (실측 확인). **공허 통과 가드 내장** — 항목명 도출이 0 이면 `exit 2`. ack 라인이 `adminmenu=3` 으로 비공허임을 수치로 낸다. **검출 경계 (과신 금지)**: 이 명령은 단언의 *실재*를 재며 그 단언이 실제로 뒤집기에 붉어지는지는 재지 않는다 — 그것은 (Dir-1) 로 이관돼 있다.
...
| 2026-08-25 | REQ-20260825-025 FR-05 (inspector tick 225) | **blue → green carry-over 후 (B-1) 임계를 `≥1` 에서 전항 요구 `asserted == adminmenu` 로 상향.** 개정 이유는 이 spec 의 결함이 아니라 **여유의 대가**다 — `≥1` 기준은 도출 3 항목 중 2 개를 지워도 rc=0 이라 **부분 퇴행에 침묵**하며, 그 축은 §동작이 이미 밝힌 대로 테스트가 구조적으로 채널이 될 수 없어(위반이 스위트를 붉게 만들지 않고 초록으로 만든다) 이 게이트가 **유일한 잠재 채널**이다. 실측 근거: seam 주입 `adminmenu=4 asserted=1` rc=0 (TSK-20260825-33) · 구별 단언 3행 전량 삭제 상태에서 전체 스위트가 `74 files / 701 passed` · `Branches 95.69%` 로 준수 트리와 **완전 동일**. **개정이 게이트 상향에 선행한다** — `scripts/check-declared-branch-discrimination.sh:99` 는 현재 `-lt 1` 이며, 게이트가 소유 spec 보다 엄격해지는 중간 상태를 만들지 않기 위해 spec 을 먼저 올린다 (슬러그 `testing/derived-population-totality-judgement` §동작 (D-4)). 상향 후에도 현 HEAD 는 충족이다 — tick 225 가 개정된 판정문을 **spec 본문에서 그대로 추출해 실행**해 `adminmenu=3 asserted=3` rc=0 을 확인했다 (문면과 실행 가능성의 괴리 방지). **미충족 항목명 stderr 열거를 판정문에 추가** (REQ FR-06) — 수치만으로는 어느 항목이 빠졌는지 관측되지 않는다. 다른 절·다른 수용 기준 항목은 수정 0. | §최종 업데이트, §수용 기준 (B-1), 본 이력 |
