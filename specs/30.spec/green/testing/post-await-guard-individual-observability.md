...
- **catch 경계의 분해능 계수** — §동작 2 는 catch 진입 경계를 (P-B) 에 **계수하도록 요구하며 그 요구는 유효하다**. 다만 §동작 5 의 출력 계약은 합산치(요구 분해능 합)만 선언하므로 catch 분 기여를 **분리 관측할 채널이 없다** — 현 HEAD 에서 명령 1회로 rc 판정 불가하여 체크박스에서 강등한다 (실측 2026-08-26: 판정 표면의 `catch` 언급 0건, `src/File/FileItem.tsx` 의 catch 경계 1건). 검출 방향은 아래 §게이트 실효 검증 이관 절이 보존한다 — 그 절의 방향 수는 (P-B) 와 같고 (P-B) 는 catch 경계를 포함하므로, catch 가드 제거 방향이 주입 대상에 포함된다.
