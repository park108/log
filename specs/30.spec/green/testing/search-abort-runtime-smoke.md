...
- [ ] (I4) timer leak 0건: `npx vitest run src/Search/Search.test.tsx` 실행 후 `act()` 경고 0 hit + console.error 0 hit 확인. 별 task / 별 회귀 fixture 위임 (CI 측정).
...
- [ ] (Must, FR-06-b) 컴포넌트 언마운트 시 timer leak 0건 (vitest `vi.useFakeTimers` + `vi.runOnlyPendingTimers` 검증) — §동작 (I4). CI 회귀 측정 위임.
