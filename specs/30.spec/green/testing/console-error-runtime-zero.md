...
## 테스트 현황
- [ ] (I1) prop-types runtime warning 0 hit — 현 baseline (M-A) `npm test 2>&1 | grep -cE "Warning: Failed prop type"` 측정 환경 부재 (silence 패턴이 console.error 흐름 차단 — §스코프 규칙 G1 PASS 평가 불가 zero-point). 회복 task 발행 + silence 분리 + fail-fast assertion 보강 또는 e2e/CI grep 채널 박제 후 marker 플립.
- [ ] (I2) React 일반 runtime warning 0 hit — 동일 zero-point (silence 차단). 회복 task 발행 + 채널 박제 후 marker 플립.
- [ ] (I3) silence 채널 분리 + fail-fast 보강 — 현 baseline silence 패턴 4 hit (§스코프 규칙 G2 박제) — fail-fast assertion 또는 별 채널 박제 0 hit. 회복 task 발행 + 1+ 채널 박제 후 marker 플립.
- [ ] (I4) 위반 검출 단일성 — 단일 진단 명령 + grep 라벨 박제 후 marker 플립. (M-A) `npm test 2>&1 | grep` 또는 동등 e2e capture 카테고리 라벨 발화 채널 박제 후.
...
## 수용 기준
- [ ] (Must, FR-01) prop-types runtime warning 0 hit — TS island Component mount + 정상 인터랙션 시 (M-A) 또는 (M-B) 또는 (M-C) 1+ 채널에서 `Warning: Failed prop type` 0 hit. 회복 task 발행 + 1+ 채널 박제 후 marker 플립.
- [ ] (Must, FR-02) React 일반 runtime warning 0 hit — TS island Component mount + 정상 인터랙션 시 (M-A)/(M-B)/(M-C) 1+ 채널에서 `Warning: Each child in a list` + `Warning: Cannot update a component .* while rendering` + `Warning: React has detected a change in the order of Hooks` 0 hit. 회복 task 발행 + 채널 박제 후 marker 플립.
- [ ] (Must, FR-03) silence 채널 분리 — silence 패턴 (`vi.spyOn(console, 'error').mockImplementation(() => {})`) 자체 제거 없이 fail-fast assertion (`expect(spy).not.toHaveBeenCalledWith(stringMatching(...))`) 또는 별 채널 (e2e console capture / CI stderr grep) 1+ 추가 박제. 회복 task 발행 + 채널 박제 후 marker 플립.
- [ ] (Must, FR-04) 위반 검출 단일성 — 단일 진단 명령 stdout 출력에서 위반 카테고리 라벨 (`prop type 위반 N` / `key 누락 N` / `concurrent update N` / `hook order N`) grep 가능 + exit code ≠ 0 (위반 발생 시). 회복 task 발행 + 진단 명령 박제 후 marker 플립.
