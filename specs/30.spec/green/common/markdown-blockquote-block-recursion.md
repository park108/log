...
- [ ] (I7 인용 안 밑줄식 제목) 계약이 게이트로 실재하고 초록이다: `bash -c 'set -- $(grep -rl "인용 안에서도 밑줄식 제목은 제목이다" src 2>/dev/null); test "$#" -ge 1 || exit 2; npx vitest run "$@" -t "인용 안에서도 밑줄식 제목은 제목이다" --coverage.enabled=false >/dev/null 2>&1'` → HEAD=`5420677` 실측 **rc=2 (게이트 부재 — 무판정)**. 미착수이며 이관처 task 는 미발행이다. **`-` 축과 `=` 축을 한 이름 안에서 둘 다 단언한다** (FR-03).
- [ ] (I5·I7 접속) 밑줄식 제목이 인용 안에서 열리면서 **블록 없는 여러 줄 묶음이 깨지지 않는다**: `bash -c 'D=""; for n in "인용 안에서도 밑줄식 제목은 제목이다" "블록이 없는 여러 줄 인용은 한 덩어리다"; do h=$(grep -rl "$n" src 2>/dev/null); test -n "$h" || exit 2; D="$D $h"; done; set -- $(printf "%s\n" $D | sort -u); test "$#" -ge 1 || exit 2; npx vitest run "$@" src/common/markdownParser.test.ts --coverage.enabled=false >/dev/null 2>&1'` → HEAD=`5420677` 실측 **rc=2 (게이트 부재)**. **이 접속이 (I7) 구현의 유일한 완화 방지선이다** — 인용 내용을 줄 단위로 쪼개 블록 판정을 붙이는 가장 쉬운 구현이 (I5) 를 깨뜨린다.
...
- [ ] (Must, REQ-20260831-079 FR-02·FR-03) 위 §테스트 현황 (I7 인용 안 밑줄식 제목) 명령 → rc=0.
- [ ] (Must, REQ-20260831-079 NFR-02) 위 §테스트 현황 (I5·I7 접속) 명령 → rc=0.
