...
> **최종 업데이트**: 2026-05-17 (by inspector — 최초 박제, REQ-082 흡수)
...
## 테스트 현황
- [ ] (I1) location-mock 단일 출처 — 현 baseline: ad-hoc 흡수 3 hit / 1 file (host object setter 우회 직접 표현) + module-local 헬퍼 1 file (3 const 박제). `src/test-utils/**` 에 동등 헬퍼 export 부재. task 회수 후 marker 플립.
- [ ] (I2) mock-narrow 단일 출처 — 현 baseline: `mock.calls[<int>]!` 5 hit / 5 file. `src/test-utils/**` 에 동등 헬퍼 export 부재. task 회수 후 marker 플립.
- [ ] (I3) 단일 출처 정합 — 본 효능 도입 후 `src/test-utils/**` 외부에 동등 헬퍼 정의 0 hit 검증. task 회수 후 marker 플립.
...
## 수용 기준
- [ ] (Must, FR-01) `src/test-utils/**` 단일 모듈에서 location-mock 헬퍼 export 박제. `grep -rnE "export.*(setLocation|mockUrlLocation|restoreLocation|locationMock)" src/test-utils/` → 1+ hit. 현 baseline 0 hit. task 회수 후 marker 플립.
- [ ] (Must, FR-02) `src/test-utils/**` 단일 모듈에서 mock-narrow 헬퍼 export 박제. `grep -rnE "export.*(firstCall|nthCall|getMockCall|narrowMockCall)" src/test-utils/` → 1+ hit. 현 baseline 0 hit. task 회수 후 marker 플립.
- [ ] (Should, FR-03) test 파일 내부 location ad-hoc 흡수 0 — `grep -rnE "delete\s+window\.location|window\.location\s*=" src --include="*.test.{ts,tsx,js,jsx}"` → 0 hit (또는 `src/test-utils/**` 내부 한정). 현 baseline 3 hit / 1 file. task 회수 후 marker 플립.
- [ ] (Should, FR-04) test 파일 내부 `mock.calls[N]!` ad-hoc 흡수 0 — `grep -rnE "\.mock\.calls\[[0-9]+\]!" src --include="*.test.ts" --include="*.test.tsx"` → 0 hit (또는 헬퍼 모듈 내부 한정). 현 baseline 5 hit / 5 file. task 회수 후 marker 플립.
...
- [ ] (Must, FR-06) 단일 출처 정합 — `grep -rnE "const\s+(setLocation|mockUrlLocation|firstCall|nthCall)\s*=" src --include="*.{ts,tsx,js,jsx}"` → `src/test-utils/**` 외부 0 hit. 현 baseline `src/common/common.test.ts` module-local 헬퍼 3 const. task 회수 후 marker 플립.
...
- [ ] (NFR-05) RULE-02 정합 — task 회수 시 변경 표면 = `src/test-utils/**` 1+ 신규/확장 모듈 + N 개 test 파일 import 치환. `src/` runtime 코드 변경 0 + `package.json` 변경 0 (vitest/jsdom 버전 영향 0). task 회수 후 marker 플립.
