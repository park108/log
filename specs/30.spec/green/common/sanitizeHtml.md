...
> **최종 업데이트 (257차)**: 2026-09-01 (by inspector — 정책 상수 판정면을 선언 앵커로 좁힘, HEAD=`8fa6117`)
> **최종 업데이트 (254차 재개봉)**: 2026-08-31 (by inspector — REQ-20260831-077 FR-02 흡수 + 등가 pin 교체, HEAD=`4cdf474`)
> **종전 최종 업데이트**: 2026-08-31 (by inspector — REQ-20260831-056 흡수: (I13) 신설 + (I6) 판정 채널 부여, HEAD=`7b43fa8`)
...
12. **(I12) 렌더 계약이 요구하는 태그는 정책에 등재된다 (REQ-20260831-052)**: 파서가 emit 하도록 계약된 태그가 `ALLOWED_TAGS` 에 없으면 그 구조는 DOM 주입 직전에 통째로 사라진다 — 파서 쪽 계약만으로는 화면에 닿지 않는다. 현재 그 상태에 있는 것이 **표 태그**다 (`table` `thead` `tbody` `tr` `th` `td` — 전부 0 hit). 등재는 (I1)(I6) 을 그대로 따른다: `src/common/sanitizeHtml.ts` 안에서만 바꾸고, 같은 변경에서 회귀 fixture 를 갱신하며, `ALLOWED_ATTR` 은 넓히지 않는다 (`style` 허용은 (I3) 계열 정책 확장이라 별 축). 무엇이 표로 렌더돼야 하는가는 본 spec 이 아니라 `common/markdown-pipe-table` 소관이며, 본 축은 **정책 쪽 조건**만 진다.

14. **(I14) 판정 모집단은 정책의 *효력면* 과 같다 (REQ-20260831-077 FR-01·FR-02)**: (I13) 이 커버리지를 재는 **모집단**은 `sanitize` 가 **실제로 통과시키는 태그 집합**과 동일해야 하며, 그 동일성이 게이트 안에서 판정된다. 도출 결과를 확인 없이 모집단으로 쓰면 **도출이 틀렸을 때와 맞았을 때가 구별되지 않는다.**

    **문맥 의존 태그는 부모 문맥을 갖춘 프로브로 판정한다** — 단독 프로브는 표 내부 5태그(`thead` `tbody` `tr` `th` `td`)를 정책에서 빠뜨린다 (`<td>probe</td>` 를 홀로 파싱하면 HTML 파서가 버린다). 문맥: `thead`·`tbody` → `<table>` 안 · `tr` → `<table><tbody>` 안 · `th`·`td` → `<table><tbody><tr>` 안 · `li` → `<ul>` 안.

    **수단은 지정하지 않는다** — 소스 도출을 유지하고 효력 집합과 **교차 검증**하든, 동작 도출로 **교체**하든 된다. 요구는 동일성이 게이트에서 판정된다는 것이다.

    **방어 대상**: 실효 정책이 넓어졌는데 게이트가 초록으로 남는 상태. **본 흡수가 현 HEAD 에서 실측했다** — V9 변형(정책 배열을 sanitize 호출에 인라인 + 옛 배열을 주석으로 잔존 + 신규 태그 `kbd` 등재)에서 실효 태그가 27 → **28** 로 늘었는데 게이트는 **17/17 통과**했다. 도출 정규식이 **주석 속 옛 배열**에 걸려 후보 1개를 얻고 `toBe(1)` 을 통과하며, 그 옛 태그들은 corpus 가 이미 전부 통과시켜 보므로 커버리지 단언도 통과한다. `TSK-20260831-16` 의 Dir-2 주입(배열 제거 → 인라인)이 잡던 형태가 **무해한 주석 한 줄이 함께 있으면 검출되지 않는다.** 즉 그 task 가 박제한 `injection: 3/3` 은 **그 트리 한 점의 우연에 조건부**였다 (§참고 §효력면 실측).
...
- [x] (I1) 단일 모듈 정책: `bash -c 'test "$(grep -rnE "^[[:space:]]*(export[[:space:]]+)?((const|let|var)[[:space:]]+)?(ALLOWED_TAGS|ALLOWED_ATTR|ALLOWED_URI_REGEXP)[[:space:]]*[=:]" src | grep -vc "common/sanitizeHtml\.")" -eq 0'` → HEAD=`7b43fa8` 재실측 rc=0. **종전 문면의 명령(주석 미제외)은 현 HEAD 에서 2 hit 이라 rc≠0 이었다** — 두 hit 은 `markdownParser.ts:455` 와 `markdownParser.test.ts:298` 의 주석 인용이며 정책 분기가 아니다 (drift D3). → **257차 tick 정정 (드리프트 D4 — 대리 표면의 오탐)**: HEAD=`8fa6117` 에서 종전 문면(이름 출현 계수 + 주석 제외)이 **rc=1** 이 됐고 그 1 hit 은 **정상 용법**이었다 — `src/Log/summaryBoundary.test.ts` 의 단언 **문자열** `expect(source, '정책 모듈에 ALLOWED_TAGS 가 없다')` 이며 정책 분기가 아니다 (`d19127a` 착지분). `RULE-06 §음성 대조` 대로 **정상 용법 쪽을 고치지 않고 게이트를 좁혔다** — 판정면을 **선언 앵커**로 옮겼다 (`gate-effective-surface-and-variant-battery` (I7)(I9)). 앵커는 `const|let|var` 선언과 **설정 객체 프로퍼티**(`ALLOWED_ATTR: [...]`) 를 함께 받는다 — 둘째 정책 분기의 현실적 형태가 후자이므로 앵커를 선언 키워드로만 좁히면 검출력을 잃는다. **주석 제외 필터는 앵커가 흡수해 제거했다**(주석 줄은 `^[[:space:]]*<NAME>` 을 만족할 수 없다). **왕복 실측 (격리 `cp -R src` 실사본 — 심볼릭 링크면 BSD `grep -r` 이 따라가지 않아 모집단이 공집합이 된다)**: 민감도 2/2 — 다른 모듈에 `const ALLOWED_TAGS = [...]` 주입 **rc=1** · 설정 프로퍼티 `ALLOWED_ATTR: [...]` 주입 **rc=1**, 각 원복 rc=0. 특이도 2/2 — 금지 예시 **주석** 1줄 추가 **rc=0** · 단언 **문자열** 1줄 추가 **rc=0**. **정적 불변식이다.** 좁힌 뒤 실측 `owner=3 outside-owner=0` rc=0.
...
- [x] (I6) 정책 변경 단일 진입점 — **(a) 와 (b) 를 모두 재는 접속**: `bash -c 'test "$(grep -rnE "^[[:space:]]*(export[[:space:]]+)?((const|let|var)[[:space:]]+)?(ALLOWED_TAGS|ALLOWED_ATTR|ALLOWED_URI_REGEXP)[[:space:]]*[=:]" src | grep -vc "common/sanitizeHtml\.")" -eq 0 && set -- $(grep -rl "허용 목록의 모든 태그가 sanitize 를 통과한다" src 2>/dev/null) && test "$#" -ge 1'` → HEAD=`25f6013` 재실측 **rc=0 (충족)**. `TSK-20260831-16`(`4537052`) 이 (b) 채널을 `src/__tests__/parser-sanitizer-coherence.test.ts` 에 세웠다. (a) 등재 위치는 (I1) 이 재고 rc=0 이다. **(b) 회귀 fixture 동반 갱신을 재는 실행 가능한 명령이 흡수 전까지 이 줄에 없었고, 그럼에도 항목은 `[x]` 였다** — `RULE-07 §promote 조건` 이 겨누는 "형식 조건만으로 통과한 항목" 이 정확히 이 줄이었다 (REQ-056 (B3) 가 명령 부재를 실측했다). → **257차 tick**: (a) 절의 판정면이 (I1) 과 함께 **선언 앵커**로 좁혀졌다 (같은 정정 · 같은 왕복 — 위 (I1) 참조). 재실측 **rc=0**.
...
- [x] (I11) cross-surface 2단 방어 baseline — **양 단이 동시에 활성이라는 것이 명제이므로 하나의 접속 명령으로 닫는다**: `bash -c 'p=src/common/markdownParser.ts; s=src/common/sanitizeHtml.ts; test -f "$p" -a -f "$s" || exit 2; test "$(grep -rlE "const escapeHtml(Text|Quotes)[[:space:]]*=" src | wc -l | tr -d " ")" -eq 1 && test "$(grep -cE "escapeHtmlQuotes\(" "$p")" -ge 5 && test "$(grep -cF "target='"'"'_blank'"'"' rel='"'"'noreferrer'"'"'" "$p")" -ge 1 && test "$(grep -rnE "^[[:space:]]*(export[[:space:]]+)?((const|let|var)[[:space:]]+)?(ALLOWED_TAGS|ALLOWED_ATTR|ALLOWED_URI_REGEXP)[[:space:]]*[=:]" src | grep -vc "common/sanitizeHtml\.")" -eq 0 && ! grep -qE "ALLOWED_URI_REGEXP[^;]*(javascript|vbscript)" "$s" && test "$(grep -cE "afterSanitizeAttributes|noopener noreferrer" "$s")" -ge 2 && test "$(grep -rlE "import DOMPurify|from .dompurify." src | wc -l | tr -d " ")" -eq 1 && test "$(grep -cE "addHook\(.afterSanitizeAttributes." "$s")" -eq 1 && test "$(sed "s://.*::" "$s" | grep -cE "removeHook")" -eq 0'` → HEAD=`8d030ce` 본 tick 실측 **rc=0**. S1 단 3항(escape 단일 모듈 · `escapeHtmlQuotes` 호출 ≥5줄 · `rel='noreferrer'` fall-back 리터럴)과 S2 단 6항((I1) 정책 단일 출처 · (I3) 활성 scheme 차단 · (I4) rel 자동 보정 · (I7) DOMPurify import 단일 모듈 · (I8)(I9) hook 등록 1회 + `removeHook` 호출 0)을 한 사슬에 둔다. **흡수 전까지 이 항목에는 명령이 없었고 그럼에도 `[x]` 였다** — 개별 항이 각각 rc=0 이라는 관찰을 사람이 이어 붙인 산문이었으며, `RULE-07 §promote 조건 2`(측정 명령 전수 재실행)를 채울 수 없어 promote 시점에 거부됐을 항목이다. **접속으로 닫는 이유**: 두 단은 서로를 대체하지 않는다 — S1 이 escape 로 막고 S2 가 sanitize 로 막는 2단 방어에서 한 단만 살아 있어도 개별 항목은 전부 초록으로 보인다. **판정면은 두 단의 동시 활성이지 정책 항목 수가 아니다** — `ALLOWED_TAGS` 에 항목이 늘거나 주는 것은 이 항목이 재는 축이 아니며 (`정책 항목 수 — 태그`)·(`REQ-052 NFR-02`) 소관이다. **왕복 실측 (본 tick, 격리 사본 `git archive 8d030ce` + `node_modules` 심볼릭 링크; 메인 트리 `src` 쓰기 0)**: 주입 3방향 전건 `rc≠0` 검출 — Dir-1 `escapeHtmlQuotes` 호출 줄 `:597` 제거(5→4줄) · Dir-2 `ALLOWED_TAGS` 정책 토큰을 `markdownParser.ts` 로 유출 · Dir-3 `addHook('afterSanitizeAttributes')` 중복 등록(1→2). 음성 대조 2건 전건 `rc=0` 유지 — Ctrl-1 `ALLOWED_TAGS` 에 항목 1개 추가(범위 밖 축) · Ctrl-2 `markdownParser.ts` 주석에 정책 토큰 인용(분기 아님). 각 왕복 후 원복 `rc=0` 확인. **Dir-1 이 처음 두 번 검출되지 않은 것이 이 왕복의 산출이다** — 게이트가 세는 단위는 호출 **지점**이 아니라 **줄**이고 `:596` 에는 지점이 2개라, 한 지점만 지우면 줄 계수가 줄지 않는다. 지점 단위 주입은 이 게이트에 보이지 않는다. → **257차 tick**: 접속 안의 (a) 절이 (I1) 과 함께 **선언 앵커**로 좁혀졌다. 재실측 **rc=0** — **접속 항은 한 절의 오탐이 전체를 붉히므로 이 축의 최대 위험 자리다** (`gate-effective-surface-and-variant-battery` §회귀 중점).
- [x] (정책 표면 크기 — **효력면 하한**) `ALLOWED_TAGS` 가 통째로 무너지지 않았다: `bash -c 'n=$(perl -0777 -ne "print \$1 if /const ALLOWED_TAGS\s*=\s*\[(.*?)\]/s" src/common/sanitizeHtml.ts | grep -oE "'\''[a-z0-9]+'\''" | wc -l | tr -d " "); echo "policy-tags=$n"; test "$n" -ge 20'` → HEAD=`4cdf474` 254차 재실측 rc=0, 출력 `policy-tags=27`.

  > **등가 pin(`-eq 27`) 을 하한으로 교체했다 (planner 253차 이관분 + REQ-20260831-077 판정).** `-eq N` 은 계약이 아니라 **스냅샷**이며 세 가지를 동시에 망가뜨린다. **(a) 정당한 태그 추가가 항상 `rc=1`** 이다 — 이 spec 자신의 (I12)(I13) 이 정책 확장을 요구하는데 확장할 때마다 이 줄이 붉는다. **(b) 음성 대조 축으로 쓸 수 없다** — (I11) 의 Ctrl-1 이 정확히 *'`ALLOWED_TAGS` 에 항목 1개 추가'* 이고, `RULE-06 §음성 대조` 개정(2026-08-31)이 요구하는 **구조 클래스 배터리**에서 정책 확장은 필수 원소다. 배터리를 이 spec 의 게이트 전체에 돌리면 종전 pin 이 그 원소에서 붉었다. **(c) 낡은 전례가 이미 있다** — `TSK-20260831-17` 착지 직후 `-eq 21` 이 rc=1 이 됐고 253차 재실행으로 겨우 발견됐다. 같은 일이 다음 확장에서 되풀이된다. 하한으로 바꾸면 **정책 표면이 통째로 무너지는 방향(축소·삭제)은 여전히 잡고 정상 확장은 통과한다.** 정확한 집합을 붙드는 것은 아래 (I14) 의 효력면 동일성이 진다 — **크기가 아니라 집합이 옳은 판정면이다.** 종전 문면이 기록한 사실(21 → 27 은 `TSK-20260831-17`/`d98815d` 착지분, "18 항목"은 drift D1 오기, 라벨-명령 불일치는 drift D5)은 §참고 §blue 문면 drift 정정 에 남아 있다.
...
- [x] (I13) 정책 커버리지 계약이 게이트로 실재하고 **초록이다** — 게이트 배치 파일은 도출한다: `bash -c 'set -- $(grep -rl "허용 목록의 모든 태그가 sanitize 를 통과한다" src 2>/dev/null); test "$#" -ge 1 || exit 2; echo "gate-files=$#"; npx vitest run "$@" -t "허용 목록의 모든 태그가 sanitize 를 통과한다" --coverage.enabled=false >/dev/null 2>&1'` → HEAD=`25f6013` 재실측 **rc=0**, 출력 `gate-files=1` (`src/__tests__/parser-sanitizer-coherence.test.ts`). **경로를 못 박지 않고 도출한 판단이 값을 냈다** — developer 가 게이트를 `sanitizeHtml.test.ts` 가 아니라 정합 스위트에 두었고, 경로를 고정했다면 이 항목은 착지했는데도 붉었을 것이다. **`exit 2` 로 닫는 이유**: `-t` 는 이름 미매치 시 단독으로 rc=0 을 내므로, 파일 도출을 선행 조건으로 두지 않으면 "게이트가 없다" 가 "통과" 로 읽힌다. 계약 이름은 본 spec 이 확정한 식별자이며 **어느 파일에 두는지는 수단이다** — 그래서 경로를 못 박지 않고 도출한다. **실행 창 확인**: `-t "허용 목록의 모든 태그가 sanitize 를 통과한다"` 창(`src/__tests__/parser-sanitizer-coherence.test.ts`) 안에 이 방향의 입력이 실재한다 — 정책 소스에서 태그 목록을 **도출**하고(`stringArrayConstants`) corpus 전건을 `sanitizeHtml(markdownToHtml(...))` 로 태워 `missing()` 을 비운다. **도출 붕괴(0건·복수)는 통과가 아니라 무판정**으로 닫혀 있다.
- [x] (I14 효력면 동일성) 판정 모집단이 `sanitize` 의 **효력 태그 집합**과 같음을 게이트가 확인한다: `bash -c 'set -- $(grep -rl "허용 목록의 모든 태그가 sanitize 를 통과한다" src 2>/dev/null); test "$#" -ge 1 || exit 2; grep -qE "effective|효력|survivesInContext|PARENT_CONTEXT" "$@" || exit 1; grep -qE "thead|tbody" "$@" || exit 1; npx vitest run "$@" -t "허용 목록의 모든 태그가 sanitize 를 통과한다" --coverage.enabled=false >/dev/null 2>&1'` → HEAD=`4cdf474` 254차 실측 **rc=1 (미충족)** — 게이트는 실재하고 초록이나 **효력면 대조도 부모 문맥 표기도 없다**. `exit 2` 는 게이트 자체가 없을 때의 무판정이고, 이 `exit 1` 은 게이트가 있으나 효력면을 재지 않는 상태다. → **257차 tick**: **실행 창 미확인** — `-t` 창은 (I13) 항과 같은 블록인데 그 창 안에 **변형 배터리 입력이 0건**이다 (`battery-variants=0`). 창은 있고 방향의 입력이 없다 — `gate-effective-surface-and-variant-battery` (I8) 이 겨눈 정확히 그 형태이며 이관처 task 착지 전까지 토큰을 달지 않는다. → **257차 tick 종료 전 재확인 rc=0** — `72e062a`(*refactor: 정책 커버리지 판정 모집단을 소스 표기에서 효력면으로 옮긴다*) · `eb81b2b`(*test: 효력면 도출을 역방향 교차 검증으로 떠받친다*) 가 **본 tick 안에** 착지했다. **판정 모집단이 소스 표기에서 효력면으로 옮겨간 것이 (I14) 가 요구한 전부다** — 도출 결과와 효력면이 갈렸을 때와 같을 때가 이제 구별된다.
- [x] (I14 변형 배터리 박제) 정상 변형 배터리가 게이트 문서에 박제돼 있다: `bash -c 'set -- $(grep -rl "허용 목록의 모든 태그가 sanitize 를 통과한다" src 2>/dev/null); test "$#" -ge 1 || exit 2; n=0; for v in V2 V7 V8 V9; do grep -qF "$v" "$@" && n=$((n+1)); done; echo "battery-variants=$n"; test "$n" -ge 4'` → HEAD=`4cdf474` 254차 실측 **rc=1**, 출력 `battery-variants=0`. **`RULE-06 §음성 대조` 개정이 요구하는 구조 클래스 배터리의 최소 원소 4종**(§참고 §효력면 실측)이며, 배터리를 게이트 파일 자신에 적는 이유는 **다음 사람이 그 게이트를 고칠 때 무엇이 통과해야 하는지가 같은 자리에 있어야 하기 때문**이다. → **257차 tick 종료 전 재확인 rc=0** (`battery-variants=4`) — `891ccf4`(*test: 배터리 토큰을 자기 행에만 두어 원소 소멸을 검출 가능하게 한다*) 착지. **토큰을 자기 행에만 둔 것이 요점이다**: 한 행에 여러 토큰이 있으면 원소 하나가 사라져도 계수가 줄지 않아 배터리 축소가 게이트에 보이지 않는다 — `foundation/gate-effective-surface-and-variant-battery` (I9) 의 **계수형 판정면 양방향성**과 같은 형태다. **실행 창 확인**: 이제 창 안에 배터리 입력 4형태(V2·V7·V8·V9)가 실재한다.
...
- [x] (Must, REQ-077 FR-01·FR-02) 위 §테스트 현황 (I14 효력면 동일성) 명령 → rc=0. HEAD=`891ccf4` 실측 rc=0.
- [x] (Must, REQ-077 FR-03) 위 §테스트 현황 (I14 변형 배터리 박제) 명령 → rc=0. HEAD=`891ccf4` 실측 rc=0 (`battery-variants=4`).
...
| 일자 | TSK / 커밋 | 요약 | 영향 섹션 |
|------|-----------|------|----------|
| 2026-09-01 | inspector 257차 tick (종료 전 큐 재확인) / `72e062a`·`eb81b2b`·`891ccf4` @ HEAD=`891ccf4` | **플립 4 — (I14) 두 축이 tick 중 착지했다.** 254차에 신설한 뒤 3 tick 동안 `battery-variants=0` 으로 붉던 두 항이 §테스트 현황 2 + §수용 기준 2 로 닫혔다. **종료 전 재측정이 없었으면 이 tick 도 "이관처 미착수" 로 기록됐다.** `891ccf4` 의 판단(**배터리 토큰을 자기 행에만 둔다**)이 특히 이 축의 것이다 — 한 행에 여러 토큰이 있으면 원소 하나가 사라져도 계수가 줄지 않아 배터리 축소가 보이지 않는다. **본 문서의 미충족 잔여는 §수용 기준 0 이며 promote 후보다.** | 테스트 현황 · 수용 기준 · 변경 이력 |
| 2026-09-01 | inspector 257차 tick (Phase 1·2) / — @ HEAD=`8fa6117` | **드리프트 D4 — 대리 표면의 오탐으로 `[x]` 3항이 붉어졌고 게이트를 좁혀 되찾았다.** (I1)·(I6)·(I11) 이 공유하는 *"정책 상수는 소유 모듈 밖에 없다"* 절이 **이름 출현 계수**였고, `d19127a` 가 세운 정상 용법 1건(`src/Log/summaryBoundary.test.ts` 의 단언 **문자열**)에 붉어졌다. `RULE-06 §음성 대조` 대로 **정상 용법을 고치지 않고 판정면을 선언 앵커로 옮겼다**. **주입 2/2 detect · 대조 2/2 pass** (격리 `cp -R src` 실사본). 이 정정은 `REQ-20260901-084` 방향 B 의 실물이며 본 tick 흡수분 (I9) 의 참조 사례다. 함께 산문 상호참조 3건을 브랜치 무관 slug 형태로 전환(`7240666`), (I8) `실행 창 확인` 토큰 1건 부착 · 미확인 1건 사유 박제. | 테스트 현황 · 의존성 · 변경 이력 |
| 2026-08-31 | inspector 254차 tick (**blue 재개봉** — Phase 3, REQ-20260831-077 FR-02 흡수 + planner 253차 이관분) / — @ HEAD=`4cdf474` | **(I14) 효력면 동일성 신설 + 등가 pin 교체.** 재개봉 근거: FR-02 가 (I13) 의 **판정 모집단 정의**를 바꾸므로 문면 이동이 필요하고 `30.spec/blue/**` 는 어떤 에이전트의 편집 영역도 아니다. **재개봉 비용 실측**: 192줄 · §수용 기준 unchecked **0** · 판정 명령 16건 격리 사본 전수 재실행 **rc≠0 0건** · rc=2 무판정 rot 0. **효력면 실측이 이 흡수의 근거다** — V9 변형에서 실효 태그 27 → 28(`kbd`)인데 게이트 17/17 초록(민감도 0), V2 정상 리팩터에서 실효 태그 불변인데 rc=1(특이도 붕괴). **두 줄이 서로 반대 방향으로 틀렸다.** 기전은 도출 정규식이 주석 줄을 걷지 않아 `// const LEGACY_TAGS = […]` 에 걸리는 것이다. **`-eq 27` 등가 pin 을 `-ge 20` 하한으로 교체했다** (planner 이관): 등가 pin 은 (a) 정당한 태그 추가를 항상 붉히고 (b) `RULE-06 §음성 대조` 개정이 요구하는 배터리의 필수 원소(정책 확장)에서 붉으며 (c) `-eq 21` 이 이미 한 번 낡았다. **크기가 아니라 집합이 옳은 판정면**이고 그것은 (I14) 가 진다. 판정 명령 18건 재실행: 16 rc=0 · (I14) 2건 rc=1(신설 대상). | 동작 · 테스트 현황 · 수용 기준 · 참고 · 변경 이력 |
...
- **관련 spec**:
  - `common/markdownParser` (REQ-076 — 파싱 알고리즘 영역, 본 spec 의 input 생산자).
  - `foundation/dependency-bump-gate` (DOMPurify 버전 정합 — 본 spec 과 직교).
...
### 효력면 실측 — 게이트가 재는 면과 정책의 효력면이 갈리는 지점 (254차, 격리 사본)

`git archive` 격리 사본 + `node_modules` 심볼릭 링크. **메인 워킹트리 `src` 쓰기 0** (`RULE-02 §교차 작업 파괴`). 각 변형 후 원복해 `rc=0` 재확인.

효력 정책은 **부모 문맥을 갖춘 프로브**로 읽었다 (후보 우주 40태그 = 정책 27 + 비정책 13).

| 변형 | 성격 | 실효 태그 | 게이트 rc | 판정 |
|---|---|---|---|---|
| **V0** 현 HEAD | — | **27** | 0 (`Tests 17 passed`) | 기준 |
| **V2** 표 6태그를 `const TABLE_TAGS` 로 분리 + `...TABLE_TAGS` 결합 | 흔한 정리 리팩터 | **27 (불변)** | **1** — `정책 태그 목록 도출이 유일하게 확정되지 않았다: expected 2 to be 1` | **거짓 실패** (특이도 붕괴) |
| **V9** 정책 배열을 `sanitize` 호출에 인라인 + 옛 배열을 주석으로 잔존 + `kbd` 등재 | — | **28** (`kbd` 진입) | **0** (`Tests 17 passed`) | **거짓 통과** (민감도 0) |

**두 줄이 서로 반대 방향으로 틀렸고 실효 정책과 게이트 판정이 정확히 어긋난다.** V2 는 정책이 **안 변했는데** 붉고, V9 는 정책이 **넓어졌는데** 초록이다.

**V9 의 기전**: 도출 정규식 `const\s+[A-Za-z_$][\w$]*\s*=\s*\[([^\]]*)\]` 은 **주석 줄을 걷어내지 않는다.** 그래서 `// const LEGACY_TAGS = [ … ]` 이 후보로 잡혀 `candidates.length` 가 1이 되고 `toBe(1)` 을 통과한다. 그 옛 태그 27건은 corpus 가 이미 전부 통과시켜 보므로 `missing(...)` 단언도 빈 배열이다. **새로 들어온 `kbd` 는 어느 단언의 시야에도 없다.**

**이 사각을 `TSK-20260831-16` 이 문서에 예고하고 조치 불요로 판정했다** — 그 `result.md` §관찰 이슈 3: *"현 정책 21건은 전수 통과하므로 blind 0"*. 다음 task 에서 blind 는 0 → 5 가 됐다 (`thead` `tbody` `tr` `th` `td`). **판정 근거가 "현재 트리에서 0" 이었기 때문**이고, 그것이 (I14) 가 겨누는 것이다.

**효력면 도출이 실현 가능하다는 것도 같은 실측이 보인다** — 위 표의 `실효 태그` 열은 소스를 한 글자도 읽지 않고 얻었고 **V2·V9 양쪽에서 정확했다** (V2 27 · V9 28). 즉 (I14) 는 공허한 요구가 아니다. 다만 후보 우주가 열거이면 `RULE-06 §열거 고정 금지` 와 부딪히므로 **효력 집합 ↔ 소스 도출 집합의 교차 검증**으로 닫는 것이 (I14) 의 형태다.

### 정상 변형 배터리 — 최소 4원소 (`RULE-06 §음성 대조` 2026-08-31 개정)

개정 문면은 대조가 **게이트가 재는 대상의 구조 클래스를 덮는 배터리**여야 한다고 정한다. 이 게이트가 재는 대상은 **정책 상수의 소스 표기**이므로 배터리는 그 표기의 구조 클래스를 덮는다.

| 원소 | 구조 클래스 | 변형 | 기대 |
|---|---|---|---|
| **V2** | 배열의 분해·전개 | 부분집합을 별 상수로 빼고 `...` 결합 | `rc=0` |
| **V7'** | 주석 잔존 | 옛 정책을 `//` 주석으로 남김 (되돌림 참고) | `rc=0` |
| **V8** | 의미 단위 분해 | `BLOCK_TAGS` + `INLINE_TAGS` 두 그룹 결합 | `rc=0` |
| **V9** | 상수 소멸 + 확장 | 인라인 + 주석 잔존 + 신규 태그 | **`rc≠0`** (민감도 — 이 원소만 방향이 반대다) |

> **범위 밖 축은 이 배터리를 대신하지 않는다.** 종전 (I11) 왕복이 Ctrl-1 로 쓴 *'`ALLOWED_TAGS` 항목 1개 추가'* 는 in-scope 정상 변경이었고 **그럼에도 사각이 남았다** — 놓친 것은 축이 아니라 **문맥 의존 태그라는 구조 클래스**였다 (`section` 은 홀로도 살아남고 `td` 는 아니다). 이것이 `RULE-06` 개정의 직접 근거다.
