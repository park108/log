...
> **위치**: `src/Log/LogItemInfo.tsx` (`link-copy-button`) · `src/Comment/CommentItem.tsx` (`reply-toggle-button`). 게이트: `src/common/a11y.audit.test.ts` · `src/common/a11y.audit.exemptions.ts` · `src/__tests__/hover-popup-anchoring.test.ts`.
> **관련 요구사항**: REQ-20260830-047 FR-01·FR-02·FR-03·FR-04·FR-06·FR-07
> **최종 업데이트**: 2026-08-31 (by inspector — REQ-20260830-047 흡수, 축 분리)

> 참조 코드는 **식별자 우선**. 라인 번호는 스냅샷 (HEAD=`efe6268`).
...
4. **(I4) 기준점 목록의 동반 갱신**: 전환으로 팝업의 기준점이 바뀌면 `src/__tests__/hover-popup-anchoring.test.ts` 의 `HOSTS` 도 함께 바뀐다. 게이트를 느슨하게 만들어 통과시키지 않는다 — 이 게이트의 요지는 "팝업을 얹으면 기준점을 함께 선언하라" 이다.
...
- 팝업을 형제로 옮기고 새 래퍼에 `position: relative` 를 주지 않으면 `top: 100%` 가 뷰포트 높이를 잡아 팝업이 화면 밖으로 간다 (실측 y=1298 — `hover-popup-anchoring.test.ts` 주석).
...
## 테스트 현황
- [ ] (I1) 두 지점의 손조립 소멸: `bash -c '! grep -rnE "^[[:space:]]*role=\"button\"" src/Log/LogItemInfo.tsx src/Comment/CommentItem.tsx'` → HEAD=`efe6268` 실측 **rc=1 (FAIL)** — 2 hit (`LogItemInfo.tsx:82` · `CommentItem.tsx:90`).
- [ ] (I1) `link-copy-button` 이 `<button>`: `bash -c 'perl -0777 -ne '\''exit(1) unless /<button[^>]*data-testid="link-copy-button"/'\'' src/Log/LogItemInfo.tsx'` → HEAD=`efe6268` 실측 **rc=1 (FAIL)**.
- [ ] (I1) `reply-toggle-button` 이 `<button>`: `bash -c 'perl -0777 -ne '\''exit(1) unless /<button[^>]*data-testid="reply-toggle-button"/'\'' src/Comment/CommentItem.tsx'` → HEAD=`efe6268` 실측 **rc=1 (FAIL)**.
- [x] (I2) 예외 2 지점 보존: `bash -c 'grep -qE "^[[:space:]]*role=\"button\"" src/Image/ImageItem.tsx && grep -qE "^[[:space:]]*role=\"button\"" src/common/UserLogin.tsx'` → HEAD=`efe6268` 실측 rc=0.
- [x] (I4) 기준점 게이트 현행 PASS: `npx vitest run src/__tests__/hover-popup-anchoring.test.ts` → HEAD=`efe6268` 실측 rc=0 (3 it).
- [x] (I5) 계약면 현행 PASS: `npx vitest run src/Log/LogItemInfo.test.tsx src/Log/LogItem.test.tsx src/Comment/CommentItem.test.tsx src/Comment/Comment.test.tsx` → HEAD=`efe6268` 실측 rc=0 (72 tests). 이 72개가 전환의 회귀 그물이다.
- [x] (I1 보조) 감사 게이트 현행 PASS: `npx vitest run src/common/a11y.audit.test.ts src/common/a11y.focus-visible.test.ts` → HEAD=`efe6268` 실측 rc=0 (8 tests). **이 rc=0 이 손조립 2건을 허용하고 있다는 점이 본 spec 의 계기다** — 패턴 B 충족은 네이티브 button 을 대신하지 못한다.
- [x] (I7) 면제 공집합: `bash -c 'perl -0777 -ne '\''exit(1) unless /PATTERN_B_EXEMPTIONS[^=]*=\s*\[\s*(?:\/\/[^\n]*\n\s*)*\]/'\'' src/common/a11y.audit.exemptions.ts'` → HEAD=`efe6268` 실측 rc=0.
...
- [x] (Must, FR-02·FR-06) `npx vitest run src/__tests__/hover-popup-anchoring.test.ts` → rc=0. HEAD=`efe6268` 실측 rc=0 (3 it). 현행 PASS 이며 전환이 이를 깨지 않는 것이 요구다 — promote 시 재실행 대상 (RULE-07 §promote 조건 2).
- [x] (Must, FR-04·NFR-03) `npx vitest run src/Log/LogItemInfo.test.tsx src/Log/LogItem.test.tsx src/Comment/CommentItem.test.tsx src/Comment/Comment.test.tsx` → rc=0. HEAD=`efe6268` 실측 rc=0 (4 files / 72 tests). 동일 — 전환 후 재실행이 (I5) 계약면 보존의 판정이다.
- [x] (Must, NFR-01) `npx vitest run src/common/a11y.audit.test.ts src/common/a11y.focus-visible.test.ts` → rc=0. HEAD=`efe6268` 실측 rc=0 (2 files / 8 tests). 동일 — 손조립이 패턴 B 로 통과하던 상태를 전환이 유지하는지를 따진다.
- [x] (Should, FR-07) `PATTERN_B_EXEMPTIONS` 공집합: 위 (I7) 명령 rc=0.
- [x] (Must, 범위 제한) 예외 2 지점 보존: 위 (I2) 명령 rc=0.

## 스코프 규칙
- **expansion**: 불허 — 전환 대상은 `src/Log/LogItemInfo.tsx` · `src/Comment/CommentItem.tsx` 2 파일과 그 팝업 기준점 CSS(`src/Log/Log.css` · `src/Comment/Comment.module.css`), 그리고 `HOSTS` 목록(`src/__tests__/hover-popup-anchoring.test.ts`) 뿐이다. 게이트 위반이 이 밖에서 나오면 격리 대상이다.
- **grep-baseline** (HEAD=`efe6268`, 2026-08-31 실측):
  - `grep -rnE "^[[:space:]]*role=\"button\"" src --include="*.tsx" | grep -v "\.test\."` → 4 hits in 4 files:
    - `src/Comment/CommentItem.tsx:90` (전환 대상), `src/Log/LogItemInfo.tsx:82` (전환 대상)
  ...
  - `grep -cE "<button" src/Log/LogItemInfo.tsx src/Comment/CommentItem.tsx` → `LogItemInfo.tsx:1` · `CommentItem.tsx:0`. **LogItemInfo 의 1 hit 은 `:172` 의 별개 button 이며 `link-copy-button` 이 아니다** — 그래서 §수용 기준은 `<button` 존재 여부가 아니라 `data-testid` 결합을 `perl -0777` 로 잰다.
...
- **`HOSTS` 가 "실제 기준점" 을 가리키는지는 게이트가 보지 않는다.** `hover-popup-anchoring.test.ts` 의 3 it 은 (a) 팝업 규칙의 좌표·폭, (b) `anchor` 클래스 규칙이 위치 지정돼 있음, (c) 팝업 클래스를 쓰는 **파일 목록**이 `HOSTS` 와 일치함만 잰다. **팝업의 실제 DOM 부모가 그 `anchor` 인지는 검사하지 않는다.** 따라서 전환 후 팝업을 다른 래퍼의 자식으로 옮겨도 파일→팝업 쌍이 그대로면 rc=0 이 나온다. (I4) 의 실질은 구현 리뷰와 (I6) 실측이 함께 지킨다.
...
- **관련 커밋**: `7cd575a` (22곳 중 19곳 전환 + 3곳 근거 박제), `520f906` (팝업 명시 좌표 + 기준점 + 게이트).
- **선행 게이트**: `src/__tests__/hover-popup-anchoring.test.ts`, `src/common/a11y.audit.test.ts`, `src/common/a11y.audit.exemptions.ts`.
