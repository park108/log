...
**결정적으로, 그 굳음이 수정을 막는 근거로 인용됐다.** blue `markdown-list-item-continuation.md:105` 는 이 공백을 *"고치지 않는다"* 면서 그 근거로 `markdownParser.test.ts:125` 가 표기를 잠근다는 것을 든다. 그 기대값은 결함 있는 산출을 그대로 옮겨 적은 것이다 — **산출이 옳은 이유는 게이트가 잠그기 때문이고, 게이트가 그렇게 잠근 이유는 산출을 받아 적었기 때문이다.** 본 계약은 그 순환을 끊는다. 같은 문단의 두 번째 근거("렌더에서 보이지 않는다")는 참이며 위에서 재현했으나, 그것은 **심각도**의 근거이지 **정당성**의 근거가 아니다.
