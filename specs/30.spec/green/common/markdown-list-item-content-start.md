...
**그리고 채널이 없는 동안 결함은 사라지지 않고 기대값으로 승격된다.** 주입된 공백은 HEAD=`8d030ce` 에서 게이트 기대값 **18곳**(단언 10줄, 전부 `<ol>`; `<ul>` 쪽은 0곳)과 spec 문서 2건 **34곳**(blue `markdown-list-item-continuation.md` 14 · blue `markdown-blank-line-list-continuity.md` 20)에 정답으로 굳어 있다. **축적은 계속됐다: `2f34d24` 10 → `d762727` 12 → `8d030ce` 18.** 마지막 +6 은 `TSK-20260831-15`(`92820ab`) 가 lazy 이어짐 계약을 박제하면서 그 기대값에 현 산출을 함께 옮겨 적은 것이다 — **본 계약이 아직 착지하지 않은 동안 그것을 고쳐야 할 자리가 절반 늘었다.** 새 계약이 쓰일 때마다 현 산출이 함께 복사된다는 이 문서의 명제가 두 tick 연속 실측으로 확인됐다.
...
- **grep-baseline** (HEAD=`c64c946`, 2026-08-31 흡수 시점 격리 사본 실측):
  ...
  - **(253차 tick 재측정, HEAD=`8d030ce`, 격리 사본)** 두 표기 합 **19** / **2 파일**. 리터럴 **18**(`src/common/markdownParser.test.ts` 단독 · 단언 **10줄**) + DOM 조립 **1**(`src/Log/LogItem.test.tsx` — `expectedChild.innerHTML = " " + contents`). 스캔 모집단 `find src -name "*.test.ts" -o -name "*.test.tsx"` → **116 파일** (불변). 줄별 occurrence: `:41`(3) · `:142`(3) · `:172`(2) · `:728`(2) · `:755`(2) · `:784`(1) · `:802`(2) · `:816`(1) · `:1010`(1) · `:1015`(1). **`92820ab` 이 `:755`·`:784`·`:802`·`:816` 을 새로 만들고 평탄 OL 게이트(`:142`)의 기대 문자열을 갱신했다** — 신규 4줄은 lazy 이어짐 계약의 기대값이며 전부 `<ol>` 이다.
    - **제외 규칙 (음성 대조 근거)**: 주석·평서문의 `<li> ` **3곳**은 세지 않는다 — 상계 `grep -rn "<li> " src --include="*.test.ts" --include="*.test.tsx"` 는 HEAD=`8d030ce` 에서 **13줄**(흡수 시점 9줄)이고 그중 3줄(`Monitor/WebVitalsItem.test.tsx` tooltip 주석 · `markdownParser.test.ts` 의 중첩 평서문 · `markdownParser.test.ts` 의 `(I9)` 주석)이 단언 밖이다. 본 tick 왕복 실측: 단언 쪽을 걷어내도 그 3곳은 그대로 남고 계수에는 들어오지 않았다.
  ...
  - **축적 실측**: 같은 계수가 `2f34d24` **10** → `d762727` **12** → `8d030ce` **18** 이다. 첫 증분 2곳은 빈 줄 목록 연속성 축의 **대조 게이트**를 새로 쓰면서 현 산출을 옮겨 적은 것이고, 두 번째 증분 **6곳**은 `TSK-20260831-15` 가 lazy 이어짐 계약을 박제하면서 같은 일을 한 것이다. **두 증분 모두 본 계약이 착지를 기다리는 동안 발생했다** — 굳음은 시간이 아니라 인접 축의 착지 횟수에 비례해 늘어난다.
  ...
  - spec 문서 쪽 굳음 (**본 계약의 대상이 아니라 배경**): `grep -o "<li> " specs/30.spec/blue/common/markdown-list-item-continuation.md | wc -l` → **14** · 같은 명령을 `specs/30.spec/blue/common/markdown-blank-line-list-continuity.md` 에 → **20**. 합 **34곳** (흡수 시점 31). **후자의 경로가 `green/` → `blue/` 로 바뀌었다** — 승격으로 움직인 경로를 문면이 따라가지 않으면 이 줄의 명령은 파일 부재로 `0` 을 내고 배경이 조용히 사라진다.
...
| 253차 tick (`8d030ce`) | **18** | 1 (`LogItem.test.tsx:464`) | `:41` `:142` `:172` `:728` `:755` `:784` `:802` `:816` `:1010` `:1015` |

**앞의 두 시점에서는 수치가 불변이고 줄 번호만 움직였다. 세 번째 시점에서는 수치가 늘었다** — `92820ab` 이 인접 축을 착지시키며 굳음 4줄을 새로 썼다. 두 부류의 drift 는 다르다: 줄 번호 이동은 게이트를 거짓으로 만들고, 계수 증가는 게이트가 **옳게** 붉어지는 폭을 넓힌다. 도출식 계수는 이 이동에 영향받지 않는다. 줄 번호를 게이트의 판정 근거로 쓰면 게이트는 트리가 움직일 때마다 거짓이 되고, 그 거짓이 `[x]` 아래에 숨는다.
