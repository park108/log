...
- 역의존: 모든 `src/**/*.test.{js,jsx}` 파일이 위 6개 이디엄 중 해당 축을 준수한다.
...
- **rationale**: 8 개 이디엄 축은 `50.blocked/spec/` 에서 격리된 6 followup (2026-04-21T05:41:49Z, category=blocked-revisit) 이 공통으로 요구한 "반복 검증 가능 불변식". 1회성 배치 리팩터 지시는 이미 done req 로 소모 완료; 본 spec 은 잔존 계약 축만 평서형으로 승화한다. baseline 수치는 HEAD=672a4df 시점 실측이며, 시점·incident 귀속 patch 플랜은 포함하지 않는다.
...
- [x] (Must, FR-08) 출력 경로 `specs/30.spec/green/common/test-idioms-spec.md` (가) 채택. 채택 사유는 §변경 이력 박제 (테스트 인프라 축이 컴포넌트 스코프와 응집도 구분 우선).
