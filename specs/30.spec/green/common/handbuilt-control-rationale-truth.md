...
> **최종 업데이트**: 2026-08-31 (by inspector — REQ-20260831-049 흡수, HEAD=`643be56`)
...
**방어 대상 2 (REQ-20260831-049 — 게이트 자신의 사각)**: 위 채널이 인용 대상을 좁히지 못해 **형제 클래스에 가려 회귀를 놓치는** 상태. 게이트는 주석이 클래스를 지목했을 때만 그 클래스와 대조하고, 지목이 없으면 그 파일이 렌더하는 클래스 **중 최소 1개**가 인용 값을 선언하는지로 폴백한다. `some` 은 OR 이므로 대상 집합이 넓어질수록 판정이 약해진다. 현 HEAD 에 그 상태가 실재한다 — `src/Image/ImageItem.tsx` 의 인용은 클래스를 지목하지 않고, 그 파일이 렌더하는 `.img--image-selected` 와 `.div--image-backdrop` 이 둘 다 `position: fixed` 다. 둘 중 하나만 회귀시키면 게이트는 rc=0 을 낸다 (REQ-049 §배경 MUT-A 실측). 더 나쁜 갈래는 **지목이 깨졌을 때**다: 주석의 클래스명에 오타가 나면 지목이 조용히 빈 집합이 되어 같은 폴백으로 강등되고, 같은 CSS 회귀에 대해 지목이 살아 있는 파일만 붉어진다 (MUT-D/MUT-E 실측). 클래스 rename 이 실제로 일어나는 방식 — 주석은 두고 CSS 만 고치는 — 이 이 경로와 정확히 겹친다. 즉 이 축의 방어 대상은 "거짓이 된 전제" 가 아니라 **그 전제를 잡으라고 세운 게이트의 사각**이며, `RULE-06 §게이트 실효 검증` 이 말하는 "주입 왕복을 `N/N detect` 로 통과한 검출력 0 게이트" 의 실물이다 — `b34c391` 의 주입은 전원 이탈(MUT-B)형만 돌았다.
...
- [ ] (I2-a) 지목 없는 인용 0 건 — HEAD=`643be56` 실측 **rc=1 (FAIL)**, 출력 `UNSCOPED src/Image/ImageItem.tsx` / `files=4 quotes=3 unscoped=1`. 모집단 도출 + 공허 가드 내장 (`files=0` 또는 `quotes=0` 이면 `exit 2`):
  ```
  ...
  ```
- [ ] (I6 투영) 폴백 삼항 부재: `bash -c '! grep -nE "q\.scoped : q\.rendered" src/__tests__/handbuilt-rationale-premise-truth.test.ts'` → HEAD=`643be56` 실측 **rc=1 (FAIL)** — `:203` 1 hit. **투영이지 불변식이 아니다** (§참고 §주입 이관 Dir-1 이 실검증).
- [ ] (I6 투영) 지목 클래스의 조용한 탈락 경로 부재: `bash -c '! grep -nE "filter\(\(c\) => rendered\.includes\(c\)\)" src/__tests__/handbuilt-rationale-premise-truth.test.ts'` → HEAD=`643be56` 실측 **rc=1 (FAIL)** — `:152` 1 hit. 투영 — 실검증은 Dir-3.
- [x] (I5, REQ-049 FR-04 보존) `ImageItem` 의 손조립은 정정 후에도 보존: `bash -c 'test "$(grep -cE "^[[:space:]]*role=\"button\"" src/Image/ImageItem.tsx)" = "1"'` → HEAD=`643be56` 실측 rc=0. 이 지점의 근거는 **참이다** — 요구되는 것은 지목 추가지 근거·손조립의 제거가 아니다.
...
- [ ] (Must, REQ-049 FR-01) 지목 없는 인용이 0 건: 위 §테스트 현황 (I2-a) 명령 → rc=0. HEAD=`643be56` 실측 rc=1 (`src/Image/ImageItem.tsx` 1건).
- [ ] (Must, REQ-049 FR-02 투영) 폴백 삼항 부재: 위 (I6 투영) 첫 명령 → rc=0. HEAD=`643be56` 실측 rc=1.
- [ ] (Must, REQ-049 FR-03 투영) 지목 미렌더의 조용한 강등 경로 부재: 위 (I6 투영) 둘째 명령 → rc=0. HEAD=`643be56` 실측 rc=1.
- [x] (Must, REQ-049 FR-04 보존) `ImageItem` 손조립 1 hit 유지: 위 (I5) 명령 rc=0.
- [x] (Must, REQ-049 NFR-01 비퇴행) 게이트가 계속 초록: `bash -c 'npx vitest run src/__tests__/handbuilt-rationale-premise-truth.test.ts >/dev/null 2>&1'` → HEAD=`643be56` 실측 rc=0 (1 file / 3 tests). 정정·게이트 수정 후에도 rc=0.
...
- **grep-baseline** (HEAD=`643be56`, 2026-08-31 재실측):
  ...
  - **(REQ-049) 인용의 지목 현황** — 위 (I2-a) perl 도출을 그대로 실행한 결과 (HEAD=`643be56`): `files=4 quotes=3 unscoped=1`, 유일 위반 `src/Image/ImageItem.tsx`. 지목이 있는 2건은 둘 다 `` `.div--logitem-linkmessage` `` 를 지목한다 (`src/Log/LogItemInfo.tsx` · `src/Comment/CommentItem.tsx`). `src/common/UserLogin.tsx` 는 `position:` 인용이 없어 모집단 4 중 인용 3 이다.
  - **(REQ-049) 게이트의 폴백 지점** — `grep -nE "q\.scoped : q\.rendered|filter\(\(c\) => rendered\.includes\(c\)\)" src/__tests__/handbuilt-rationale-premise-truth.test.ts` → 2 hits: `:152` (지목의 조용한 탈락) · `:203` (렌더 집합 폴백). 이 2 라인이 (I6) 의 정정 지점이다.
...
`RULE-07 §수용 기준 문장 규약` 의 '가정 주입 요구' 부류라 체크박스로 두지 않고 이관한다. 이관처 task 가 발행되지 않으면 이 절이 곧 미이관 상태의 박제다. 검출 방향 4 · 음성 대조 2 — REQ-20260831-049 이 저장소 밖 HEAD 사본에서 전건 실측했다.

- **Dir-1 (민감도, I6 지목 부재)** — `.img--image-selected` 만 `fixed` → `absolute` (형제 `.div--image-backdrop` 은 `fixed` 유지) → `rc≠0`. **현 게이트 실측 `rc=0` (놓침)** — 이 방향이 이번 수정의 존재 이유다.
- **Dir-2 (민감도, I2-a)** — 주석에서 클래스 지목만 삭제 (인용 값 유지) → `rc≠0` (fail-closed).
- **Dir-3 (민감도, I6 지목 미렌더)** — 지목 클래스명을 렌더되지 않는 이름으로 바꾼다 (오타·rename 모사) → `rc≠0`. **현 게이트 실측 `rc=0` (무발화)**. 같은 CSS 회귀에서 지목이 살아 있는 파일만 붉어지는 비대칭이 이 방향의 실물이다.
- **Dir-4 (민감도, 기존 축 보존)** — 지목 클래스의 `position` 값을 실제로 회귀 → `rc≠0` (현 게이트도 검출). 실패 메시지는 대조한 클래스를 출력해야 한다 (I8).
- **Ctrl-1 (특이도)** — 지목을 유지한 채 주석 문구만 재서술 (범위 밖 축 = 문체·분량) → `rc=0`.
- **Ctrl-2 (특이도)** — 지목 클래스의 `position` **이외** 속성 변경 (예: `z-index`) → `rc=0`. 좌표·형태는 `hover-popup-anchoring` 소관이라 본 게이트의 범위 밖이다.
...
- **(I8) 실패 메시지의 구별 발화와 (I9) 경계 주석의 사실성은 고장을 내야 판정된다.** (I8) 은 위반을 주입해야 메시지가 보이고, (I9) 는 폴백 제거 이후에야 현 경계 문장의 참·거짓이 갈린다. 둘 다 §주입 이관(Dir-2·Dir-3 의 출력 확인)과 구현 리뷰로 판정한다.
- **(I6) 의 두 수용 기준은 게이트 소스 문자열에 대한 투영이다.** `q.scoped : q.rendered` · `filter((c) => rendered.includes(c))` 0 hit 은 현 구현의 폴백 지점이 사라졌음을 재지, 새 구현이 fail-closed 임을 재지 않는다. 리팩터로 같은 폴백이 다른 표기로 되살아나면 두 명령은 통과한다. **민감도의 판정은 §주입 이관 Dir-1~3 이 전담**하며, 두 투영은 그때까지의 회귀 감시용이다.
...
- **관련 커밋**: `7cd575a` (3곳 손조립 유지 근거 박제 — "재론되지 않도록"), `520f906` (팝업 명시 좌표 + 기준점 + 게이트 — 전제를 거짓으로 만든 커밋), `8772f0d` (거짓 전제 정정 — TSK-20260831-01-a), `b34c391` (발화 채널 부착 — TSK-20260831-01-b · 폴백 도입 지점이기도 하다: `:152` `:203`).
