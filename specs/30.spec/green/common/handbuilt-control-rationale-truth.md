...
**방어 대상 (silent regression)**: 근거의 전제만 조용히 거짓이 되고 결론은 남는 상태. `7cd575a` 는 이 주석들의 목적을 "재론되지 않도록" 이라고 적었다 — 즉 이 문장들은 미래의 판단을 **선점**한다. 선점하는 문장이 거짓이면, 다음에 이 코드를 보는 사람은 **이미 사라진 위험**을 근거로 전환을 다시 포기한다. 이 축을 보는 자동 게이트는 하나도 없다: `src/common/a11y.audit.test.ts` 는 손조립이 패턴 B(`tabIndex` + `onKeyDown`)를 갖췄는지만 보고, `src/__tests__/hover-popup-anchoring.test.ts` 는 CSS 계약의 **형태**만 본다. 둘 다 주석이 무엇을 주장하는지 읽지 않으므로, 전제 소멸은 어떤 rc 도 바꾸지 않은 채 통과한다.
...
### 회귀 중점
- 팝업 CSS 를 다시 `position: fixed` 로 되돌리거나 좌표를 걷으면 (I2) 의 기준 사실이 바뀐다 — 주석이 아니라 CSS 가 움직인 경우다. 이때도 두 쪽이 어긋나면 위반이며, `hover-popup-anchoring` 게이트가 CSS 쪽을 먼저 잡는다.
...
- 직교: `src/common/a11y.audit.test.ts` (패턴 B 충족 여부만), `src/__tests__/hover-popup-anchoring.test.ts` (CSS 계약 형태만). 둘 다 본 축을 보지 않는다 — 그 사실이 §역할 의 방어 대상 근거다.

## 테스트 현황
- [ ] (I2) 거짓 전제 소멸: `bash -c '! grep -nF "position: fixed" src/Log/LogItemInfo.tsx src/Comment/CommentItem.tsx'` → 현 HEAD=`efe6268` 실측 **rc=1 (FAIL)**. 2 hit: `src/Log/LogItemInfo.tsx:75` · `src/Comment/CommentItem.tsx:85`. 두 주석 모두 팝업을 `position: fixed` + 좌표 부재로 서술하나 실제 CSS 는 `absolute` + 명시 좌표다 (`520f906` 이후).
- [x] (I2) 기준 사실 실재: `bash -c 'grep -A4 -nE "^\.div--logitem-linkmessage" src/Log/Log.css | grep -qE "position:[[:space:]]*absolute"'` → HEAD=`efe6268` 실측 rc=0.
- [x] (I5) 범위 밖 2 지점 보존: `bash -c 'grep -qE "^[[:space:]]*role=\"button\"" src/Image/ImageItem.tsx && grep -qE "^[[:space:]]*role=\"button\"" src/common/UserLogin.tsx'` → HEAD=`efe6268` 실측 rc=0.
...
## 수용 기준
- [ ] (Must, FR-05) 거짓 전제 소멸: `bash -c '! grep -nF "position: fixed" src/Log/LogItemInfo.tsx src/Comment/CommentItem.tsx'` → rc=0.
...
- **grep-baseline** (HEAD=`efe6268`, 2026-08-31 실측):
  - `grep -nF "position: fixed" src/Log/LogItemInfo.tsx src/Comment/CommentItem.tsx` → 2 hits in 2 files:
    - `src/Log/LogItemInfo.tsx:75`
    - `src/Comment/CommentItem.tsx:85`
  - `grep -rnE "^[[:space:]]*role=\"button\"" src --include="*.tsx" | grep -v "\.test\."` → 4 hits in 4 files:
    - `src/Comment/CommentItem.tsx:90`, `src/Image/ImageItem.tsx:74`, `src/common/UserLogin.tsx:68`, `src/Log/LogItemInfo.tsx:82`
- **rationale**: `position: fixed` 문자열은 이 두 파일에서 오직 거짓이 된 전제 문장에만 나타난다 (실측 2/2). 따라서 0 hit 은 (I2) 의 **투영**으로 충분하다 — 불변식 자체는 아니다 (§참고).
...
- **(I1) "손조립 지점은 고유한 근거 주석을 갖는다" 의 전수 판정 채널이 없다.** 주석과 JSX 속성의 결합은 위치 근접성으로만 표현돼 있어 grep 으로 "이 `role="button"` 에 딸린 주석" 을 지목할 수 없다. 현 4 지점은 육안 확인으로 전부 근거를 갖는다 (HEAD=`efe6268`): `LogItemInfo.tsx:73-79` · `CommentItem.tsx:83-86` · `ImageItem.tsx:26-29` · `UserLogin.tsx:59-66`.
- **(I3) "전제가 거짓이 되면 결론도 정정된다" 는 의미 판정이라 명령으로 잴 수 없다.** 게이트(`grep -F "position: fixed"` 0 hit)는 **전제 문자열의 소멸**만 본다. 전제만 지우고 "전환하지 않는다" 만 남기면 게이트는 rc=0 을 내지만 (I3) 위반이다. 이 방향은 본 spec 의 정정 task 리뷰에서 사람이 판정한다.
...
- **관련 커밋**: `7cd575a` (3곳 손조립 유지 근거 박제 — "재론되지 않도록"), `520f906` (팝업 명시 좌표 + 기준점 + 게이트 — 전제를 거짓으로 만든 커밋).
