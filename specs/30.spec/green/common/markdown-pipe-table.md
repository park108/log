...
- [x] (I2 정적 zero-point) sanitize 허용 태그에 표 태그가 등재돼 있다: `bash -c 'f=src/common/sanitizeHtml.ts; grep -q "ALLOWED_TAGS" "$f" || exit 2; n=$(grep -oE "'\''(table|thead|tbody|tr|th|td)'\''" "$f" | wc -l | tr -d " "); echo "table-tag-entries=$n"; [ "$n" -ge 6 ]'` → HEAD=`25f6013` 재실측 **rc=0**, 출력 `table-tag-entries=6`. `TSK-20260831-17`(`d98815d`) 착지. 파일에 `ALLOWED_TAGS` 가 없으면 `exit 2` 로 무판정 처리한다 (공허 통과 차단).
