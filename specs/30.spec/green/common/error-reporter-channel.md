...
> **최종 업데이트**: 2026-04-21 (by inspector, 신설 — REQ-20260421-039 흡수)
...
| # | 경로 | 지점 | 라인 | 도메인 | 비고 |
|---|------|------|------|--------|------|
| D1 | `src/Search/Search.jsx` | List API errorType 분기 실패 else + catch | `:43, :53` (2) | Search | 2 hits |
| D2 | `src/Image/ImageSelector.jsx` | Images first fetch errorType 분기 + catch + Images next fetch errorType 분기 + catch | `:47, :53, :90, :96` (4) | Image | 4 hits (2×2 구조) |
...
**합계**: 24 hits / 8 files (2+4+4+2+4+4+4 = 24; Search 1 + Image 1 + File 4 + Comment 1 = 7 파일 + Image/File 내부 중복 없음 — 실제 파일 수 7 이나 File 내부 4 파일로 집계 시 8 파일 계산 정합).
...
- **grep-baseline** (inspector 발행 시점, HEAD=bdc3964 실측):
  ...
  - `grep -rnE "^\s+console\.(error|warn|info|log)\(" src --include="*.jsx" --include="*.js" | grep -v ".test." | grep -v "src/common/errorReporter.js" | grep -v "src/common/common.js"` → **24 hits / 8 files** (HEAD=bdc3964; 목표 0).
    - `src/Search/Search.jsx:43, 53` (2)
    - `src/Image/ImageSelector.jsx:47, 53, 90, 96` (4)
  ...
  - `grep -rnE "reportError\(" src --include="*.jsx" --include="*.js" | grep -v ".test."` → **19 hits** (HEAD=bdc3964; 목표 ≥ 19 + 24 = 43 hits; 실측 정확 수치는 정상화 task 결과에서 확정 — 각 `console.error(...)` 를 `reportError(...)` 로 1:1 치환 시 단순 합).
    - `src/common/errorReporter.js:7, 10, 11` (3; 정의 + JSDoc 포함).
    - `src/App.jsx:9, 99, 107, 115` (4).
    - `src/Monitor/ApiCallItem.jsx:6, 92, 98` (3).
    - `src/Monitor/ContentItem.jsx:5, 96, 102` (3).
    - `src/Monitor/VisitorMon.jsx:6, 152, 158` (3).
    - `src/Monitor/WebVitalsItem.jsx:5, 100, 106` (3).
...
- **rationale**: gate (a) 는 본 불변식의 negative **목표값** (0 hit 수렴). gate (b) 는 positive **목표값** (현 19 + 정상화 24 = 43 hit 근접). gate (c) 는 §허용 예외 2 항목 enumeration 의 현장 근거. 정상화 완료 후 gate (a) = 0 / gate (b) ≈ 43 / gate (c) = α 1 + β 유지.
...
- [ ] (Must, REQ-20260421-039 FR-03) `Search/Search.test.jsx` — D1 정상화 후 reportError 이디엄 케이스 추가. 향후 task.
- [ ] (Must, REQ-20260421-039 FR-03) `Image/ImageSelector.test.jsx` — D2 정상화 후 reportError 이디엄 2 케이스 (first fetch + next fetch) 추가. 향후 task.
