...
> **최종 업데이트**: 2026-04-21 (by inspector, Phase 1 reconcile 2/2 ack — TSK-82 D3 File.jsx @337d694 + TSK-83 D4 FileItem.jsx @690aa74)
...
**합계**: 24 hits / 8 files (2+4+4+2+4+4+4 = 24; Search 1 + Image 1 + File 4 + Comment 1 = 7 파일 + Image/File 내부 중복 없음 — 실제 파일 수 7 이나 File 내부 4 파일로 집계 시 8 파일 계산 정합). **잔존** @HEAD=690aa74: **12 hits / 3 files** (D1/D2/D3/D4 정상화 완료 후; D5 FileDrop.jsx 4 + D6 FileUpload.jsx 4 + D7 Comment.jsx 4).
...
- **grep-baseline** (inspector 발행 시점, HEAD=bdc3964 실측):
  ...
  - `grep -rnE "^\s+console\.(error|warn|info|log)\(" src --include="*.jsx" --include="*.js" | grep -v ".test." | grep -v "src/common/errorReporter.js" | grep -v "src/common/common.js"` → 발행 시점 **24 hits / 8 files** (HEAD=bdc3964; 목표 0). **HEAD=690aa74 잔존 — 12 hits / 3 files** (D1/D2/D3/D4 정상화 완료 후):
  ...
  - `grep -rnE "reportError\(" src --include="*.jsx" --include="*.js" | grep -v ".test."` → 발행 시점 **19 hits** (HEAD=bdc3964 — 실측 이후 재집계 정정: 호출식 한정 시 9 hits; `App.jsx:9,99,107,115` 는 `import` / JSX prop reference 로 호출식 `reportError(` 패턴 비매치). **HEAD=690aa74 실측 — 21 hits** (호출식 한정; 정상화 진행 중 각 domain 치환마다 +2 또는 +4 증가):
    ...
    - `src/File/File.jsx:65, 72, 108, 115` (4) — **신규 TSK-82 @337d694**.
    - `src/File/FileItem.jsx:36, 44` (2) — **신규 TSK-83 @690aa74**.
...
- **rationale**: gate (a) 는 본 불변식의 negative **목표값** (0 hit 수렴). gate (b) 는 positive **목표값** (현 21 + 정상화 잔존 12 = 33 hit 근접; 정정 집계 — 호출식 한정). gate (c) 는 §허용 예외 2 항목 enumeration 의 현장 근거. 정상화 완료 후 gate (a) = 0 / gate (b) ≈ 33 / gate (c) = α 1 + β 유지.
