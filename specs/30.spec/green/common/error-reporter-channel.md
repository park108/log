...
> **최종 업데이트**: 2026-04-21 (by inspector, Phase 1 reconcile 2/2 ack — TSK-84 D5 FileDrop.jsx @602a7bc + TSK-85 D6 FileUpload.jsx @c5eed40)
...
**합계**: 24 hits / 8 files (2+4+4+2+4+4+4 = 24; Search 1 + Image 1 + File 4 + Comment 1 = 7 파일 + Image/File 내부 중복 없음 — 실제 파일 수 7 이나 File 내부 4 파일로 집계 시 8 파일 계산 정합). **잔존** @HEAD=c5eed40: **4 hits / 1 file** (D1/D2/D3/D4/D5/D6 정상화 완료 후; D7 Comment.jsx 4).
...
- **grep-baseline** (inspector 발행 시점, HEAD=bdc3964 실측):
  ...
  - `grep -rnE "^\s+console\.(error|warn|info|log)\(" src --include="*.jsx" --include="*.js" | grep -v ".test." | grep -v "src/common/errorReporter.js" | grep -v "src/common/common.js"` → 발행 시점 **24 hits / 8 files** (HEAD=bdc3964; 목표 0). **HEAD=c5eed40 잔존 — 4 hits / 1 file** (D1/D2/D3/D4/D5/D6 정상화 완료 후):
  ...
  - `grep -rnE "reportError\(" src --include="*.jsx" --include="*.js" | grep -v ".test."` → 발행 시점 **19 hits** (HEAD=bdc3964 — 실측 이후 재집계 정정: 호출식 한정 시 9 hits; `App.jsx:9,99,107,115` 는 `import` / JSX prop reference 로 호출식 `reportError(` 패턴 비매치). **HEAD=c5eed40 실측 — 29 hits** (호출식 한정; 정상화 진행 중 각 domain 치환마다 +2 또는 +4 증가):
    ...
    - `src/File/FileDrop.jsx:41, 47, 62, 68` (4) — **신규 TSK-84 @602a7bc**.
    - `src/File/FileUpload.jsx:45, 51, 66, 72` (4) — **신규 TSK-85 @c5eed40**.
...
- **rationale**: gate (a) 는 본 불변식의 negative **목표값** (0 hit 수렴). gate (b) 는 positive **목표값** (현 29 + 정상화 잔존 4 = 33 hit 근접; 정정 집계 — 호출식 한정). gate (c) 는 §허용 예외 2 항목 enumeration 의 현장 근거. 정상화 완료 후 gate (a) = 0 / gate (b) ≈ 33 / gate (c) = α 1 + β 유지. D1~D6 정상화 완료 (TSK-80~85), 잔존 D7 Comment 1 파일 (4 hits).
