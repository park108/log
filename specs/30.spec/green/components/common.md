...
> **관련 요구사항**: REQ-20260421-022, REQ-20260421-025, REQ-20260421-032, REQ-20260421-038, REQ-20260422-045, REQ-20260517-074
> **최종 업데이트**: 2026-05-17 (by inspector, REQ-20260517-074 흡수 — §에러 처리 ErrorFallback 분기 enum + §UI 셸 Skeleton variant enum + §스코프 규칙 reportError 단일 채널 grep 게이트 박제)
...
- `ErrorFallback.tsx` — 기본 폴백 UI. CSS `ErrorFallback.css`. 자세한 분기 enum / 메시지 / 접근성 속성은 § ErrorFallback 분기 계약 (REQ-20260517-074) 참조.
- `errorReporter.ts` — `reportError(error, errorInfo)` 를 `console.error` 단일 채널로 위임 (추후 Sentry 연동 spec 분리). 자세한 단일 채널 grep 게이트는 § 스코프 규칙 grep-baseline (d-1/d-2) 참조.
- **호출측 경유 계약 (REQ-20260422-045 FR-03, 선행 REQ-20260421-039 FR-06)**: 런타임 도메인 에러 지점 (`Search`, `ImageSelector`, `File`, `FileItem`, `FileDrop`, `FileUpload`, `Comment`) 은 `console.error` 직접 호출 대신 `reportError` 경유 — 예: `src/Comment/Comment.jsx`. 단일 채널 계약 세부는 선행 done REQ-20260421-039 + § 스코프 규칙 grep-baseline (d-1) 참조.

### ErrorFallback 분기 계약 (REQ-20260517-074)
- **분기 함수 불변식 (FR-01)**: `ErrorFallback` 은 props `error?: Error | null` 을 받아 다음 두 path 의 OR 로 Network 분기를 판정한다 — (a) `error.name === 'NetworkError'`, (b) `error.message` 가 `/failed to fetch|network/i` 정규식과 매치. 두 path 중 어느 하나라도 true 면 Network 분기, 둘 다 false 면 Render error 분기. `error` 가 `null` / `undefined` 이면 Render error 분기 (Network 분기 false 귀결). 분기 함수는 internal helper (`isNetworkError`) 로 캡슐화하여 두 path 의 OR 의미를 단일 식별자로 박제한다.
- **분기 메시지 enum 불변식 (FR-02)**: 분기별 사용자 가시 메시지는 다음 2종 텍스트로 박제된다 — Network: `'연결을 확인하고 다시 시도하세요'`, Render error (그 외 default): `'예기치 않은 오류가 발생했습니다'`. 메시지 enum 변경은 시스템 계약 변경 — spec 갱신 동반 PR 만 진입 가능. i18n / 다국어화는 Out-of-Scope (별 spec).
- **role="alert" 알림 채널 불변식 (FR-03)**: ErrorFallback wrapper `<div>` 는 `role="alert"` 속성을 가진다. WCAG 2.1 §4.1.3 Status Messages 정합 — 보조 기술이 에러 발생을 즉시 알림 채널로 인지. 본 속성 제거 / 변경은 접근성 회귀.
- **reset 조건부 렌더 + 라벨 (FR-04 Should)**: ErrorFallback 은 props `reset?: () => void` 을 받아 `typeof reset === 'function'` 인 경우에만 reset 버튼을 렌더하고, 그 외 (undefined / null / 비-함수) 에는 버튼을 표시하지 않는다. 버튼 라벨은 `'다시 시도'` 텍스트 + `type="button"` 속성으로 박제. reset 호출 후의 상태 회복 책임은 호출측 (보통 ErrorBoundary 부모) 에게 위임.

### Skeleton variant 계약 (REQ-20260517-074)
- **variant enum 불변식 (FR-05)**: `Skeleton` 컴포넌트는 props `variant?: 'page' | 'list' | 'detail'` 을 받으며, 허용 값 enum 은 `VALID_VARIANTS = ['page', 'list', 'detail'] as const` 3종으로 박제된다. variant 미지정 시 기본값은 `'page'`. 허용 enum 외 값 (예: `'banner'`, 임의 문자열, undefined) 이 입력되면 안전 fallback 으로 `'page'` 로 수렴한다 — `VALID_VARIANTS.includes` 가드. enum 확장 (4번째 값 추가) 은 시스템 계약 변경 — spec 갱신 동반 PR 만 진입 가능.
- **role="status" + aria-label 알림 채널 불변식 (FR-06 Should)**: Skeleton wrapper `<div>` 는 `role="status"` + `aria-label="로딩 중"` + `className="skeleton skeleton--{variant}"` + `data-testid="skeleton-{variant}"` 속성을 가진다. WCAG 2.1 §4.1.3 정합 — 보조 기술이 로딩 상태를 비-침투 알림 채널로 인지. variant 별 block count 시각 디테일은 본 spec 박제 가치 낮음 — Out-of-Scope.
...
- `Footer.jsx` — 하단 푸터.
- `Skeleton.tsx` — `variant` 별 로딩 스켈레톤. `Skeleton.css`. 자세한 variant enum / 안전 fallback / a11y 속성은 § Skeleton variant 계약 (REQ-20260517-074) 참조.
...
- **grep-baseline** (inspector 발행 시점, HEAD=29d9da0 실측):
  ...
  (REQ-20260517-074 FR-07 / FR-02 / FR-05 / FR-08)
  - (d-1) **[REQ-074 FR-07 단일 채널 grep]** `grep -nE "console\.error" src --include="*.ts" --include="*.tsx" --include="*.js" --include="*.jsx" -r | grep -v "\.test\." | grep -v "errorReporter\."` → **목표 0 lines**. 현 시점 (HEAD=`9e5f00a`): **0 lines** — 단일 채널 계약 PASS. `errorReporter.ts` 정의 (`console.error('[reportError]', error, errorInfo)`) 는 본 게이트 범위 밖 (제외 규칙 박제). `*.test.*` 테스트 파일도 범위 밖.
  - (d-2) **[REQ-074 FR-08 reportError 호출 광범위성]** `grep -rnE "reportError" src --include="*.ts" --include="*.tsx" --include="*.js" --include="*.jsx" | grep -v "\.test\."` → **목표 1+ hit (광범위 사용)**. 현 시점 (HEAD=`9e5f00a`): **50 hits** — `App.jsx`, `Monitor/*`, `File/*`, `Comment/*`, `Search/*`, `Image/*`, `errorReporter.ts` 정의 등 도메인 횡단 단일 채널 사용 증거.
  - (e-1) **[REQ-074 FR-02 분기 메시지 enum]** `grep -n "연결을 확인하고 다시 시도하세요\|예기치 않은 오류가 발생했습니다" src/common/ErrorFallback.tsx` → **목표 2 hits** (Network / Render error 각 1행). 현 시점 (HEAD=`9e5f00a`): **2 hits** @`:31,32`.
  - (e-2) **[REQ-074 FR-05 variant enum]** `grep -nE "VALID_VARIANTS|'page'\s*\|\s*'list'\s*\|\s*'detail'" src/common/Skeleton.tsx` → **목표 1+ hit**. 현 시점 (HEAD=`9e5f00a`): **3 hits** (`type SkeletonVariant` 정의 + `VALID_VARIANTS` 상수 + 가드).
  - (e-3) **[REQ-074 FR-03/06 a11y 속성 박제]** `grep -nE "role=\"alert\"" src/common/ErrorFallback.tsx` → 1 hit + `grep -nE "role=\"status\"|aria-label=\"로딩 중\"" src/common/Skeleton.tsx` → 2 hits.

- **rationale**: REQ-022 gate (a) 는 문자열 NODE_ENV 분기 제거 완료 상태 확인 (0 hit = OK). gate (b)(c) 는 env 외부화·setCookie 속성 정의 존재 확인 (1+ hit = OK). REQ-025 gate (a) 는 현실이 계약 미준수 (0 hit) 이나 spec 불변식은 유지 — 향후 task carve 대상. gate (b) 는 비표준값 0 hit = OK. REQ-074 gate (d-1) 은 호출측 `console.error` 직접 호출이 회귀로 재진입하지 않음을 PR 단계 자동 검출 (반복 검증 가능). gate (d-2) 는 단일 채널이 실제로 도메인 횡단 광범위 사용됨을 박제 — 본 수치가 0 으로 떨어지면 모든 도메인이 `console.error` 로 회귀했다는 별도 신호. gate (e-1/e-2/e-3) 는 분기 메시지 / variant / a11y 속성이 PR 에서 무단 변경되어도 spec 게이트 부재 회피.
...
- [x] (Must, REQ-20260517-074 FR-01/02/03) `ErrorFallback.test.tsx` — Network 분기 2 path (`name === 'NetworkError'` + `/failed to fetch/i`) + default 분기 메시지 검증 (`:6-31`) + `role="alert"` 검증 (`:50-54`). HEAD=`9e5f00a` PASS.
- [x] (Should, REQ-20260517-074 FR-04) `ErrorFallback.test.tsx:33-48` — reset 조건부 렌더 검증 (`typeof reset === 'function'` 일 때만 표시).
- [x] (Must, REQ-20260517-074 FR-05/06) `Skeleton.test.tsx:5-31` — variant enum 3종 + invalid value → `'page'` fallback 검증 + a11y 속성 (`role="status"`, `aria-label="로딩 중"`) 검증. HEAD=`9e5f00a` PASS.
- [x] (Must, REQ-20260517-074 FR-07) `console.error` 직접 호출 0 hit 게이트 — § 스코프 규칙 grep-baseline (d-1) HEAD=`9e5f00a` 실측 0 lines PASS.
...
- [x] (Must, REQ-20260517-074 FR-01) § ErrorFallback 분기 계약 의 분기 함수 불변식 박제 (`name === 'NetworkError'` OR `/failed to fetch|network/i` 두 path OR).
- [x] (Must, REQ-20260517-074 FR-02) § ErrorFallback 분기 계약 의 분기 메시지 enum 2종 박제 (Network `'연결을 확인하고 다시 시도하세요'` / Render error `'예기치 않은 오류가 발생했습니다'`).
- [x] (Must, REQ-20260517-074 FR-03) § ErrorFallback 분기 계약 의 `role="alert"` 알림 채널 불변식 박제 (WCAG 2.1 §4.1.3 정합).
- [x] (Should, REQ-20260517-074 FR-04) § ErrorFallback 분기 계약 의 reset 조건부 렌더 (`typeof reset === 'function'`) + 라벨 `'다시 시도'` 박제.
- [x] (Must, REQ-20260517-074 FR-05) § Skeleton variant 계약 의 variant enum 3종 (`'page' | 'list' | 'detail'`) + 기본값 `'page'` + invalid value fallback `'page'` 박제.
- [x] (Should, REQ-20260517-074 FR-06) § Skeleton variant 계약 의 a11y 속성 (`role="status"`, `aria-label="로딩 중"`) 박제.
- [x] (Must, REQ-20260517-074 FR-07) § 스코프 규칙 grep-baseline (d-1) 에 `reportError` 단일 채널 grep 게이트 박제 — `console.error` 직접 호출 0 hit (errorReporter.ts + `*.test.*` 제외 규칙 명문화). HEAD=`9e5f00a` 실측 0 lines PASS.
- [x] (Should, REQ-20260517-074 FR-08) § 스코프 규칙 grep-baseline (d-2) 에 `reportError` 호출 광범위성 baseline 박제 — 50 hits in src non-test (`App.jsx` / `Monitor/*` / `File/*` / `Comment/*` / `Search/*` / `Image/*`). 회귀 시 0 으로 떨어지면 별도 탐지.
- [x] (NFR, REQ-20260517-074 NFR-01) ErrorFallback `role="alert"` + Skeleton `role="status"` + `aria-label="로딩 중"` 가 PR review 시 평가 기준으로 spec 박제 — WCAG 2.1 §4.1.3 Status Messages 정합.
...
| 2026-05-17 | inspector / REQ-20260517-074 (HEAD=`9e5f00a`) | **REQ-074 흡수** — blue `components/common.md` → green carry-over 후 § 에러 처리 절 확장 + § ErrorFallback 분기 계약 (FR-01/02/03/04) + § Skeleton variant 계약 (FR-05/06) 2 서브섹션 신설 + § UI 셸 Skeleton 항 1행 cross-ref 갱신 + § 스코프 규칙 grep-baseline (d-1/d-2 reportError 단일 채널 + 광범위성) (e-1/e-2/e-3 분기 메시지·variant·a11y 박제) 5 gate 추가 + § 테스트 현황 5 marker + § 수용 기준 9 marker (FR-01~08 + NFR-01) 추가. 박제 baseline: (a) ErrorFallback 분기 함수 `isNetworkError` `error.name === 'NetworkError'` OR `/failed to fetch\|network/i` 두 path OR (HEAD `:21-26`), (b) 분기 메시지 enum 2종 — Network `'연결을 확인하고 다시 시도하세요'` + Render error `'예기치 않은 오류가 발생했습니다'` (HEAD `:30-32`), (c) wrapper `role="alert"` (HEAD `:35`), (d) reset 조건부 렌더 (`typeof reset === 'function'`) + 라벨 `'다시 시도'` (HEAD `:38-46`), (e) Skeleton variant enum `'page' \| 'list' \| 'detail'` + `VALID_VARIANTS` 상수 + invalid fallback `'page'` (HEAD `:11,17,25-29`), (f) wrapper `role="status"` + `aria-label="로딩 중"` (HEAD `:38-43`), (g) `console.error` 직접 호출 0 lines (errorReporter.ts + `*.test.*` 제외) (HEAD 실측 0), (h) `reportError` 호출 50 hits (HEAD 실측). consumed followup referer (우선순위 1, error-boundary 3 hits): `specs/10.followups/20260517-0308-restore-deleted-specs.md`. consumed: REQ-20260517-074. RULE-07 자기검증: (1) 분기 enum / variant enum / 단일 채널 / a11y 속성 모두 "시스템이 무엇이어야 하는가" 평서문 — ✓. (2) grep / 테스트로 반복 검증 가능 — ✓. (3) 특정 incident·릴리스·날짜 비의존 — ✓ (관찰 baseline 수치는 §스코프 규칙 grep-baseline 박제 — RULE-07 정합). Out-of-Scope: Sentry 연동 (REQ-20260418-005 §13), variant 별 block count (시각 디테일), 분기 메시지 i18n, followup referer 표 2~7 순위 토픽 (env-spec / sanitizeHtml-spec / markdownParser-spec / accessibility-spec / css-modules-spec / search-abort-runtime-smoke-spec — 별 req 후보). 본 spec 은 green 영역 — planner promote 단계에서 blue/components/common.md 로 흡수 예정. | §최종 업데이트, §관련 요구사항, §에러 처리 (cross-ref 갱신), §ErrorFallback 분기 계약 (신설), §Skeleton variant 계약 (신설), §UI 셸 (1행 갱신), §스코프 규칙 grep-baseline (d-1/d-2/e-1/e-2/e-3 추가), §테스트 현황 (5 marker 추가), §수용 기준 (9 marker 추가), 본 이력 |
