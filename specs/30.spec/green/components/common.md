...
> **관련 요구사항**: REQ-20260421-022, REQ-20260421-025, REQ-20260421-032, REQ-20260421-038
> **최종 업데이트**: 2026-04-21 (by inspector, REQ-038 흡수 — isAdmin() Cognito User Pool Groups 기반 이관, env 경로 계약 교체)
...
- **grep-baseline** (inspector 발행 시점, HEAD=29d9da0 실측):
  ...
  - (b-1) **[REQ-038 FR-07 positive]** `grep -nE "cognito:groups" src/common/common.js` → **목표 1+ hit**. 현 시점 baseline (HEAD=0f03547): **0 hit** — migration 구현 task 미발행 상태. 목표값 수렴은 planner/developer task 영역.
  - (b-2) **[REQ-038 FR-07 negative]** `grep -nE "VITE_ADMIN_USER_ID" src/common/common.js` → **목표 0 hit**. 현 시점 baseline (HEAD=0f03547): **3 hits** (`:160` TODO 주석, `:176` prod 비교, `:180` dev 비교) — 교체 완료 후 0 hit 수렴.
...
- [ ] (Must, REQ-20260421-038 FR-08) `isAdmin()` `cognito:groups` 7 케이스 매트릭스 회귀 테스트 — `common.test.js` 의 기존 `describe('isAdmin matrix (REQ-20260421-017)')` (`:861`) 블록을 제거하고 `describe('isAdmin cognito:groups matrix (REQ-20260421-038 FR-01)')` 로 교체. 7 케이스 (쿠키 무 / parseJwt null / groups 필드 부재 / `[]` / `['user']` / `['admin']` / `['admin','editor']`) 박제. migration 구현 task 미발행 상태 — planner/developer 영역 실현 대기.
...
- [ ] (Must, REQ-20260421-038 FR-02/08) `isAdmin()` group claim 부재 안전 기본값 회귀 테스트 — `common.test.js` 의 기존 `describe('isAdmin() env 미주입 안전 기본값 (REQ-20260421-032 FR-08)')` (`:923`) 블록을 제거하고 `describe('isAdmin() group claim 부재 안전 기본값 (REQ-20260421-038 FR-02)')` 로 교체. 2 케이스 (groups 필드 null / groups 필드 비-배열) 박제. migration 구현 task 미발행 상태.
...
- [ ] (Must, REQ-20260421-038 FR-03) 런타임 소스 `src/**` (except test) 에서 `VITE_ADMIN_USER_ID_PROD|VITE_ADMIN_USER_ID_DEV` 0 hit + `.env.example` / `.env.*.local` / Amplify 콘솔 2 키 제거 + TODO 주석 제거. 현 baseline: common.js 3 hit + .env.example 4 hit 잔존 — planner/developer task 영역 실현 대기.
...
- [ ] (Must, REQ-20260421-038 FR-08) `src/common/common.test.js` 의 구 `isAdmin matrix` + `env 미주입` 2 describe 블록을 `isAdmin cognito:groups matrix (REQ-20260421-038 FR-01)` + `isAdmin() group claim 부재 안전 기본값 (REQ-20260421-038 FR-02)` 2 describe 로 교체. planner/developer 영역.
- [ ] (Must, REQ-20260421-038 FR-09) `.env.example:18-21` 4 행 제거. (β) 택 시 `VITE_ADMIN_GROUP_NAME=` 1 키 + 주석 1 행 추가. planner/developer 영역.
