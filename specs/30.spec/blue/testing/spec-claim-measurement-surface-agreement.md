...
- [x] 시그니처 고정 제거 — HEAD=`61ce5ab` (tick 227 재실행) **0 hit** (`grep -cF`) · `inFactory` 2 hit 잔존. (S-1) 충족 (TSK-20260826-40 / `05fc642`). 종전: `5b2ed3b..d7b08dd` 델타에 `src/__tests__/` 변경이 0건이라 tick 224·225 와 동일.
- [x] 산문 면역의 특이도 증인 — HEAD=`61ce5ab` (tick 227 재실행) `witness=1`. (S-2)(R-4) 충족 (TSK-20260826-40 / `05fc642`).
- [x] 도출 대조 — tick 223 실측 게이트 `predicates=13` = 진리원 도출 13. (S-3).

## 수용 기준

> 전 항목 명령 1회로 rc 판정 가능 (RULE-07 §수용 기준 문장 규약). 측정 명령은 `src/**` · `scripts/**` · `package.json` 만 참조한다 — **spec 파일 경로는 어느 것도 참조하지 않는다** (RULE-07 §promote 조건 2; green/blue 는 승격으로 경로가 바뀐다). **HEAD=`61ce5ab` (tick 227) 기준 4/4.**

- [x] (Must, S-1) 시그니처 고정 제거 + 불변식 보존 — `bash -c 'test "$(grep -cF "createQueryTestWrapper\s*=\s*\(\)" src/__tests__/react-query-test-queryclient-single-source.test.ts)" -eq 0 && test "$(grep -cE "inFactory" src/__tests__/react-query-test-queryclient-single-source.test.ts)" -ge 1'` → **rc=0**. **HEAD=`61ce5ab` (tick 227) 실측 rc=0 / 빈 괄호 리터럴 **0 hit** · `inFactory` 2 hit → 충족** (TSK-20260826-40 / `05fc642` — `\(\)` → `\([^)]*\)`). 교정 전 동일 트리 실측 rc=1 / 1 hit (`:85`). **`grep -F` 여야 한다** — 파일에 담긴 것은 정규식 **리터럴 텍스트**라 `-E` 로 해석하면 `\s*` 가 메타로 풀려 HEAD 에서도 **0 hit** 이 나오고 false negative 가 된다 (실측: `-F` → 1, `-E` → 0). 두 번째 조건은 (R-1) 대응 — 시그니처 고정만 지우고 팩토리 내부 생성 판정은 남긴다.
- [x] (Must, S-2) 산문 면역의 특이도 증인 — 게이트가 훑는 in-scope 테스트 파일 중 **주석에 금지 토큰을 담은 파일이 ≥1 실재**하고, 그 상태에서 게이트가 rc=0 이다. 판정:
  ```
  ...
  ```
  → **rc=0**. **HEAD=`61ce5ab` (tick 227) 실측 rc=0 / `witness=1` (`src/Log/hooks/log-detail-cache-key-identity.test.jsx`) → 충족** (TSK-20260826-40 / `05fc642`). 게이트가 파일 전문 정규식에서 **행 단위 + 행 선두 `//`·`*`·`/*` 제외**로 바뀌어 산문 면역을 얻었고, 증인은 `:33` 의 우회 표현을 자연스러운 원문으로 되돌린 결과다. **게이트 소스에서 제외 규칙을 grep 하지 않는 이유**는 §발화 채널 §관측 표면 주의 참조 — 죽은 코드가 통과한다. `src/__tests__/` 를 증인 후보에서 빼는 이유는 게이트 자신이 그 디렉터리를 스캔 대상에서 제외하기 때문이다(`:36` FR-09) — 거기 둔 주석은 증인이 되지 못한다 ((R-4)). 증인은 인위적 산물이 아니라 **자연스러운 서술의 복원**이다: `src/Log/hooks/log-detail-cache-key-identity.test.jsx` 의 제약 설명 주석이 토큰을 피해 우회 표현으로 쓰여 있다.
...
| 2026-08-26 | TSK-20260826-40 / `05fc642` (inspector tick 227) | **Phase 1 재실행 4/4 ack — 3 cycle stale 해소, 수용 기준 2/4 → 4/4.** (S-1) 빈 괄호 리터럴 `\(\)` → `\([^)]*\)` 로 좁혀져 `grep -cF` **0 hit**, `inFactory` 2 hit 잔존 → **초과 강제 제거 + 불변식 보존**이 동시에 성립. (S-2) 게이트가 파일 전문 정규식에서 **행 단위 판정 + 행 선두 `//`·`*`·`/*` 제외**(`COMMENT_LINE`)로 바뀌어 산문 면역을 얻었고, `src/Log/hooks/log-detail-cache-key-identity.test.jsx:33` 의 우회 표현이 자연 원문으로 복원돼 `witness=1`. 판정 명령은 **spec 파일에서 추출해 실행**했으며 result.md 주장을 받아쓰지 않았다. 특이도/민감도가 **같은 파일·같은 토큰·같은 위치에서 행 선두 주석 여부만으로** 갈리는 것을 게이트 소스(`hasDirectInstantiation`)로 확인했다. (S-3) `truth=13 gate=13` · (NFR-01) rc=0 유지 → 동반 조건 성립. **promote 후보.** | 수용 기준, 테스트 현황 |
