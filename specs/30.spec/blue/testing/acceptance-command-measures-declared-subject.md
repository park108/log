...
- 교정 대상 4건: slug `components/monitor` (N-5)(N-6) · slug `testing/derived-population-totality-judgement` (T-4) · slug `foundation/auth-entry-failure-user-observable-surface` (S-2).
...
- [x] 함정 부류 재발 채널 — HEAD=`61ce5ab` 착지. `scripts/check-acceptance-criteria.sh` G-3·G-4 + `ACC_SPEC_ROOT` seam + `DERIVE_MIN=100` + 종료 코드 3분할(0/1/2). (C-5) 충족.

## 수용 기준

> 전 항목 명령 1회로 rc 판정 가능 (`RULE-07 §수용 기준 문장 규약`). 명령은 `specs/30.spec` 트리를 **루트로** 훑으며 **자신을 포함한 어떤 spec 의 경로 리터럴도 참조하지 않는다** — 따라서 승격으로 경로가 바뀌어도 깨지지 않는다 (`§promote 조건 2`). 대상 spec 은 **slug 로 해석**하며 green 우선·없으면 blue 다. **HEAD=`61ce5ab` (tick 227) 기준 5/5.**
...
- [x] (Must, C-1) 판정 명령에 주소 범위 재시작 형태가 없다 — 판정:
  ...
  ```
  → **rc=0**. **HEAD=`61ce5ab` (tick 227 재실행) 실측 rc=0 / `judgement-commands=324 range-restart=0` → 충족** (tick 226 동일 판정 `306`). **동일 트리 교정 전 실측 rc=1 / `judgement-commands=294 range-restart=1`** (seam `ACC_SPEC_ROOT` 로 tick 226 이전 spec 판본을 주입) — 유일 hit 이 (N-5) 의 판정 명령이었다. **공허 통과 가드 내장** — 도출이 100 미만이면 `exit 2` 로, 추출기가 낡아 모집단이 비는 상태를 충족으로 읽지 않는다. **모집단이 산문이 아니라 명령으로 한정된다** (§동작 (P-5)) — 함정을 설명하는 문장은 계수되지 않는다.
- [x] (Must, C-2) 판정 명령에 실행 금지어가 없다 — 판정:
  ...
  ```
  → **rc=0**. **HEAD=`61ce5ab` (tick 227 재실행) 실측 rc=0 / `judgement-commands=324 unexecutable-verb=0` → 충족** (tick 226 동일 판정 `306`). **동일 트리 교정 전 실측 rc=1 / `unexecutable-verb=1`** — hit 은 slug `foundation/measurement-tree-attribution` (M-9) 의 판정 명령이었고, 그 명령은 `RULE-02 §금지` 어를 포함해 **inspector 도 planner 도 실행할 수 없었다.** 실행할 수 없는 명령은 `RULE-07 §promote 조건 2`("측정 명령을 실제로 실행한다")를 영구히 만족시킬 수 없으므로, 이 부류는 도달 불가와 **같은 결과**를 낳는다. **검출 토큰을 `printf` 분할로 조립하는 것이 요점** — 리터럴로 담으면 이 명령 자신이 유일 hit 이 되어 자기봉쇄가 된다 (§동작 (P-5)).
- [x] (Must, C-3) 교정된 판정 명령이 도달 가능하다 — 대상을 **slug 로 해석**해 문면 명령을 **추출 실행**한다. 판정:
  ```
  ok=0; bad=0; for e in "components/monitor:N-5" "components/monitor:N-6" "testing/derived-population-totality-judgement:T-4" "foundation/auth-entry-failure-user-observable-surface:S-2"; do slug=${e%:*}; lab=${e##*:}; f=""; for r in green blue; do if [ -f "${ACC_SPEC_ROOT:-specs/30.spec}/$r/$slug.md" ]; then f="${ACC_SPEC_ROOT:-specs/30.spec}/$r/$slug.md"; break; fi; done; if [ -z "$f" ]; then echo "slug 미해석: $slug" >&2; bad=$((bad+1)); continue; fi; cmd=$(awk -v L="$lab" 'BEGIN{B=sprintf("%c",96);F=B B B;RE=B "[^" B "]+" B} !hit && $0 ~ ("^- \\[[ x]\\] \\((Must|Should)[^)]*, " L "\\)") {hit=1; l=$0; while (match(l,RE)) {c=substr(l,RSTART+1,RLENGTH-2); if (c ~ /^(bash -c|npx |npm run |awk |test |grep -)/) {print c; exit} l=substr(l,RSTART+RLENGTH)} next} hit {t=$0; sub(/^[[:space:]]+/,"",t); if (index(t,F)==1) {if (fc) exit; fc=1; next} if (fc) print t}' "$f"); if [ -z "$cmd" ]; then echo "판정 명령 추출 0: $slug ($lab)" >&2; bad=$((bad+1)); continue; fi; out=$(printf '%s\n' "$cmd" | bash 2>&1); rc=$?; if [ "$rc" -eq 0 ]; then ok=$((ok+1)); else bad=$((bad+1)); echo "UNREACHABLE $slug ($lab) rc=$rc :: $(printf '%s' "$out" | head -1)" >&2; fi; done; echo "repaired-reachable=$ok/4 failed=$bad"; [ "$ok" -eq 4 ]
  ```
  → **rc=0**. **HEAD=`61ce5ab` (tick 227 재실행) 실측 rc=0 / `repaired-reachable=4/4` → 충족** (tick 226 동일). **동일 트리 교정 전 실측 rc=1 / `repaired-reachable=2/4`** — (N-5) `header=4 bar=3 shared=3` rc=1 · (T-4) rc=1 이 도달 불가였다. **경로 리터럴이 아니라 slug 해석인 이유** — 라인 번호 고정은 편집으로 즉시 낡고, 경로 고정은 **승격 `mv` 로 깨진다.** green 우선 해석은 파이프라인 의미론(`green` 이 최신 판본)과 일치하며, 승격 후에는 blue 판본이 교정판이므로 산출이 불변이다. **추출 실행이 요점** — 손으로 옮겨 적으면 인용 단계가 줄어 결과가 뒤집힌다 (§동작 (P-2) 의 오진이 정확히 그렇게 발생했다).
- [x] (Must, C-4) 교정이 계약을 완화하지 않았다 — 교정된 판정식이 **준수 트리에서 특이도를 유지**한다. 대상을 **slug 로 해석**해 문면 명령을 **추출 실행**하고, rc 뿐 아니라 **산출 수치**까지 판정한다. 판정:
  ```
  slug=foundation/auth-entry-failure-user-observable-surface; lab=S-2; f=""; for r in green blue; do if [ -f "${ACC_SPEC_ROOT:-specs/30.spec}/$r/$slug.md" ]; then f="${ACC_SPEC_ROOT:-specs/30.spec}/$r/$slug.md"; break; fi; done; if [ -z "$f" ]; then echo "slug 미해석: $slug" >&2; exit 2; fi; cmd=$(awk -v L="$lab" 'BEGIN{B=sprintf("%c",96);F=B B B;RE=B "[^" B "]+" B} !hit && $0 ~ ("^- \\[[ x]\\] \\((Must|Should)[^)]*, " L "\\)") {hit=1; l=$0; while (match(l,RE)) {c=substr(l,RSTART+1,RLENGTH-2); if (c ~ /^(bash -c|npx |npm run |awk |test |grep -)/) {print c; exit} l=substr(l,RSTART+RLENGTH)} next} hit {t=$0; sub(/^[[:space:]]+/,"",t); if (index(t,F)==1) {if (fc) exit; fc=1; next} if (fc) print t}' "$f"); if [ -z "$cmd" ]; then echo "판정 명령 추출 0: $slug ($lab)" >&2; exit 2; fi; out=$(printf '%s\n' "$cmd" | bash 2>&1); rc=$?; echo "$out"; fc=$(printf '%s' "$out" | sed -nE 's/.*failure-cases=([0-9]+).*/\1/p' | head -1); un=$(printf '%s' "$out" | sed -nE 's/.*unobserved=([0-9]+).*/\1/p' | head -1); if [ -z "$fc" ] || [ -z "$un" ]; then echo "산출 수치 부재" >&2; exit 2; fi; if [ "$fc" -lt 1 ]; then echo "공허: failure-cases=0" >&2; exit 2; fi; echo "specificity: rc=$rc failure-cases=$fc unobserved=$un"; [ "$rc" -eq 0 ] && [ "$un" -eq 0 ]
  ```
  → **rc=0**. **HEAD=`61ce5ab` (tick 227) 실측 rc=0 / `failure-cases=3 unobserved=0` → 충족.** **tick 227 에 문면을 산문에서 명령으로 교체했다** — 종전 판본은 "교정 전후 두 판정식을 같은 트리에서 실행해 산출이 일치" 라는 **절차 서술**이었고, 그 형태로는 (a) '교정 전' 판본이 현 HEAD 에 없어 재실행이 불가능하고, (b) 복원하려면 **본 spec 자신의 경로를 git 리비전과 함께 참조**해야 해 `RULE-07 §promote 조건 2` 를 어긴다. 그래서 **재판정 가능한 절반**(준수 트리 특이도)만 체크박스로 남기고, 교정 전후 대조의 역사 수치는 §참고로 내렸다. **공허 가드 3중** — slug 미해석 · 판정 명령 추출 0 · `failure-cases<1` 은 전부 `exit 2`(무판정)이며 충족으로 읽지 않는다. **판정 선언 항목이면서 모집단 도출이 0 이던 2건 중 1건이 이 항목이었다** (REQ-20260826-028 §배경) — 산문 판정은 `check:acceptance-criteria` 의 G-3·G-4 모집단 밖이라 **구조적으로 무검출**이었고, 펜스 명령으로 바꿔 모집단에 편입시켰다.
- [x] (Should, C-5) 함정 부류 재발 채널 — `check:acceptance-criteria` 가 (C-1)(C-2) 의 검출을 수행하고 그 수치를 **비공허 하한과 함께** 낸다. 판정: `bash -c 'out=$(npm run --silent check:acceptance-criteria 2>&1); echo "$out"; echo "$out" | grep -qE "judgement-commands=[0-9]+" && echo "$out" | grep -qE "range-restart=[0-9]+"'` → **rc=0**. **HEAD=`61ce5ab` (tick 227) 실측 rc=0 / `judgement-commands=324 range-restart=0 unexecutable-verb=0 forbidden-verbs=5` → 충족 (tick 227 착수 시점 동일 트리 `303` — 본 tick 이 판정 문면 3건을 산문/절단 스팬에서 펜스로 옮겨 도출이 늘었다. **모집단 크기는 편집에 따라 움직이는 도출량이지 상수가 아니다**)** (TSK-20260826-41 / `61ce5ab` — `scripts/check-acceptance-criteria.sh` 에 G-3·G-4 부착). **이 축의 상시 채널이 inspector Phase 1 에서 `check:*` 로 옮겨졌다.** `forbidden-verbs=5` 는 위반 수가 아니라 **검출 토큰 어휘 수**이며, G-4 의 관용구 열거가 비지 않았음을 보이는 보조 단언이다 (`RULE-06 §열거 고정 금지`). **검출 부류는 열거이며 전칭을 주장하지 않는다** (§역할 (iii)).
...
| 2026-08-26 | TSK-20260826-41 / `61ce5ab` (inspector tick 227) | **Phase 1 재실행 5/5 ack — 수용 기준 4/5 → 5/5.** (C-5) 착지: `scripts/check-acceptance-criteria.sh` 에 G-3(주소 범위 재시작)·G-4(실행 금지어) + `ACC_SPEC_ROOT` seam + `DERIVE_MIN=100` + 종료 코드 3분할(0/1/2). tick 227 이 게이트 민감도를 **저장소 밖 사본 루트**에 위반 1건씩 주입해 직접 확인했다 — G-3 주입 `rc=1` + `파일:라인` 발화, G-4 주입 `rc=1` + 발화, 원복 후 `shasum` 일치 · 저장소 트리 변경 0. `forbidden-verbs=5` 는 위반 수가 아니라 **검출 토큰 어휘 수**(G-4 관용구 열거 비공허 보조 단언)임을 확인해 문면에 박제했다. **(C-4) 판정 문면을 절차 서술에서 펜스 명령으로 교체.** 종전 문면은 판정을 **선언**했으나 실행 가능한 명령 스팬이 없어 `check:acceptance-criteria` 판정 명령 모집단에서 이탈했고 — tick 227 독립 실측 결과 §수용 기준 최상위 체크박스 783항 중 판정 선언 69항, 그중 도출 0 이 **2항**이었으며 본 항목이 그 하나였다 (REQ-20260826-028 §배경 수치를 받아쓰지 않고 도출기 문면을 스크립트에서 추출해 재현, `783/69/226/67` 정확 일치) — G-3·G-4 가 이 항목을 **구조적으로 무검출**로 두고 있었다. 교체 시 '교정 전' 판본은 현 HEAD 에 없어 재실행이 불가능하고 복원하려면 **본 spec 자신의 경로를 git 리비전과 함께 참조**해야 해 `RULE-07 §promote 조건 2` 를 어기므로, **재판정 가능한 절반**(준수 트리 특이도)만 체크박스로 남기고 교정 전후 대조의 역사 수치는 문면 주석으로 보존했다. 실측 `failure-cases=3 unobserved=0` rc=0 — blue 박제와 동일, 계약 완화 0. **promote 후보.** | 수용 기준, 테스트 현황, 변경 이력 |
