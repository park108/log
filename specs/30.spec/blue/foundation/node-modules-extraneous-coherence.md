...
- [x] (G3) 신규 dep 추가/제거 후 동일 PR 안에 G1·G2 동시 충족 — 단일 사례 (환경 회복 시점) 박제, 차기 dep 이벤트 후 재검증으로 marker 플립 누적. — **측정**: 본 세션에서 dep 추가(`eslint-plugin-react-hooks`)와 제거(`browserslist` 키)가 각각 1 커밋 안에서 G1·G2 동시 충족을 유지했다 (`declared=30 installed=30`).
...
- [x] (G6.1) `grep -nE "check:deps|check-deps" .husky/pre-commit` → **≥ 1 hit**. HEAD=`59c407d` 재실측 **1 hit** @`:18` `bash scripts/check-deps-coherence.sh || exit 1` (TSK-20260518-26 / `59c407d` 회수; baseline `cd75eed` 0 hit → 1 hit). gate (af) 회복.
- [x] (G6.2) drift fixture (extraneous 1+ 또는 `N ≠ M`) 상태 `git commit -m "test"` → rc=1 + commit object 미생성 + stderr 메시지. fixture: `npm install --no-save <extraneous-pkg>` 후 commit 시도. 회수 후 marker 플립. — **주입 검증**: 선언만 있고 미설치인 dep 을 넣어 `declared=31 != installed=30` 으로 **rc=1 검출 확인**. 최초 주입에서는 미검출이었고 §검출 공백 회수 참조.
- [x] (G6.3) drift 미도입 (G1=0 ∧ G2 N=M) 상태 `git commit -m "test"` → rc=0 + commit object 생성. false-positive 0. 회수 후 marker 플립. — **측정**: 본 세션 6 커밋이 모두 drift 없는 상태에서 pre-commit 통과 (false-positive 0).
...
- [x] **(Must, G6.1 — REQ-007 흡수)** `.husky/pre-commit` 본문에 `check:deps` 효능 명령 (`npm run check:deps` 또는 `bash scripts/check-deps-coherence.sh` 직접 호출 등) 이 1+ 회 호출된다. `grep -nE "check:deps|check-deps" .husky/pre-commit` → **≥ 1 hit**. HEAD=`59c407d` 재실측 **1 hit** @`:18` `bash scripts/check-deps-coherence.sh || exit 1` (staged 조건부, `package.json`/`package-lock.json` 변경 시 한정 호출 — 수단 (b) 채택, G6.4 수단 중립 정합). TSK-20260518-26 / `59c407d` 회수 ack.
- [x] **(Must, G6.2 — REQ-007 흡수)** deps coherence drift 도입 commit (G1 extraneous ≠ 0 또는 G2 N ≠ M) 시도 시 `pre-commit` hook rc ≠ 0 → commit object 미생성 + stderr extraneous/diff 메시지. fixture 환경 재현 가능 — `npm install --no-save <extraneous-pkg>` 또는 `package.json` declared 1 항목 제거 후 `git commit -m "test"` → rc=1. — 주입 검증 완료 (위 G6.2 동일 근거).
- [x] **(Must, G6.3 — REQ-007 흡수)** drift 미도입 (G1=0 ∧ G2 N=M) 상태 `git commit -m "test"` → rc=0 + commit object 생성. false-positive 0. 본 게이트가 정상 commit 차단을 도입하지 않음. — 본 세션 6 커밋으로 확인.
- [x] **(Must, G6.4 — REQ-007 흡수, 회귀 0)** pre-commit 채널 추가 후 기존 hook 분기 — `npx lint-staged` (line 1) + `check-spec-coherence` 조건부 (line 4-7, REQ-076) + `check-vite-env-coherence` 조건부 (line 11-13, REQ-083) — rc 와 grep 정합 유지. REQ-076 / REQ-080 / REQ-083 효능 회귀 0. HEAD=`59c407d` 재실측: `grep -nE "^npx lint-staged" .husky/pre-commit` → 1 hit @`:1` + `grep -nE "check-spec-coherence" .husky/pre-commit` → 1 hit @`:6` + `grep -nE "check-vite-env-coherence" .husky/pre-commit` → 1 hit @`:12` + 자매 채널 보존: `.husky/pre-push:1` (1 hit) + `.github/workflows/ci.yml:29` (1 hit) + `package.json:26` (1 hit) 전수 PASS. result.md DoD `npm run lint` rc=0 / warning 0 + `npm test` 57 files 500 tests PASS / 회귀 0 + `npm run build` rc=0 + `npm run check:deps` rc=0 `[deps] extraneous=0 declared=29 installed=29 (PASS)` 박제.
...
| 2026-05-18 | inspector 106차 Phase 1 hook-ack / TSK-20260518-26 / `59c407d` | (G6.1) + (Must, G6.1) + (Must, G6.4 회귀 0) marker 3건 [ ]→[x] 플립 — `.husky/pre-commit:18` `bash scripts/check-deps-coherence.sh || exit 1` (staged 조건부, line 16-19 if/then/fi block 6 line append) 부착 ack. HEAD=`59c407d` 재실측: gate (af) `grep -nE "check:deps\|check-deps" .husky/pre-commit` → **1 hit** @`:18` (baseline `cd75eed` 0 hit → 1 hit; G6.1 zero-point cell 회복) + G6.4 회귀 0 자매 hook 분기 보존 grep gates 전수 PASS (`^npx lint-staged` 1 hit @`:1` + `check-spec-coherence` 1 hit @`:6` + `check-vite-env-coherence` 1 hit @`:12`) + 자매 채널 보존 (`.husky/pre-push:1` + `.github/workflows/ci.yml:29` + `package.json:26` 전수 1 hit 유지). result.md DoD `npm run lint` rc=0 / warning 0 + `npm test` 57 files / 500 tests PASS / 회귀 0 + `npm run build` rc=0 + `npm run check:deps` rc=0 `[deps] extraneous=0 declared=29 installed=29 (PASS)` 박제. `git merge-base --is-ancestor 59c407d HEAD` PASS. RULE-07 정합 — 수단 중립 평서문 (§역할 line 10 "자동 게이트 hook 본문 위치·순서·실행 형태 (`npm run` vs `bash` 직접) / 조건부 vs 무조건 부착" + §동작 G6.4 "수단 중립") 보존, 채택 수단 (b) staged 조건부 + `bash` 직접 호출은 marker 박제 영역 한정. spec 본문·src/**·`scripts/check-deps-coherence.sh` 본체·`package.json scripts.check:deps`·`.husky/pre-push`·`.github/workflows/ci.yml` 변경 0 동반 정합 (G6.4 회귀 0). (G6.2) (G6.3) (Must, G6.2) (Must, G6.3) 4 marker 유지 — result.md `[~]` 박제 (자동 검증 미수행, fixture PR diff 잔존 금지 의무) — 후속 task TSK-20260518-27 가설 영역 (자동 fixture 박제). G3 (시점 비의존) marker 유지 — 차기 dep 이벤트 대기. | 헤더 §최종 업데이트 + §테스트 현황 (G6.1) + §수용 기준 (Must, G6.1) + (Must, G6.4) + §변경 이력 |
| 2026-08-24 | (수동 — 운영자) / 본 변경 | 마커 회수 + 검출 공백 1건 회복. G6.2 주입 검증에서 게이트가 미설치 선언을 설치된 것으로 계수해 drift 를 놓치는 것을 발견 — `scripts/check-deps-coherence.sh` 의 installed 계수에서 `UNMET DEPENDENCY`/`invalid` 라인 제외로 회수. G3·G6.2·G6.3 flip, 수단 위임 2항목 §참고 강등. green→blue promote. | §테스트 현황 / §수용 기준 / §참고 |
...
G6.2 를 실제로 주입해 보니 **검출되지 않았다** — 선언만 있고 설치되지 않은 dep 을 추가했는데 게이트가 `declared=31 installed=31 (PASS)` 를 반환했다.

원인은 `scripts/check-deps-coherence.sh` 의 installed 계수 방식이었다. `npm ls --depth=0` 은 미설치 선언도 `└── UNMET DEPENDENCY <name>@<range>` 형태의 트리 라인으로 출력하는데, 스크립트가 `^[├└]` 로만 세는 바람에 그 라인이 "설치됨" 으로 계수됐다. 그러면 declared 와 installed 가 항상 같아져 **G2 가 잡으라는 drift (선언 O / 설치 X) 를 정확히 놓친다.**

`grep -cvE 'UNMET DEPENDENCY|invalid'` 를 추가해 회수했고, 재주입 시 `G2 VIOLATION: declared N=31 != installed M=30` 으로 rc=1 검출된다. 정상 상태 rc=0 도 재확인.
