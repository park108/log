...
> **최종 업데이트**: 2026-05-19 (by inspector — 120차 tick / 사용자 trigger surface — (I2) + (I3) 2 marker future-event-dependent 분기 B 재배치 — checklist 영역 → `**[deferred: ...]**` 본문 보존 + checklist 마커 제거; spec unchecked 0 도래).
...
## 수용 기준
- [x] (Must, FR-01) island 정의 세 번째 축 효능 박제 — 6 island 디렉터리 selector 정밀 grep (M-B) 0 hit 회복 PASS (HEAD=`b76dc02` 시점 실측 — TSK-20260517-21 회수 `efe55e8`, hook-ack: result.md "회귀 0" + "exit=0" 13 hit + HEAD ancestor PASS).
...
- [x] (Must, FR-07) 수단 라벨 0 — `awk '/^## 역할/,/^## 의존성/' specs/30.spec/blue/foundation/island-proptypes-zero.md | grep -vE '`[^`]*default[^`]*`' | grep -cE "기본값|권장|우선|default|best practice"` → 0 hit (§스코프 규칙 G3 자기 검증).
...
- **grep-baseline** (HEAD=`3bc98e5`, 2026-05-17 — REQ-088 흡수 시점 실측):
  ...
  - (G3) **[FR-07 수단 라벨 자기 검증]** `awk '/^## 역할/,/^## 의존성/' specs/30.spec/blue/foundation/island-proptypes-zero.md | grep -vE '`[^`]*default[^`]*`' | grep -cE "기본값|권장|우선|default|best practice"` → **0 hit** (본 spec §역할 + §동작 + §회귀 중점 + §의존성 어디서도 PropTypes 제거 수단 후보 라벨 부여 0). HEAD=`3bc98e5` 박제 시점 PASS.
  - (G4) **[NFR-01 시점 비의존 자기 검증]** `awk '/^## 역할/,/^## 테스트 현황/' specs/30.spec/blue/foundation/island-proptypes-zero.md | grep -cE "48 hit|57 hit|13 파일|46 hit|11 파일"` → **0 hit** (본 spec §역할 + §동작 + §회귀 중점 + §의존성 어디서도 절대 수치 박제 0 — 수치는 §스코프 규칙 grep-baseline 한정). HEAD=`3bc98e5` 박제 시점 PASS.
...
| 2026-05-17 | inspector (Phase 1 hook-ack) / HEAD=`b76dc02` (TSK-20260517-21 회수 `efe55e8`) | (I1) + (I4) §테스트 현황 marker + (Must FR-01) §수용 기준 marker 플립 — selector 정밀 grep (M-B) 5 island 디렉터리 57→0 hit + 6 island 디렉터리 (Search 포함) 전수 0 hit 회복 PASS. 광역 grep (M-A) 5 island 디렉터리 48→2 hit (자기 진단 주석 2건 잔존 `ErrorFallback.tsx` + `Skeleton.tsx` — FR-06 분기, 별 task 영역). 비-island 41 hit 무영향 + lint warning 0 + typecheck exit=0 + 440/440 test pass + build exit=0 (result.md "회귀 0" + "exit=0" 13 hit + efe55e8 HEAD ancestor PASS). 잔여 (I2)(I3) marker [ ] 유지 — "신규 island 수렴 시점" 검증 의존 (별 시점 hook-ack 영역). (Should FR-04)(FR-05) 기 [x] (평서 정합). | 테스트 현황 + 수용 기준 + 변경 이력 |
