...
> **최종 업데이트**: 2026-05-19 (by inspector — 120차 tick / 사용자 trigger surface — (I9) + (FR-06) 2 marker future-event-dependent 분기 B 재배치 — checklist 영역 → `**[deferred: ...]**` 본문 보존 + checklist 마커 제거; spec unchecked 0 도래)
...
## 테스트 현황
- [x] (I1) warn level rule 정의 상시 박제: `eslint.config.js` 가 1+ `warn` level rule 을 정의하는 동안 자동 채널 1+ 가 `--max-warnings=0` 또는 동등 효능 호출. TSK-20260517-17 / `78f87a0` 회수 — `package.json:23` `"lint": "eslint ./src --max-warnings=0"` 박제, CI Lint step (`.github/workflows/ci.yml:31` `npm run lint`) 가 본 script 호출로 채널 전파. HEAD=`49f3f93` 재실측: `grep -nE "max-warnings" package.json .github/workflows/ci.yml .husky/* eslint.config.js` → 1 hit @`package.json:23` PASS.
...
- [x] (Must, FR-02) 회귀 검증 — 임시 fixture (예: `src/__warn_probe__.ts` 미사용 import 1건) 도입 시 자동 채널 rc ≠ 0 fail + 제거 시 rc=0 회복. TSK-20260517-17 / `78f87a0` result.md G2 (`npm run lint` rc=1 + 'useState' is defined but never used + "ESLint found too many warnings (maximum: 0)" stdout) + G3 (제거 후 rc=0 회복) + G4 (잔존 0 — `git status` + `find` 모두 0) 사이클 박제. HEAD=`49f3f93` 재실측 PASS.
...
- [x] (NFR-05) RULE-02 정합 — TSK-20260517-17 / `78f87a0` result.md 변경 파일 = `package.json` 1줄 (수단 (a) 채택, `scripts.lint` 옵션 주입). `src/**` 변경 0 (fixture 도입/제거 사이클은 PR diff 외부 — G4 `git status` 잔존 0 박제). NFR-05 PASS.
...
| 2026-05-17 | inspector (Phase 1 ack, TSK-20260517-17 회수) / `78f87a0` | I1·I3·I5 + Must FR-01·FR-02 + Should FR-05 + NFR-05 marker 7건 `[ ]→[x]` 플립. ack 근거: TSK-20260517-17 `78f87a0` (HEAD=`49f3f93` 조상). hook-ack: `60.done/2026/05/17/task/eslint-warn-zero-master-gate/result.md` G0~G8 박제 (G0 zero-point 유지 rc=0 / G1 `--max-warnings` 1 hit `package.json:23` / G2 fixture `src/__warn_probe__.ts` rc=1 + warning 1+ + "ESLint found too many warnings (maximum: 0)" / G3 fixture 제거 rc=0 회복 / G4 잔존 0 / G5 `npm run lint` rc=0 / G6 `npm test` 전수 PASS coverage 97.74% / G7 `npm run build` rc=0 / G8 자매 check 4종 rc=0 — `lint:spec-coherence` + `check:deps` + `check:vite-env` + `check:node-coherence`). 변경 표면 `package.json` 1줄 (`scripts.lint` 옵션 주입, 수단 (a) 채택 — 본 spec §동작 7 수단 라벨 0 정합) + `src/**` 변경 0 (NFR-05 PASS). HEAD=`49f3f93` 재실측: `grep -nE "max-warnings" package.json .github/workflows/ci.yml .husky/* eslint.config.js` → 1 hit @`package.json:23` + `grep -cE "'warn'" eslint.config.js` → 3 hit + `grep -nE "reportUnusedDisableDirectives" eslint.config.js` → 2 hit (`'off'` 명시 유지) — baseline 정합. (I4)(I6)(I7)(I8) + FR-03·FR-04·FR-07 + NFR-01~04 직전 ack 유지. (I9) + FR-06 (`reportUnusedDisableDirectives` 부수 보호 표면) 차기 이벤트 대기 `[ ]` 유보 — 미래 `'off'→'warn'` 전환 시점 자동 차단 효능 surface. | 헤더 · §테스트 현황 · §수용 기준 · 본 이력 |
