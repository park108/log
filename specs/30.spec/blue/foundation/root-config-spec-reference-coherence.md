...
> **최종 업데이트**: 2026-08-24 (수동 — 운영자: 마커 회수 + green→blue promote)
...
- [x] (Must FR-02) root 파일군 내 모든 `specs/30\.spec/(blue|green)/.*\.md` 참조는 디스크에 실재한다 (`test -e` 전원 EXISTS). baseline 1 MISSING → 회수 후 0 MISSING (HEAD 실측 박제, inspector Phase 1 ack). — **측정**: root 파일군 spec 참조 전원 EXISTS (`G2 0 MISSING`).
- [x] (Must FR-03) root 파일군 내 `specs/30\.spec/green/...` 참조는 동일 slug 가 `specs/30\.spec/blue/...` 에 동시 실재하지 않는다. baseline 1 STALE → 회수 후 0 STALE (HEAD 실측 박제, inspector Phase 1 ack). — **측정**: 동일 slug 동시 실재 0.
...
- [x] **(Must, FR-06, REQ-20260518-020)** production↔fixture 묶음 처리 — root 파일군 spec 참조 회복 commit 과 `src/__tests__/root-config-spec-reference-coherence.test.ts` 의 `EXPECTED_*` baseline const 갱신은 단일 task scope / 단일 PR / 단일 commit 묶음 단위. 측정: 회복 commit `git diff --name-only` 출력에 (a) root 파일군 1+ 매칭 ∧ (b) 본 fixture 파일 매칭 동시 박제 + `npm test` rc=0 PASS. baseline drift surface (HEAD `9c5a82a`): G-B 3 MISSING (`scripts/check-{node-version,vite-env,package-manager}-coherence.sh:3` 의 green path 부재) + G-C 3 STALE (동일 slug blue 동시 실재). TSK-20260518-14 격리 사례 (sh script 단일 토큰 치환 후 fixture FAIL) 가 묶음 처리 위반 surface — 회복 task 재발행 (TSK-20260518-14-a 또는 동등) 시 sh script 치환 + fixture `EXPECTED_GB_MISSING` 3→0 + `EXPECTED_GB_EXISTS` 2→5 + `EXPECTED_GC_GREEN_PATHS` 4→1 + `EXPECTED_GC_STALE` 3→0 + `EXPECTED_GC_OK` 1 유지 동시 commit 필수. — **측정**: 본 세션 promote 4건 모두 spec 이동과 `src/__tests__/root-config-spec-reference-coherence.test.ts` baseline 갱신을 동일 커밋에서 처리했다 (eslint-react-hooks promote 시 G-B 5→6 격차를 fixture 가 검출).
- [x] **(Must, FR-07, REQ-20260518-020)** 회복 후 마커 동시 플립 — 본 spec G-B + G-C + FR-02 + FR-03 4 marker `[ ]→[x]` 와 시점 비의존 누적 1건 (G-E §FR 누적 사례 +1) 은 동일 묶음 회복 commit 단일 hook-ack 으로 inspector Phase 1 reconcile 시점 동시 플립. 분할 commit 시 marker flip 도 분할 진행 불허 (RULE-07 정합 — 회수 사례 누적은 시점 비의존 평서이나 marker hook-ack 은 단일 commit 단위 atomic). 측정: hook-ack commit 의 `git show --stat <commit>` 에 root 파일군 + fixture 파일 동시 변경 박제 + 동일 commit 으로 본 spec 4 marker flip. — **측정**: G-B·G-C·FR-02·FR-03 4 marker 를 본 갱신에서 동시 flip.
...
| 2026-08-24 | (수동 — 운영자) / 본 변경 | 마커 회수 — baseline 이 박제한 1 MISSING / 1 STALE 은 이미 해소된 상태였고 `check:spec-coherence` 가 `G2 0 MISSING` 을 반환한다. G-B·G-C·FR-02·FR-03·FR-06·FR-07 flip, deferred 6항목 §참고 강등. green→blue promote. | §테스트 현황 / §수용 기준 / §참고 |
...
- 본 세션 promote 6건에서 fixture 의 `EXPECTED_GB_*` baseline 을 매번 수동 갱신해야 했다. root 파일이 spec 을 참조하고 그 spec 이 promote 되면 참조가 끊기는 구조가 반복 비용을 만든다 — 참조를 slug 기준으로 두고 blue/green 을 탐색하는 방식이 대안이나 별 axis.
