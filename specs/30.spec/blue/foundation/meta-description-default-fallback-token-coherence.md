...
HEAD=`5a12a0b` 격리 사본(`git clone --local --no-hardlinks` + `node_modules` 심볼릭 링크, **실 `src`**)에서 **spec 파일에서 추출해** 실행했다 (손 전사 0 — `RULE-06 §추출 실패 검출`). 메인 워킹트리 무접촉.

- **§수용 기준 측정 명령 7/7 rc=0.** 8 항 중 `(Must, 범위 제한)` 1 항은 축 소유처를 지목하는 범위 선언이라 측정 명령이 없다.
- **진단 명령 9 run, 0 failed** (`RULE-07 2-bis` — §수용 기준 이 참조하지 않는 §테스트 현황·§동작·§의존성 판정): G-A `1` · G-B `1` · G-C `1` · G-E `2` · 길이 `node -e` → `63` · 게이트 스위트 `src/__tests__/meta-description-token-coherence.test.ts` **6 passed** · CI `.github/workflows/ci.yml` `Test` step **1** · revdep 도출 `revdep-docs=3` · 발화 채널 baseline **0 hit** (G-G 선언대로).
- **flip-check (민감도)** — 전건 원복 후 `rc=0` 복귀 확인: (H) 쪽만 텍스트 변경 → FR-04 `rc=1` (`H-code-units=34 D-code-units=63`) · (D) 쪽만 변경 → `rc=1` (`63 / 13`) · `<meta>` 제거 → FR-04 `rc=2` (**무판정**, 선언대로 — 빈 값끼리 동치로 읽지 않는다) · 진짜 두 번째 `<meta>` 등록 → FR-01 `2`·FR-06 `2` 로 `rc=1`.
- **control (음성 대조)** — 본 계약이 명시적으로 범위 밖이라 선언한 축(**문자 집합 무관**)을 골랐다: 비-ASCII 요약문을 (H)(D) 양면에 동시 적용 → FR-01·FR-04·FR-05·FR-06 전건 `rc=0`. 이때 출력이 `H-code-units=46 D-code-units=46 utf8-bytes=67` 로 **두 단위가 갈렸다** — 258차가 `byte` 와 `code unit` 을 나눈 것이 실측으로 확인된다 (ASCII baseline 에서는 63/63 으로 겹쳐 보이지 않던 구별이다).
- **목격자** (`RULE-06 §수치가 움직이지 않는 변형`): 위 대조를 **유지한 채** (D) 한 글자를 덧붙여 `rc=1` (`46 vs 47`) 을 받고, **목격자만** 걷어내 `rc=0` 을 확인했다. 대조의 `rc=0` 이 침묵이 아니라 판정임을 이것이 보증한다.

### 승격 시점 미해소 — `LogSingle` 좌표 드리프트 3 건 (planner 384차 실측)

258차 재개봉이 확장자(`.jsx`→`.tsx`)는 고쳤으나 **줄 번호는 그대로 두었다.** HEAD=`5a12a0b` 실측:

| 문면 | 실측 | 대상 |
|---|---|---|
| `LogSingle.tsx:39` (본문 4 회) | `:50` | `return () => { setMetaDescription(); }` — 무인자 cleanup |
| `LogSingle:57` | `:90` | `summary + ellipsis` 인자 |
| `LogSingle:63` | `:96` | `PAGE_NOT_FOUND` 인자 |

`index.html:7` · `src/common/common.ts:12` · `:14` 는 실측 일치다. **이 드리프트는 판정 명령이 아니라 산문이라 어떤 게이트도 붉지 않는다** — `RULE-07 §promote 조건 2` 의 상호참조 조항이 겨누는 것과 같은 부류이며, 여기서는 대상이 spec 경로가 아니라 코드 좌표다. 식별자(`setMetaDescription`)는 정확하므로 도출 가능하다.

### 승격 시점 미해소 — 앵커 없는 계수 2 건 (planner 384차 flip-check)

**FR-01 과 FR-05 는 줄 앵커도 구조 판정도 없는 토큰 계수라 주석을 위반으로 센다** (과잉 특정).

- `index.html` 에 `<!-- legacy: name="description" was here -->` 를 넣으면 FR-01 이 `html-description-meta=2` → `rc=1`. **같은 입력에서 FR-06 은 `content-form-matches=1` 로 `rc=0`** 이다 — 뒤따르는 `content="..."` 를 요구해 **구조**를 보기 때문이다. 두 항이 같은 입력에 반대 rc 를 낸다.
- `src/common/common.ts` 에 `// note: DEFAULT_META_DESCRIPTION is the fallback` 를 넣으면 FR-05 가 `identifier-distribution=3` → `rc=1`.

정상 변형을 위반으로 계수하는 것이므로 `RULE-06 §음성 대조` 는 **게이트를 좁히라**고 판정한다. 처방은 같은 문서 안에 이미 있다 — FR-06 의 형태 판정이 건전한 쪽이다. **승격 시점 현 HEAD 에서 두 명령의 `rc` 는 `0`** 이므로 `RULE-07 §promote 조건 2` 는 충족하며, 이 항목은 차기 게이트 수정 task 의 이관 대상이다.
