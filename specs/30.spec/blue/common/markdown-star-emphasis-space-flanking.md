...
의도적으로 하지 않는 것: **`~~` 축** (같은 부류의 어긋남이나 **본 계약이 소유하지 않는다** — 한국어 늘임표기 손상 표본이 나와 `markdown-tilde-strikethrough-space-flanking.md` 가 별 spec 으로 소유한다), `* 기울임 *` 이 글머리 목록으로 잡히는 것 (블록 패스가 인라인 패스보다 앞서기 때문 — 다른 코드 지점), `***셋***` 3중 구분자의 중첩 조합 (`markdown-emphasis-delimiter-parity` §참고 §미측정 소유), `_`·`__`·`*` 의 현 계약 변경, 강조의 CSS 표현 (계약면은 산출 태그다).
...
8. **(I8) `~~` 축은 본 계약이 소유하지 않는다 (범위 제한)**: 본 계약은 `**` 의 여닫이 판정만 옮긴다. `~~` 의 판정을 **요구하지도 금지하지도 않으며** 그 축은 `markdown-tilde-strikethrough-space-flanking.md` 가 소유한다. 본 계약의 구현이 `~~` 등록을 **없애거나 늘리지 않았다**는 것만 대조로 관측한다 — 등록은 정확히 1건이다.

   > **좁힌 이유 (2026-08-31).** 종전 문면은 `~~ 취소 ~~` → `<p><del> 취소 </del></p>` 를 "유지된다" 로 못 박고 게이트가 3인자 등록(`…"del"\)`)을 고정했다. 그것은 `TSK-20260831-10-b` 가 물결 축으로 넘치지 않았는지 재는 **task 시점 대조**로는 옳았으나, 계약으로 상주하면 **저장소가 물결 축의 결함을 영구히 옳다고 선언**하게 된다. 그 선언이 참인 동안에는 이웃 계약이 도착할 수 없다. 등록 arity 를 빼고 **등록 1건**만 재도록 좁히면 두 계약이 동시에 참이다.
...
- 직교: `specs/30.spec/green/common/markdown-emphasis-delimiter-parity.md` — 그 spec 의 **(I6) 별표 축 범위 제한**이 본 계약의 도착과 **동시에 참일 수 없어**, 본 흡수가 같은 tick 에 blue 사본을 green 으로 받아 (I6) 을 좁혔다. 좁힌 문면이 본 spec 을 인수처로 지목한다 (§변경 이력). `specs/30.spec/green/common/markdown-tilde-strikethrough-space-flanking.md` — **`~~` 축의 소유처**이며 본 계약의 (I7) 단일 출처 명제를 물려받는다 (그 spec 은 같은 게이트를 다시 세우지 않는다). 본 계약의 (I8) 이 그 도착에 맞춰 좁혀졌다 (§변경 이력). `specs/30.spec/blue/common/markdownParser.md` (`inlineParsing` 알고리즘 — 본 계약은 그것을 대체하지 않고 등록 인자를 옮긴다).
...
- [x] (I8 범위 제한 대조) `~~` 등록이 정확히 1건으로 유지된다: `bash -c 'test "$(grep -cE "inlineParsing\(parsed, \"~~\", \"del\"" src/common/markdownParser.ts)" -eq 1'` → HEAD=`ed64fb3` 재실측 rc=0. **닫는 괄호를 패턴에서 뺀 것이 요점**이다 — 본 계약이 재는 것은 `~~` 등록의 **존재와 유일성**이지 인자 개수가 아니다. 종전 패턴(`…\"del\"\)`)은 3인자 형태를 고정해 이웃 계약의 도착을 막았다 (§동작 (I8) 좁힘 사유). 구현 후에도 rc=0 이어야 한다.
...
- [x] (Must, 범위 제한) 위 §테스트 현황 (I8 범위 제한 대조) 명령 → rc=0. HEAD=`ed64fb3` 재실측 rc=0 (좁힌 패턴 · 전환 전후 모두 **1**).
...
| 2026-08-31 | inspector 251차 tick (Phase 3, REQ-20260831-064 흡수의 충돌 해소) / — @ HEAD=`ed64fb3` | **(I8) 좁힘 + Ctrl-1 이전 + 거짓이 된 전제 정정.** 물결 축이 이웃 spec 으로 도착하면서 본 spec 의 세 자리가 그 도착과 **동시에 참일 수 없었다**: (1) **(I8)** 이 `~~ 취소 ~~` → `<p><del> 취소 </del></p>` 를 "유지된다" 로 못 박고 게이트가 3인자 등록(`…"del"\)`)을 고정했다 — 등록 arity 를 빼고 **등록 1건**만 재도록 좁혔다 (좁힌 패턴은 전환 전후 모두 **1** 이라 마커 불변 · 격리 사본 양쪽 실측). 그대로 두면 저장소가 물결 축의 결함을 **영구히 옳다고 선언**하게 되고, 그 선언이 참인 동안 이웃 계약이 도착할 수 없다. (2) **Ctrl-1** 이 `~~` 축에서 뽑혀 있었다 — 그 축에 계약이 도착하면 대조가 게이트가 겨누는 축과 같아져 대조이기를 그만두므로 (`RULE-06 §음성 대조`) **제목 축**(`markdownParser.test.ts:626`, blue `markdown-atx-heading-closing-sequence` 소유)으로 옮겼다. (3) **§역할·§참고 §미측정의 전제** *"산문 손상 사례가 없어"* 가 거짓이 됐다 — 한국어 늘임표기가 그 표본이며 운영자가 직접 재현했다. 전제를 지우지 않고 **거짓이 된 사실과 인수처를 함께** 박제했다. **본 spec 을 물결 축으로 확장하지 않은 이유**: 같은 tick Phase 1 에서 §수용 기준 11/11 이 되어 promote 후보가 됐고, 물결 축을 접으면 이미 착지한 `**` 계약이 물결 구현이 끝날 때까지 blue 로 가지 못한다. 제목·§역할이 `**` 에 한정돼 있다는 점은 이 spec 자신이 `markdown-emphasis-delimiter-parity` 에서 갈라져 나온 기준과 같다. 마커 변동 0 (11/11 유지). | 동작 · 테스트 현황 · 수용 기준 · 역할 · 의존성 · 참고 |
...
- **Ctrl-1 (특이도)** — 본 spec 이 범위 밖으로 선언한 **제목 축**(`제목의 닫는 # 은 제목 글자가 아니다`, `markdownParser.test.ts:626`)의 정상 가산 단언 추가 → `rc=0`. **가산형으로 수행한다** (기존 단언에 추가; 문자열 교체 금지) — `TSK-20260831-07` 에서 교체형 Ctrl 이 같은 task 의 정적 문자열 핀 DoD 와 상충해 `rc=1` 을 낸 실측이 있다.

  > **종전 Ctrl-1 은 `~~` 축이었고 2026-08-31 에 옮겼다.** 물결 축에 이웃 계약(`markdown-tilde-strikethrough-space-flanking`)이 도착하면서 그 축은 더 이상 "아무도 건드리지 않는 범위 밖" 이 아니게 됐다. 대조를 게이트가 겨누는 축 **또는 곧 바뀔 축**에서 뽑으면 대조가 대조이기를 그만둔다 (`RULE-06 §음성 대조`). 제목 축은 별 blue 계약(`markdown-atx-heading-closing-sequence`)이 소유하며 본 계약과 코드 지점이 겹치지 않는다.
...
- **`~~` 축은 본 계약이 판정하지 않으며, 이제 이웃 spec 이 소유한다.** 종전에 여기 적혀 있던 "산문에서 짝맞는 `~~` 가 공백을 사이에 두고 나오는 사례를 확인하지 못해 손상 근거가 없다" 는 **더 이상 참이 아니다** — 한국어 늘임표기(`와~~ 좋다 정말 대박~~`)가 그 표본이며 운영자가 직접 재현했다. 축은 `specs/30.spec/green/common/markdown-tilde-strikethrough-space-flanking.md` (REQ-20260831-064) 로 갔고, (I8) 은 "본 계약이 그 축을 건드리지 않았다" 만 남긴다.
