...
인용(`>`) 내용에도 블록 패스가 적용된다 — 목록·ATX 제목·**밑줄식 제목**·중첩 인용이 인용 **안에서** 블록으로 렌더된다. 독자가 인용 안에서 마크업 문자를 글자로 보지 않는다.

**의도적으로 하지 않는 것**:

- 인용 안 코드 펜스·표는 기대 산출이 확정되지 않아 범위 밖이다.
- **인용 안 가로줄**(`> ---` 단독 — 앞에 인용 내용이 없는 경우)의 기대 산출은 정하지 않는다. HEAD=`5420677` 산출은 `<blockquote>---</blockquote>` 이나 **본 계약은 그것을 "유지된다" 로 적지 않는다** — (I7) 착지 구현이 최상위 판정을 재사용하면 이 자리도 함께 움직일 수 있고, 움직이든 아니든 그 방향은 별 계약 사안이다.
...
7. **(I7) 인용 안 밑줄식 제목** (REQ-20260831-079 FR-02·FR-03): 인용 내용의 밑줄식 제목이 인용 안에서 제목으로 렌더된다. `> 제목\n> ---` → `<blockquote><h2>제목</h2></blockquote>` · `> 제목\n> ===` → `<blockquote><h1>제목</h1></blockquote>`. **밑줄 줄은 산출에 남지 않는다.**

   > **지금은 `---` 가 글자로 샌다.** HEAD=`5420677` 격리 사본 실측 — `> 제목\n> ---` → `<blockquote>제목<br />---</blockquote>` · `> 제목\n> ===` → `<blockquote>제목<br />===</blockquote>`. **독자는 원문에 없는 대시 세 개를 본다.** 같은 글에서 `> # 제목`(ATX)은 동작하는데 `> 제목\n> ---`(setext)은 동작하지 않으므로, **표기에 따라 제목이 되기도 안 되기도 한다** — (I2) 가 이미 닫은 자리의 미열거 이웃이다.

   > **최상위 계약의 상속이지 재정의가 아니다.** 밑줄 개수 하한 · `*`·`_` 는 밑줄이 아님 · 앞에 내용이 없으면 밑줄이 아님 같은 규칙은 `common/markdown-setext-heading-underline` (I1)~(I5) 가 정한 것을 상속한다 — §역할 의 *"블록 패스가 적용된다는 것이지 그 패스의 규칙을 다시 쓰는 것이 아니다"* 와 같은 판단이다.

   > **(I5) 와 충돌하지 않는다.** (I5) 는 **블록 문법이 없는** 여러 줄 인용이 한 덩어리로 남는 것을 요구하며, `> 제목\n> ---` 는 블록 문법이 있는 경우다. 다만 구현이 (I5) 를 깨뜨리는 방향으로 갈 수 있으므로 접속 판정이 둘을 함께 닫는다.
...
- [x] (I7 인용 안 밑줄식 제목) 계약이 게이트로 실재하고 초록이다: `bash -c 'set -- $(grep -rl "인용 안에서도 밑줄식 제목은 제목이다" src 2>/dev/null); test "$#" -ge 1 || exit 2; npx vitest run "$@" -t "인용 안에서도 밑줄식 제목은 제목이다" --coverage.enabled=false >/dev/null 2>&1'` → HEAD=`5420677` 실측 **rc=2 (게이트 부재 — 무판정)**. 미착수이며 이관처 task 는 미발행이다. **`-` 축과 `=` 축을 한 이름 안에서 둘 다 단언한다** (FR-03). · HEAD=`a1ed6c3` 재측정 **rc=0** (`d93506d` 착지).
- [x] (I5·I7 접속) 밑줄식 제목이 인용 안에서 열리면서 **블록 없는 여러 줄 묶음이 깨지지 않는다**: `bash -c 'D=""; for n in "인용 안에서도 밑줄식 제목은 제목이다" "블록이 없는 여러 줄 인용은 한 덩어리다"; do h=$(grep -rl "$n" src 2>/dev/null); test -n "$h" || exit 2; D="$D $h"; done; set -- $(printf "%s\n" $D | sort -u); test "$#" -ge 1 || exit 2; npx vitest run "$@" src/common/markdownParser.test.ts --coverage.enabled=false >/dev/null 2>&1'` → HEAD=`5420677` 실측 **rc=2 (게이트 부재)**. **이 접속이 (I7) 구현의 유일한 완화 방지선이다** — 인용 내용을 줄 단위로 쪼개 블록 판정을 붙이는 가장 쉬운 구현이 (I5) 를 깨뜨린다. · HEAD=`a1ed6c3` 재측정 **rc=0** (`d93506d` 착지).
...
- [x] (Must, REQ-20260831-079 FR-02·FR-03) 위 §테스트 현황 (I7 인용 안 밑줄식 제목) 명령 → rc=0. · HEAD=`a1ed6c3` 재측정 **rc=0** (`d93506d` 착지).
- [x] (Must, REQ-20260831-079 NFR-02) 위 §테스트 현황 (I5·I7 접속) 명령 → rc=0. · HEAD=`a1ed6c3` 재측정 **rc=0** (`d93506d` 착지).
- [x] (Must, 범위 제한) 인용 안 코드 펜스·표 · **인용 안 가로줄(`> ---` 단독)** · 지연 인용 · `>` 앞 들여쓰기 · 인용 내부 목록 경계 세부 규칙은 본 계약의 요구 대상이 아니다 — §역할 · §참고 §미측정.
...
- **grep-baseline** (HEAD=`8d030ce`, `git archive` 격리 사본 실측):
  ...
  - (I7 추가, HEAD=`5420677`) 계약 이름 `인용 안에서도 밑줄식 제목은 제목이다` → **0 hit** (신설 대상). 인용 안 setext 손상 실측 2행: `> 제목\n> ---` → `<blockquote>제목<br />---</blockquote>` · `> 제목\n> ===` → `<blockquote>제목<br />===</blockquote>`.
...
| 일자 | TSK / 커밋 | 요약 | 영향 섹션 |
|------|-----------|------|----------|
| 2026-09-01 | inspector 256차 tick (Phase 1 drift reconcile) / `d93506d` @ 측정 HEAD=`a1ed6c3` | **[ ] → [x] 4** — (I7 인용 안 밑줄식 제목) 과 (I5·I7 접속) 이 `d93506d` (*fix: 인용 안에서도 밑줄식 제목은 제목이다*) 로 착지했다. 두 판정 명령을 문서에서 추출해 `git archive a1ed6c3` 격리 사본에서 실행 **전건 rc=0** (이름 2건 실재 — `markdownParser.test.ts:447` · `:378`). **접속 항이 함께 초록인 것이 이 착지의 요점**이다: 인용 내용을 줄 단위로 쪼개는 가장 싼 구현은 (I5)(블록 없는 여러 줄 인용은 한 덩어리)를 깨뜨리는데 (I7) 만 재는 게이트에는 보이지 않는다. 1차 측정(`c82c629`)에서는 두 명령 모두 **rc=2 (부재 무판정)** 였다. **§수용 기준 22/22 promote 후보.** | §테스트 현황 · §수용 기준 |
| 2026-09-01 | inspector 255차 tick (Phase 3, REQ-20260831-079 FR-02·FR-03 흡수) / — @ HEAD=`5420677` | **(I7) 인용 안 밑줄식 제목 신설.** 이 자리는 (I2) 가 닫은 *"인용 안 ATX 제목"* 의 **미열거 이웃**이다 — §역할 이 *"블록 문법이 동작한다"* 고 선언했는데 §범위 제한 에도 §미측정 에도 setext 가 없어 **"다루지 않음" 과 "지원함" 이 구별되지 않았다.** 두 계약 사이의 틈이지 어느 한쪽의 위반이 아니었다. **독자 피해가 이 축을 (1)번 자리보다 무겁게 만든다**: `---` 가 인용 본문에 글자로 남아 **원문에 없는 대시 세 개가 화면에 나타난다.** 같은 글에서 표기에 따라 제목이 되기도 안 되기도 한다. **최상위 계약의 상속으로 세웠다** — 밑줄 개수 하한 · `*`·`_` 배제 같은 규칙을 다시 쓰지 않고 `markdown-setext-heading-underline` (I1)~(I5) 에서 물려받는다. §역할 이 목록에 대해 이미 내린 판단과 같다. **(I5) 와의 접속 항을 함께 세운 것이 판단이다**: 인용 내용을 줄 단위로 쪼개 블록 판정을 붙이는 **가장 싼 구현이 (I5)(블록 없는 여러 줄 인용은 한 덩어리)를 깨뜨린다.** (I7) 만 재는 게이트에는 그 파손이 보이지 않는다. **`> ---` 단독(인용 안 가로줄)은 범위 밖으로 두되 "유지된다" 로 적지 않았다** — (I7) 구현이 최상위 판정을 재사용하면 함께 움직일 수 있고, 지금 못 박으면 어느 쪽이든 거짓이 될 수 있다. 빈 줄로 갈린 인용 안 setext 도 같은 이유로 미측정에 남겼다. unchecked 0 → 4 (promote 후보에서 내려온다). | §역할 · §동작 · §테스트 현황 · §수용 기준 · §스코프 규칙 · §참고 |
...
- **인용 안 가로줄**(`> ---` 단독)의 기대 산출은 정하지 않는다. HEAD=`5420677` 산출은 `<blockquote>---</blockquote>` 이고 CommonMark 는 `<blockquote><hr /></blockquote>` 지만, **(I7) 착지가 이 자리를 함께 움직일지가 구현 선택에 달려 있어 지금 못 박으면 어느 쪽이든 거짓이 될 위험이 있다.** 같은 판단이 오늘 두 번 더 있었다 (`markdown-emphasis-flanking-punctuation` §짝짓기 축 S_P·P_P · `markdown-setext-heading-underline` 목록 안 setext).
- **빈 줄로 갈린 인용 안 setext**(`> 앞\n>\n> 제목\n> ---`)는 HEAD 산출이 `<blockquote>앞<br /><br />제목<br />---</blockquote>` 다. (I7) 이 이 형태를 포함하는지는 정하지 않는다 — 빈 줄 경계 축((I5)·`:244`)과 겹친다.
