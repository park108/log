...
- [x] FR-01/02/03 변경 적용 후 `git diff --stat .github/workflows/ci.yml` → 1 file / 3 insertions / 3 deletions. (TSK-56 / 5adc828 ack — stat `1 file / 3 insertions(+) / 3 deletions(-)` 일치.)
- [x] FR-05 변경 후 CI run annotation 에서 `Node.js 20 actions are deprecated` 0 건. (TSK-56 run `24701522811` ack — deprecation annotation 0.)
- [ ] FR-06 변경 후 CI run 에서 `lint` / `test` / `build` 3 단계 PASS (baseline 46 files / 376 tests). **[deferred: CI run `24701522811` 에서 lint PASS · test 1 fail (`LogSingle.test.jsx:56` 5016ms timeout) · build skipped — §FR-08 경로로 TSK-20260421-58 `logsingle-prod-server-serial-timeout-diagnosis` 에 이관. 해당 task 완료 후 CI 3단계 PASS 재실측 필요.]**
- [x] FR-07 `package.json` / `package-lock.json` 변경 0 건. (TSK-56 / 5adc828 ack — diff 없음.)

## 수용 기준
- [x] (Must) FR-01 — `.github/workflows/ci.yml:16` `uses: actions/checkout@v6`. (TSK-56 / 5adc828 ack — grep 현 HEAD 재실행: `v4` 0 hits, `v6` 1 hit at `:16`.)
- [x] (Must) FR-02 — `.github/workflows/ci.yml:19` `uses: actions/setup-node@v6`. (TSK-56 / 5adc828 ack — `v4` 0 hits, `v6` 1 hit at `:19`.)
- [x] (Must) FR-03 — `.github/workflows/ci.yml:21` `node-version: '24'`. (TSK-56 / 5adc828 ack — `'20'` 0 hits, `'24'` 1 hit at `:21`.)
- [x] (Must) FR-04 — `git diff --stat` 1 파일 / 3 라인 변경, diff 초과분 0. (TSK-56 / 5adc828 ack — `1 file changed, 3 insertions(+), 3 deletions(-)` 일치.)
- [x] (Must) FR-05 — CI run annotation 에서 `Node.js 20 actions are deprecated` 0 건 (result.md 에 run ID 박제). (TSK-56 run `24701522811` ack — deprecation annotation 0건.)
- [ ] (Must) FR-06 — `lint` / `test` / `build` 3 단계 모두 PASS (result.md 에 run ID + 각 단계 결과 박제). **[deferred: CI run `24701522811` — lint PASS, test 1 fail (`LogSingle.test.jsx:56` 5016ms timeout), build skipped. §FR-08 경로로 TSK-20260421-58 이관. TSK-58 완료 후 재실측 필요.]**
- [x] (Must) FR-07 — `package.json` / `package-lock.json` 변경 0 건. (TSK-56 / 5adc828 ack — git diff 해당 파일 변경 0.)
- [x] (Should) FR-08 — `test` 단계 실패 시 followup 발행 방침 1~2 줄 result.md 에 명시 (성공 시 자동 충족). (TSK-56 ack — result.md §관찰된 이슈 에 followup `10.followups/20260421-0254-ci-logsingle-prod-server-timeout-observed-in-node24-run.md` 분리 박제.)
- [x] (NFR-01) 변경 파일 1 개 / 변경 라인 3 줄 (diff stat 기준). (TSK-56 ack — `1 file / 3+/3-`.)
- [ ] (NFR-02) `test` 단계 실패 수 baseline 대비 동일 또는 개선. **[deferred: TSK-58 LogSingle flake 해소 대기 — baseline run `24700462600` 46 files / 376 tests 0 fail 대비 run `24701522811` 은 1 test fail. FR-06 와 동일 경로로 TSK-58 완료 후 재실측.]**
- [x] (NFR-03) `Node.js 20 actions are deprecated` 0 회. (TSK-56 run `24701522811` ack.)
- [ ] (NFR-04) Node 24 환경에서 `npm ci` / `npm run lint` / `npm test` / `npm run build` 4 명령 성공 (CI run 이 정본). **[deferred: 로컬 Node 24 4명령 전부 PASS (result.md 박제), CI run `24701522811` 은 `npm test` 1 fail 로 정본 기준 미충족. TSK-58 완료 후 재실측.]**
- [x] (NFR-05) 단일 revert commit 으로 원복 가능 (3 라인 diff 범위 내). (TSK-56 / 5adc828 ack — 3 라인 단일 커밋 revert 가능.)
- [x] (NFR-06) result.md 에 (a) CI run ID, (b) 변경 전/후 annotation 개수, (c) 변경 전/후 test files/tests 합계, (d) 사용된 actions 태그(`@v6`) + node-version 값 박제. (TSK-56 ack — result.md §테스트 결과·§Baseline 비교 에 4 항목 전부 박제.)
...
| 2026-04-21 | TSK-20260421-56 / 5adc828 | drift reconcile ack — Must FR-01/02/03/04/05/07 + Should FR-08 + NFR-01/03/05/06 (10건) [x] flip. FR-06 / NFR-02 / NFR-04 3건은 CI run `24701522811` 의 `LogSingle.test.jsx:56` flake 로 `[deferred: TSK-58 완료 후 재실측]` 태깅. 현 HEAD grep: `@v4`/`'20'` 각 0 hits, `@v6`/`'24'` 각 1 hit (L16/L19/L21). 로컬 Node 24 `npm ci / lint / test (375 pass) / build` 전원 PASS. | §테스트 현황·§수용 기준 |
