...
### 3.10.1 [WIP] `navigator.onLine` non-configurable lock 해제 + FR-03 E2E assertion 강화 (REQ-20260420-018)

> 관련 요구사항: REQ-20260420-018 FR-01 ~ FR-05, US-01 ~ US-02

**맥락 (2026-04-20 관측)**: `src/App.test.jsx:23` (첫 online/offline 케이스 종료 직전) 이 `Object.defineProperty(navigator, 'onLine', { value: true, configurable: false })` 로 `navigator.onLine` 을 **non-configurable** 로 잠근다. 결과: 같은 파일 후속 케이스(특히 FR-03 `'updates isOnline state on online/offline event'`) 가 `navigator.onLine` 을 redefine 할 수 없어 `TypeError: Cannot redefine property: onLine` 이 발생하고, 본래 설계였던 E2E 어서트 (`dispatchEvent` → `.div--offline-contents` 토글 관측) 를 **wiring 계약** 수준(핸들러 reference 공유 + throw-free dispatch) 으로 축소해야 했다. §3.10 위 FR-03 "Should" 렌더 전환 어서트가 이 lock 때문에 기본 미수행 상태.

**결함 패턴 (As-Is)**:
```jsx
// src/App.test.jsx 첫 케이스 말미 (:23)
Object.defineProperty(navigator, 'onLine', { value: true, configurable: false });
// ↑ 이후 같은 파일 모든 케이스가 navigator.onLine 을 redefine 할 수 없음
```

**정합 패턴 (To-Be)**:
- `:23` descriptor 완화: `{ value: true, configurable: true }` (1 키 변경).
- 파일 내 `afterAll` 또는 `afterEach` 에 **명시적 복원 훅** 박제 — 후속 케이스의 redefine 자유 보존 + 다른 파일로의 오염 방지 양립:
  ```js
  afterAll(() => {
    Object.defineProperty(navigator, 'onLine', { value: true, configurable: true });
  });
  ```
- (대안, FR-04 Could) setup 단계 getter 기반 격리:
  ```js
  let onLineValue = true;
  Object.defineProperty(navigator, 'onLine', { configurable: true, get: () => onLineValue });
  // 이후 각 케이스는 onLineValue = false; 로 토글
  ```
  — 파일 내부 격리 스코프 도입. FR-01/02 대신 채택 가능 (planner/developer 판정).

**FR-03 E2E 강화 (Should)**: `:164` 케이스 (`'updates isOnline state on online/offline event'`) 본문을 원 설계대로 보강:
1. `navigator.onLine = false` redefine (또는 getter 토글).
2. `window.dispatchEvent(new Event('offline'))`.
3. `await` 후 `document.querySelector('.div--offline-contents')` 존재 확인 (또는 testing-library `getByText` 등가).
4. 복원.

**grep 수용 기준 (FR-05)**:
- `grep -nE "configurable:\\s*false.*onLine|onLine.*configurable:\\s*false" src/App.test.jsx` — expected **0 hits**.
- `grep -n "afterAll\\|afterEach" src/App.test.jsx` — expected ≥ 1 hit (복원 훅 존재).
- `grep -n "\\.div--offline-contents" src/App.test.jsx` — expected ≥ 1 hit (E2E 어서트 강화 적용 시).

**수용 기준 (REQ-20260420-018 §10)**:
- [ ] FR-01 `src/App.test.jsx:23` descriptor = `{ value: true, configurable: true }` (1 키 변경).
- [ ] FR-02 파일 내 `afterAll`/`afterEach` 복원 훅 존재 (`navigator.onLine` = true + configurable 유지).
- [ ] (Should) FR-03 `:164` 케이스에 offline UI 분기 렌더 어서트 추가 (`.div--offline-contents` 또는 등가).
- [ ] (Could) FR-04 getter 격리 대안 채택 시 FR-01/02 대체 — planner/developer 판정 박제.
- [ ] FR-05 grep 쿼리 3종 충족.
- [ ] `npm test -- src/App.test.jsx` 단독 및 `npm test` 풀 스위트 모두 PASS.
- [ ] `src/App.jsx` 무수정 (REQ-20260419-039 완결 범위 유지).

**범위 밖**:
- `src/App.jsx` online/offline 핸들러 로직 변경 (REQ-20260419-039 에서 완결).
- 다른 테스트 파일의 `navigator` mutation 패턴 표준화 — 별 REQ 후보.
- e2e (Playwright) 단의 offline 시나리오 — 현재 unit/jsdom 한정.
- `.div--offline-contents` BEM/CSS modules 마이그레이션 대응 — 별 REQ.

**§3.6 / §3.10 과의 관계**: §3.6 resize listener 카운트와 §3.10 online/offline 바인딩 회귀 어서트는 모두 **사이드이펙트 1회성** 관찰 패턴. 본 §3.10.1 은 §3.10 FR-03 Should 를 "실행 가능" 상태로 해제해 관측 범위를 wiring → E2E 로 승격한다.
...
| 2026-04-20 | (pending, REQ-20260420-018) | §3.10.1 신설 — `src/App.test.jsx:23` `navigator.onLine` non-configurable lock 해제 + `afterAll` 복원 훅 박제 + FR-03 E2E assertion `.div--offline-contents` 렌더 전환 관측 강화 (WIP) | 3.10, 3.10.1 |
