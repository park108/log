...
> **최종 업데이트**: 2026-04-20 (by inspector, WIP — §5.3 신설 (REQ-20260420-023 Search.jsx `document.getElementById` 2건 선언화))
> **상태**: Active (LogList/File/Writer/Search/LogSingle 완료 — ImageSelector(optional §7 미결) 잔여; §5.2 hoverPopup WIP; §5.3 Search.jsx DOM 명령형 WIP)
> **관련 요구사항**:
...
> - `specs/requirements/ready/20260420-search-jsx-getelementbyid-declarative-refactor.md` (REQ-20260420-023, `src/Search/Search.jsx` toListButton 핸들러의 `document.getElementById` 2건 선언적 ref/state 전환; §5.3 에서 관리)
...
- Monitor 도메인 CSS Modules 미그레이션 — 별 트랙.

### 5.3 [WIP] REQ-20260420-023 수용 기준 (`Search.jsx` toListButton 의 `document.getElementById` 2건 선언화)

> 관련 요구사항: REQ-20260420-023 FR-01 ~ FR-04, US-01 ~ US-02

**맥락 (2026-04-20 관측)**: `src/Search/Search.jsx` 의 `toListButton` onClick 핸들러가 하위 `SearchInput` 내부의 `<input id="query-string-by-enter">`, `<input id="query-string-by-button">` 를 부모 측에서 `document.getElementById` 로 조회해 `.value = ""` 로 초기화한다 (2 hits). REQ-20260420-026 (SearchInput `setAttribute` → `className`, commit `e735aea`) 에서 형제 파일은 이미 선언적 전환됐으나 부모에서 자식 내부 DOM 을 직접 뮤테이션하는 명령형 패턴은 잔존. 컴포넌트 경계 파손 + React 19 concurrent rendering 하에서 `.value` mutation 이 리콘실리에이션과 swap-out 될 때 flicker/inconsistency 리스크 + 동일 라우트 중첩 시 id 충돌로 잘못된 요소 조작 위험. 선례: `src/File/FileDrop.jsx` imperative DOM/`classList` → 선언적 React (REQ-20260419-010), `src/common/common.js:hoverPopup` → 선언적 훅/컴포넌트 (REQ-20260420-001, §5.2 WIP).

**grep 실측 (2026-04-20)**:
- `grep -rn "getElementById" src/Search/Search.jsx` → 2 hits (`:113`, `:114`, toListButton 블록).
- `grep -rn "getElementById" src/Search/` → 4 hits: 프로덕션 2 (`Search.jsx`) + 테스트 2 (`SearchInput.test.jsx:92`, `:113` — 블랙박스 검증 용도, 본 REQ scope 밖 유지).

**대체 전술 2 옵션 (planner 결정 — REQ-20260420-023 §13 미결)**:
- **옵션 A — `forwardRef` + `useImperativeHandle`**: `SearchInput` 이 `reset()` 메서드만 노출. 내부 uncontrolled input 구조 유지 → diff 규모 최소. 기존 onSubmit 경로 불변.
- **옵션 B — controlled state 전환**: 상위 `Search.jsx` 에 `queryString` state 추가, `SearchInput` 은 `value` + `onChange` props 수신. state 기반 선언성 최대화. 단 onSubmit 경로 재검토 필요.

**권장 (discovery, REQ-023 §13)**: 옵션 A — 기존 uncontrolled 구조 + forwardRef 가 변경 범위 최소. 단 planner 가 최종 결정.

**FR 요약**:
- **FR-01 DOM 명령형 제거** (Must): `src/Search/Search.jsx` toListButton 블록에서 `document.getElementById` 2건 제거.
- **FR-02 선언적 reset 경로** (Must): SearchInput 이 controlled (`value`/`onChange`) **또는** `useImperativeHandle` 로 `reset()` expose.
- **FR-03 회귀 테스트 유지** (Must): 기존 `Search.test.jsx` "List 클릭 → 입력창 reset" 시나리오 PASS. 시나리오 부재 시 신규 it 추가 (test scope 는 프로덕션 소스 한정 — 테스트 내 `getElementById` 2건은 유지).
- **FR-04 grep 게이트** (Must): `grep -rn "getElementById" src/Search/Search.jsx` → 0 hits.

**수용 기준 (REQ-20260420-023 §10)**:
- [ ] FR-01 ~ FR-04 구현
- [ ] `rg -n "getElementById" src/Search/Search.jsx` → 0 hits
- [ ] `npm test -- --run` 전체 PASS, `src/Search/Search.test.jsx` 회귀 0
- [ ] `npm run build` 성공
- [ ] List 버튼 클릭 → 두 입력창 reset 동작 수동 스모크 또는 자동 테스트 PASS

**범위 밖**:
- SearchInput 의 `<input id="...">` attribute 자체 제거 (별 REQ — anchor 용도 검토 필요).
- 테스트 파일(`SearchInput.test.jsx:92, 113`) 의 `getElementById` 블랙박스 검증 — 유지 (본 REQ 는 프로덕션 소스 한정).
- Playwright/E2E 도입 (REQ-040 React 19 bump 이후 별 REQ).
- `getElementById` 사용처 전반 sweep — 본 REQ 는 `Search.jsx` 한정. 여타 호출자(`src/common/common.js:hoverPopup` 등)는 §5.2 또는 별 REQ 소관.

**선례 참조**:
- `src/File/FileDrop.jsx` imperative DOM/`classList` → 선언적 React (REQ-20260419-010, `specs/requirements/done/2026/04/19/20260419-filedrop-imperative-dom-classlist-react-refactor.md`) — 구조 유사.
- `src/HoverPopup/...` (REQ-20260420-019, hoverPopup 선언화) — 동일 가족, §5.2 WIP.

**hoverPopup (§5.2) 와의 경계**: §5.2 는 공통 helper `hoverPopup(e, id)` + 7 호출자 일괄 이관이 스코프 — 자식 컴포넌트 내부 DOM 을 부모가 명령 조작하는 본 §5.3 패턴과는 대상이 다르다. 두 섹션 모두 "명령형 DOM → 선언적 React" 가족이나 **스코프 중복 없음** (본 spec SSoT 분리 원칙).
...
- §5.3 REQ-20260420-023 의 대체 전술 A(`forwardRef` + `useImperativeHandle`) vs B(controlled state) — planner 결정. 기본 권장 A (diff 최소).
- §5.3 SearchInput 내부 `<input id="query-string-by-enter|button">` 의 id attribute 가 테스트 외 외부 참조(CSS/analytics/href anchor) 에 사용되는지 — 사용이 없으면 후속 REQ 에서 id 제거 검토.
...
| 2026-04-20 | (inspector drift reconcile) | §3 헤더 rename: "(To-Be)" 제거 (planner §4 Cond-3 충족, d0d49c6 선례) | 3 |
| 2026-04-20 | (pending, REQ-20260420-023) | §5.3 신설 — `src/Search/Search.jsx` toListButton 핸들러의 `document.getElementById` 2건(`:113`, `:114`) 선언적 `forwardRef + useImperativeHandle` 또는 controlled state 전환. 테스트 블랙박스 용도(`SearchInput.test.jsx:92, 113`) 유지. 선례: FileDrop(REQ-010) + hoverPopup §5.2. planner 가 A/B 옵션 확정. §1 상태/업데이트, §7 미결 2항 추가, §9 관련 문서 확장. (WIP) | 1, 5.3, 7, 8, 9 |
...
- 선례 요구사항 (done):
  ...
  - `specs/requirements/done/2026/04/19/20260419-filedrop-imperative-dom-classlist-react-refactor.md` (FileDrop imperative DOM/`classList` → 선언적 React, §5.3 선례)
  - `specs/requirements/done/2026/04/20/20260420-hoverpopup-imperative-dom-declarative-refactor.md` (hoverPopup 선언화 — §5.2/§5.3 가족)
- WIP / ready 요구사항:
  - `specs/requirements/ready/20260420-search-jsx-getelementbyid-declarative-refactor.md` (REQ-20260420-023, §5.3 소관)
