...
> **최종 업데이트**: 2026-04-20 (by inspector, drift reconcile — FR-03/04/05/06/07/08 완료 박제)
> **상태**: Active (LogList/File/Writer/Search 완료 — LogSingle FR-01/02 + ImageSelector(optional) 잔여)
> **관련 요구사항**:
...
|------|------------|-----------|-----------|-----------|------|
| `src/Log/LogSingle.jsx:19` | `logItem` | `itemLoadingStatus` (NOT_FOUND / FOUND / DELETED) | `setLogItem(<...>)` + `useEffect` | 렌더 본문 조건부 (`{status === 'FOUND' && <LogItem .../>}`) | **잔여** (FR-01) |
| `src/Log/LogSingle.jsx:20` | `toListButton` | `isLoading` / `queryString.get("search")` | `setToListButton(<...>)` + `useEffect` | 렌더 본문 인라인 조건부 | **잔여** (FR-02) |
...
> grep 실측 (2026-04-20):
> - `grep -rn "setSeeMoreButton\|setToListButton\|setFileUploadUI\|setChangeHistory\|setLogItem" src/` → LogSingle 9 hit (FR-01/02 잔여) + ImageSelector 3 hit (Optional) = 12 총. 나머지 5 컴포넌트 0 hit.
...
- 2026-04-20 실측: **12 hit** (LogSingle 9 = FR-01/02 잔여, ImageSelector 3 = Optional). 5 컴포넌트 스윕 완료로 감소 중. FR-01/02 (LogSingle) 마감 시 ImageSelector 포함 여부(§7 미결)에 따라 0 또는 3 hit 로 수렴.
...
- [ ] `grep -n "setSeeMoreButton\|setToListButton\|setFileUploadUI\|setChangeHistory\|setLogItem" src/` → 0 결과 — 2026-04-20 실측 12 hit (LogSingle 9 + ImageSelector 3). LogSingle FR-01/02 + Optional 결정 후 수렴.
- [ ] FR-01 `src/Log/LogSingle.jsx` `logItem` state 제거 + `itemLoadingStatus` 기반 인라인 조건부 렌더 — **잔여**
- [ ] FR-02 `src/Log/LogSingle.jsx` `toListButton` state 제거 + `isLoading` / `queryString.get("search")` 인라인 조건부 — **잔여**
...
- [x] FR-08 JSX 전용 `useEffect` 블록 ≥ 3 개 제거 — 실측 ≥ 5 (LogList/Search/File×2/Writer). FR-01/02 머지 후 +2.
- [~] 각 컴포넌트 테스트 100% PASS — 렌더 결과(표시 조건, 클릭 핸들러, 스타일) 완전 보존 (NFR-02) — 완료 5 파일 기준 각 task result.md 에서 PASS 박제. LogSingle 잔여.
