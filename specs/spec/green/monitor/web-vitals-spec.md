...
**맥락 (2026-04-20 관측 post-6881e1d)**: `src/Monitor/WebVitalsItem.jsx` 본문 (`:179` 스냅샷) 은 `<li className="li li--monitor-detailpillaritem">{ description }</li>` 로 tooltip 내부에 `description` 을 렌더하며, PropTypes 에서 `description: PropTypes.string` 로 선언돼 있다. 그러나 `src/Monitor/WebVitalsItem.test.jsx` 는 모든 `it` 에 `title=...` 만 전달하고 `description` 을 미전달 → React 는 undefined child 를 무시해 `<li>` 가 **빈 상태로 렌더** 됨. 결과: 본 도메인 hoverPopup 이관(상류 REQ) 직후 `/🟢|🟡|🔴/` 최소 매칭으로 tooltip 어서트를 완화해야 했던 근인.

자매 컴포넌트 `src/Monitor/ApiCallItem.jsx` 는 **데드 prop** 1건: `unit: PropTypes.string` 선언이 본문 어디에서도 참조되지 않음 (grep 0 hits). 호출자가 `unit=` 를 전달해도 무시됨 — 정적 분석 noise.

**결함 매트릭스 (As-Is)**:
| 컴포넌트 | PropTypes 선언 | 본문 사용 | 테스트 전달 | 상태 |
|---|---|---|---|---|
| `WebVitalsItem.jsx` | `description: PropTypes.string` | `:37 / :123 / :136 / :157 / :179` 렌더 | **미전달** (drift) | tooltip `<li>` 빈 렌더 |
| `ApiCallItem.jsx` | `unit: PropTypes.string` | **참조 없음** | — | 데드 prop |
| `VisitorMon.jsx` | (미관측, FR-05 감사 대상) | — | — | 감사 필요 |

**정합 패턴 (To-Be)**:

**FR-01 (Must) — WebVitalsItem.test.jsx description 전달**:
- tooltip 어서트 블록에 `description="..."` 전달 (metric 별 정확한 라벨 또는 정규식 친화 문자열).

**FR-02 (Must) — tooltip description 렌더 어서트 강화**:
- tooltip 내부 description 텍스트 어서트 추가 (`getByText` 또는 등가). 기존 `/🟢|🟡|🔴/` 매칭 유지(회귀 없음).
- 예:
  ```jsx
  render(<WebVitalsItem name="CLS" title="..." description="Cumulative Layout Shift (metric)" />);
  // ...
  expect(within(tooltip).getByText(/Cumulative Layout Shift/)).toBeInTheDocument();
  ```

**FR-03 (Must) — ApiCallItem unit PropTypes 삭제**:
- `src/Monitor/ApiCallItem.jsx` 의 `unit: PropTypes.string` 1줄 삭제. 호출자에서 `unit=` 전달 여부 grep 후 잔재도 정리 (FR-04).

**FR-05 (Should) — Monitor 도메인 PropTypes 감사**:
- `src/Monitor/{WebVitalsItem,ApiCallItem,VisitorMon}.jsx` 각 PropTypes 선언 vs 본문 참조 표 산출. 불일치 건수 기록 → task result.md 박제. 추가 drift 발견 시 별 REQ 승격.

**grep 수용 기준 (FR-06)**:
- `grep -cE "description=" src/Monitor/WebVitalsItem.test.jsx` — expected ≥ 1 hit (테스트 전달 존재).
- `grep -cE "unit:\\s*PropTypes" src/Monitor/ApiCallItem.jsx` — expected **0 hits** (삭제 확인).
...
- PropTypes → TypeScript 전환 — 상위 전략 결정 필요.
- tooltip 의 접근성(`role="tooltip"`, `aria-describedby`) 강화 — 별 followup / `§4.A.9` (REQ-021) 에서 분리.
- `unit` prop 의 재도입 (실제 사용이 필요하다면 별 REQ).

**상류 REQ 와의 관계**:
- Monitor hoverPopup 이관 완료 (`6881e1d`) 후 tooltip 구조가 `useHoverPopup` 훅 기반으로 안정된 시점에 수행. 본 §7.2 가 tooltip 어서트 커버리지를 완결.
