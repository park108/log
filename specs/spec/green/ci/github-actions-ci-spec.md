...
### 3.1.3 [WIP] ESLint v9 + flat config 마이그레이션 CI 영향 (REQ-20260419-003 / REQ-20260420-022 재집행)

> 관련 요구사항: REQ-20260419-003 FR-07, NFR-05; **REQ-20260420-022** FR-06, FR-07 (drift 2차 재집행 CI 측면)

**[REQ-20260420-022 drift 재집행 맥락]**: REQ-003 done 상태에도 `.eslintrc.yml` + `eslint@8.28.0` + `eslint-plugin-jsx-a11y` 미설치가 실측 확인 (SSoT: `common/accessibility-spec.md` §3.4.2). 본 §3.1.3 의 CI 측면 수용 기준은 REQ-022 재집행 PR 에서 재검증. 추가 CI 영향은 없음 (workflow 변경 0, Node 20 유지).
...
### 3.5.4 [WIP] Markdown 링크 유효성 CI 검증 (REQ-20260420-032)

> 관련 요구사항: REQ-20260420-032 FR-01 ~ FR-07, US-01 ~ US-02

**맥락 (2026-04-20 관측)**: `docs/testing/operator-manual-smoke-index.md` 13-row × 평균 2~3 relative 링크 + `specs/spec/blue/**/*.md` 의 `[REQ-...](../../requirements/done/...)` 밀집 relative 링크로 총 30+ 링크가 tracked 문서에 존재한다. 현재 검증은 task 별 수동 `test -e` 루프 (TSK-20260420-21 선례) 뿐이며 PR 단계 자동 적발 없음. 파일 이동/삭제 회귀는 운영자 수동 smoke 까지 드러나지 않는 silent breakage 리스크.

**도구 선택 (FR-01)**:
- **기본 선택: `markdown-link-check`** (Node 스택 동형, `^3.13` 최신). `lychee` (Rust) 선택 시 runner 에 Rust 도구체인 비용 발생 — rationale 필수.
- devDependency 로 `package.json` 에 추가. npx 실행 경로 대신 스크립트 고정.

**npm script (FR-02)**:
```json
"scripts": {
  "docs:link-check": "markdown-link-check -c .markdown-link-check.json docs/**/*.md specs/spec/blue/**/*.md"
}
```

**설정 파일 `.markdown-link-check.json` (FR-03)**:
- `ignorePatterns`: **gitignored 로컬 전용 경로** (`specs/followups/`, `specs/requirements/`, `specs/task/`) — 본 REQ Out-of-Scope (§3.2 제약). 이들은 원격에 없어 외부에서 404.
- `timeout`: `10s`. `retryOn429`: `true`. 외부 URL flake 완화.
- `aliveStatusCodes`: `[200, 206, 301, 302, 304]`.

**CI workflow (FR-04, FR-05)**:
- `.github/workflows/docs-link-check.yml` 신규 파일 또는 기존 `ci.yml` 에 step 추가 (planner 결정).
- PR trigger (`pull_request`). `paths: ['docs/**/*.md', 'specs/spec/blue/**/*.md', '.markdown-link-check.json']` 필터로 불필요 실행 방지.
- exit non-zero → PR 머지 블록 (branch protection 등록 필수).

**외부 URL 정책 (FR-06)**:
- 429/timeout retry + skip 옵션 — flake 방지. PR cost < 2분 목표 (NFR-02).
- 외부 URL 전수 weekly scheduled 검증은 별 REQ 후보 (§13).

**앵커 검증 (FR-07, Could)**:
- `markdown-link-check` 기본 옵션은 `#section` 앵커 미검증. 별 REQ 로 분리 가능 (본 REQ Out-of-Scope).

**§3.1.1 / §3.1.3 / §3.5.3 와의 관계**:
- Lint/Build 잡 변경 없음. 독립 잡으로 추가 (또는 기존 `ci.yml` 에 step 추가).
- `size-limit` / devtools grep 스텝과 직교.

**수용 기준 (REQ-20260420-032 §10)**:
- [ ] `package.json:scripts` 에 `docs:link-check` 추가
- [ ] `.markdown-link-check.json` 존재 + `specs/followups/`, `specs/requirements/`, `specs/task/` ignore pattern 포함
- [ ] `.github/workflows/docs-link-check.yml` (또는 기존 workflow 내 step) 추가
- [ ] 의도적 깨진 링크 smoke 시 CI fail 재현 (PR 직전 실증 후 revert)
- [ ] `npm run docs:link-check` exit 0 (현 baseline)
- [ ] README 또는 CONTRIBUTING.md 에 1줄 안내 (선택)
- [ ] 본 잡 추가 후 PR CI 총 시간 +≤2분 (NFR-02)

**범위 밖**:
- `specs/followups/**`, `specs/requirements/**`, `specs/task/**` — 로컬 전용, 원격 부재.
- README 외부 URL 전수 점검 — 별 REQ 또는 scheduled.
- Markdown 문법 lint (`markdownlint`) — 별 REQ.
- 앵커 검증 — 별 REQ (FR-07 Could).
...
| 2026-04-20 | (pending, REQ-20260420-022) | §3.1.3 확장 — REQ-003 재집행 (drift 2차) CI 맥락 박제. Workflow 변경 없음, 설정 파일 이관으로 v9 흡수, Node 20 유지 전제. SSoT 는 `common/accessibility-spec.md` §3.4.2 (REQ-022 추가 수용 기준 6건 포함). (WIP) | 3.1.3 |
| 2026-04-20 | (pending, REQ-20260420-032) | §3.5.4 신설 — Markdown 링크 유효성 CI 검증. `markdown-link-check` 기본 선택, `.markdown-link-check.json` 설정 (gitignored 로컬 전용 경로 ignore), PR trigger workflow, external URL 429/timeout retry+skip. tracked 문서(`docs/**/*.md` + `specs/spec/blue/**/*.md`) 만 검증 대상. 앵커 검증은 별 REQ (Could). (WIP) | 3.5.4 |
