...
4. **spec 승격 / 격리** —
   ...
   - **`[deferred]` 제외 규약** — inspector 가 운영자 영역 / cross-cutting / 외부 선행 의존 섹션·항목에 `**[deferred: {사유}]**` 마커를 달아두면 판정 제외. 두 가지 문법이 있고 판정기는 둘 다 인식한다:
     - **(섹션 단위)** 섹션 헤더 바로 다음 줄: `**[deferred: 운영자 baseline 수행 대기]**`.
       - 블록 범위: 다음 동급 이상의 `#` 헤더까지. 범위 내 `[WIP]` · `- [ ]` · `To-Be` 전부 제외.
       - 용도: 운영자 / cross-cutting / 외부 선행 의존 섹션 통째 차단.
     - **(라인 단위)** 같은 줄 끝 inline: `- [ ] (Should) console.error 출력 0 — **[deferred: 간접 관측 수단 부재]**`.
       - 범위: 해당 한 라인만 제외. 섹션 단위 차단이 부적절한 **Acceptance 섹션 내 (Should)/(Could) · 수동 검증 라인** 에 사용.
       - 용도: `## 수용 기준` 처럼 Must + Should + Could 혼재 섹션에서 Must 만 ack 하고 나머지는 라인 단위로 제외.
       - 허용 위치: `- [ ]` 체크박스 라인 및 `[WIP]` 태그가 포함된 본문 라인. 섹션 헤더 (`^#+`) 에는 섹션 단위 문법 사용 (라인 단위 쓰지 않음).
     - **판정 스캔 로직**: 각 `[WIP]` · `- [ ]` · `^#+ .*To-Be` 토큰에 대해 "자기 라인에 `[deferred: ` 인라인" 또는 "상위 섹션 헤더 다음 줄 `[deferred: ` 블록" 중 하나라도 해당하면 제외. 둘 다 해당 없으면 카운트.
     - **전체 spec 의 섹션 100% 가 `[deferred]` 면 승격 금지** — 진척 0 은 승격 대상이 아니라 blocked 후보. 라인 단위 deferred 가 혼재해도 active 라인이 0 이면 동일 판단.
