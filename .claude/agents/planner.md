...
3. **작업지시서 작성** — 태스크 1건당 1파일:
   ...
   - 필수 섹션:
     ...
     - `## 범위 밖` — 인접 작업 차단
4. **spec 승격 / 격리** —
   - **전체 태스크화 → 승격**: `mkdir -p` 후 `mv green/F blue/F` (같은 경로, 기존 blue 덮어씀).
   - **부분 (1회차)**: green 에 남김 → 다음 주기 재평가.
   - **부분 (2회차 이상 연속 정체)**: `specs/spec/blocked/` 로 이동 + `{같은경로}_reason.md`. **자동 재시도 중단**.
   - 판정용 상태 파일: `specs/spec/green/.planner-seen` (파일별 last-diff-hash JSON). planner 만 읽고 쓴다.
5. **보고** — `RULE-04` 블록 먼저. 상세: 생성 task 목록, blue 승격 spec, blocked 이동 spec, 잔존 green, 의존성 그래프.
