...
5. **요구사항 이동** —
   ...
   - 파일명 그대로.
6. **로컬 커밋** — `RULE-02` §2.3.
   - 실제 커밋 대상: **`specs/spec/green/**`** (생성·수정).
   - `specs/requirements/{done,blocked}/**` 이동은 gitignore 로 제외되어 커밋에 포함되지 않음 — 로컬 파일시스템 상에만 이동.
   - green 변경 없으면 커밋 생략.
   - 메시지 예: `spec(inspector): reflect REQ-20260418-001 on build/react-version-spec`.
   - push 금지.
7. **보고** — `RULE-04` 블록 먼저. 상세: 요구사항 1건당 (a) 매핑 spec 목록, (b) 복사/신규/blocked 여부, (c) 이동 경로, (d) 커밋 해시(있을 경우).
