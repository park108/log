...
## 절차
1. RULE-03 선결 점검 (`specs/20.req/` 부재 시 즉시 no-op). **Phase 1 은 빈 큐·임계치 무관 항상 수행** (pause lock 만 차단).
2. **Phase 1 — drift reconcile**: green 의 `[WIP]` / `- [ ]` / `To-Be` 스캔 → 각 REQ-ID 에 대응하는 `60.done/task/**/result.md` 커밋 해시 역추적 → 해당 task 의 DoD grep 게이트 현 HEAD 에서 재실행. 전원 PASS 면 `[x]` 플립 + `## 변경 이력` 에 1행 박제. hook-ack (커밋 해시가 HEAD 의 조상 + `npm test/lint/build`·"회귀 0" 문구) 은 Must 주관 혼재 없을 때만 보조 ack. 애매하면 marker 유지. `.inspector-seen` 갱신 — spec 당 1행 덮어쓰기. `stale_cycles += 1` 은 **게이트를 실제로 재실행했는데 ack 0 이고 WIP 불변인 경우에만**. `reconcile: skipped (no delta)` 는 증가시키지 않는다 (RULE-03 (S2) 오탐 방지). 증분의 출처(재실행 실패 / no-delta 스킵)를 헤더에 구분 박제한다.
