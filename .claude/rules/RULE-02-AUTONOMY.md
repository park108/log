...
```
git add <paths>                       # 신규 파일이 있을 때만 필요
git commit -m "..." -- <paths>        # 경로는 반드시 맨 뒤
```

말미 pathspec 이 **범위를 확정한다** — 인덱스에 다른 writer 의 것이 들어와 있어도 그 경로들만 커밋된다. `git add` 는 **신규(미추적) 파일을 인덱스에 올리기 위해서만** 쓰고, 대상은 커밋할 그 경로들로 한정한다.

> **함정 둘, 2026-09-01 실측:**
> - **경로는 맨 뒤다.** `--` 뒤는 전부 pathspec 으로 읽히므로 `git commit -- <path> -m "..."` 는 `-m` 과 메시지를 **파일 이름으로 해석**해 실패한다 (운영자가 직접 걸렸다 — 커밋이 안 된 것이 다행이었다).
> - **신규 파일은 `git add` 없이 커밋되지 않는다.** 미추적 경로는 pathspec 에 매치되지 않아 `did not match any file(s) known to git` 로 실패한다 (developer 실측). 기존 파일만 고쳤다면 `add` 는 불필요하다. 커밋 직후 아래를 확인한다:
