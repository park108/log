...
**커밋은 인덱스가 아니라 경로로 한다.** `git add` 후 `git commit` 대신 `git commit -m "..." -- <paths>` 를 쓴다.

> **경로는 반드시 맨 뒤다.** `--` 뒤는 전부 pathspec 으로 읽히므로 `git commit -- <path> -m "..."` 는 `-m` 과 메시지를 **파일 이름으로 해석**해 실패한다 (2026-09-01 운영자 실측 — 커밋이 안 된 것이 다행이었다). 커밋 직후 아래를 확인한다:
