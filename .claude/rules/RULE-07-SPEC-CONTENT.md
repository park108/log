...
2-bis. **§테스트 현황의 나머지 판정 명령도 실행하고 결과를 박제한다 (2026-09-01 신설).** 실패는 **promote 를 막지 않는다** — 그러나 **기록하지 않는 것은 금지**한다. `RULE-04` notes 에 `diagnostics: M run, K failed` 를 `promote-verify` 와 나란히 적고, 실패한 명령을 `_reason` 이 아니라 **spec §참고 에 평서문으로** 남긴다.

   > **왜 막지 않는가** — §수용 기준 은 그 계약의 **주장**이고 §테스트 현황 의 나머지는 **증거·진단**이다. 진단 하나가 깨졌다고 계약이 거짓이 되지는 않으므로 승격 차단은 과하다. **그러나 침묵은 다르다** — 승격된 문서 안에 거짓인 줄이 표시 없이 남으면, 읽는 사람은 그것을 검증된 것으로 읽는다.
   >
   > **실측 (2026-09-01)** — 막 승격된 `sanitizeHtml` 의 `(정책 표면 크기)` 줄이 **양방향으로** 깨져 있었다. 특이도: 소유 spec 자신이 `rc=0` 이어야 한다고 선언한 배터리 원소 V8 에서 `rc=1`. 민감도: 주석 잔존 + 실제 정책 27→3 붕괴를 함께 넣으면 `perl` 비탐욕 매치가 **주석 배열**을 먼저 잡아 `rc=0` — **막겠다고 선언한 붕괴를 정확히 못 본다.** planner 의 flip-check 가 자발적으로 잡았고, 규약이 요구한 것이 아니었다.
   >
   > 같은 부류가 이날 세 번째다 (`(I6)` 산문 · 물결 계약의 §의존성 · 본 건). **셋 다 §수용 기준 밖이라 promote-verify 가 보지 않는 자리였다.**
